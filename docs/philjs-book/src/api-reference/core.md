...
**Signature**: `(type: string | Component, props: Props | null, ...children: any[]) => JSXElement`

**Description**: Low-level element factory used by the JSX runtime. Useful when you need to build elements dynamically without JSX syntax.

**Example**:
```typescript
import { createElement } from '@philjs/core';

const element = createElement('button', { className: 'primary' }, 'Save');
```

**See also**: [jsx()](#jsx), [Fragment](#fragment)
...
**Description**: Route-aware budget manager used by `performanceBudgets` to store thresholds and generate reports.

**Example**:
```typescript
import { performanceBudgets } from '@philjs/core';

const manager = performanceBudgets;
manager.setBudget('/checkout', { lcp: 2000, tti: 3000 });

const report = manager.check('/checkout');
console.log(report.pass);
```

**Common Methods**:
- `setBudget(route: string, budgets: PerformanceBudgets)`
- `getBudget(route: string)`
- `check(route: string, metrics?: PerformanceMetrics)`

**See also**: [performanceBudgets](#performancebudgets)
...
For detailed accessibility APIs, see [Advanced: Accessibility](../advanced/accessibility.md)
...
For detailed A/B testing APIs, see [Advanced: A/B Testing](../advanced/ab-testing.md)
...
For detailed PPR APIs, see [Advanced: Partial Pre-rendering](../advanced/ppr.md)
...
For detailed Activity Component APIs, see [Advanced: Activity Component](../advanced/activity.md)
...
- [Router API](./router.md) - Router APIs
- [Data API](./data.md) - Data fetching APIs
- [Advanced Topics](../advanced/overview.md) - Advanced features
