...
function createProjectStructure(path, _template) {
...
function generatePackageJson(path, name, _pm) {
    const pkg = {
        name,
        version: "0.1.0",
        type: "module",
...
function generateTsConfig(path) {
    const tsconfig = {
        compilerOptions: {
            target: "ES2022",
            module: "ESNext",
...
function generateViteConfig(path) {
...
function generateGitignore(path) {
...
function generateReadme(path, name) {
    const readme = `# ${name}
...
function generateAppFiles(path, _template) {
    // index.html
...
`;
    ...
    const routeDefinitionsForTypes = [
        {
            path: "/",
            layout: () => null,
            component: () => null,
...
async function prompt(_message, defaultValue) {
