...
export interface BevyApp {
    ...
    addEventListener<T extends BevyEventType>(type: T, listener: BevyEventListener<T>): () => void;
    /** Get the ECS world */
    getWorld(): BevyWorld;
    /** Dispose the app */
 ...
 * Bevy loading state
 */
export type BevyState = 'idle' | 'loading' | 'compiling' | 'instantiating' | 'initializing' | 'running' | 'paused' | 'error' | 'disposed';
/**
 * Entity ID type
 */
...
export type EntityGeneration = number & {
    readonly __brand: 'EntityGeneration';
};
/**
 ...
 * Event type names
 */
export type BevyEventType = 'ready' | 'error' | 'resize' | 'focus' | 'blur' | 'pause' | 'resume' | 'frame' | 'asset-loaded' | 'asset-error' | 'entity-spawned' | 'entity-despawned' | 'component-added' | 'component-removed' | 'custom';
/**
 ...
 * Event data mapping
 */
export type BevyEventData<T extends BevyEventType> = T extends 'ready' ? {
    app: BevyApp;
} : T extends 'error' ? {
    error: Error;
} : T extends 'resize' ? {
    width: number;
    height: number;
} : T extends 'focus' ? {} : T extends 'blur' ? {} : T extends 'pause' ? {} : T extends 'resume' ? {} : T extends 'frame' ? {
    delta: number;
    elapsed: number;
    fps: number;
} : T extends 'asset-loaded' ? {
    path: string;
    type: string;
} : T extends 'asset-error' ? {
    path: string;
    error: Error;
} : T extends 'entity-spawned' ? {
    entity: BevyEntity;
} : T extends 'entity-despawned' ? {
    entityId: EntityId;
} : T extends 'component-added' ? {
    entity: BevyEntity;
    component: string;
} : T extends 'component-removed' ? {
    entity: BevyEntity;
    component: string;
} : T extends 'custom' ? Record<string, unknown> : never;
/**
 * Event listener type
 */
export type BevyEventListener<T extends BevyEventType = BevyEventType> = (event: BevyEvent<T>) => void;
/**
 * Query filter types
 */
 ...
 */
export type BevyAssetType = 'texture' | 'audio' | 'mesh' | 'font' | 'scene' | 'shader' | 'material' | 'animation' | 'binary';
/**
 * Asset metadata
 */
 ...
 */
export type KeyCode = 'KeyA' | 'KeyB' | 'KeyC' | 'KeyD' | 'KeyE' | 'KeyF' | 'KeyG' | 'KeyH' | 'KeyI' | 'KeyJ' | 'KeyK' | 'KeyL' | 'KeyM' | 'KeyN' | 'KeyO' | 'KeyP' | 'KeyQ' | 'KeyR' | 'KeyS' | 'KeyT' | 'KeyU' | 'KeyV' | 'KeyW' | 'KeyX' | 'KeyY' | 'KeyZ' | 'Digit0' | 'Digit1' | 'Digit2' | 'Digit3' | 'Digit4' | 'Digit5' | 'Digit6' | 'Digit7' | 'Digit8' | 'Digit9' | 'Space' | 'Enter' | 'Escape' | 'Tab' | 'Backspace' | 'ArrowUp' | 'ArrowDown' | 'ArrowLeft' | 'ArrowRight' | 'ShiftLeft' | 'ShiftRight' | 'ControlLeft' | 'ControlRight' | 'AltLeft' | 'AltRight' | 'MetaLeft' | 'MetaRight' | 'F1' | 'F2' | 'F3' | 'F4' | 'F5' | 'F6' | 'F7' | 'F8' | 'F9' | 'F10' | 'F11' | 'F12';
/**
 * Mouse button codes
 */
 ...
 */
export type GamepadButton = 'South' | 'East' | 'North' | 'West' | 'LeftTrigger' | 'LeftTrigger2' | 'RightTrigger' | 'RightTrigger2' | 'Select' | 'Start' | 'Mode' | 'LeftThumb' | 'RightThumb' | 'DPadUp' | 'DPadDown' | 'DPadLeft' | 'DPadRight';
/**
 * Gamepad axis codes
 */
export type GamepadAxis = 'LeftStickX' | 'LeftStickY' | 'RightStickX' | 'RightStickY' | 'LeftZ' | 'RightZ';
/**
