 ...
 * Global Godot states
 */
const godotStates = new WeakMap();
/**
 * Get or create Godot state for a canvas
 */
function getGodotState(canvas) {
 ...
 */
async function loadGodotScript(wasmPath) {
 ...
 */
export async function createGodotInstance(canvas, props) {
    ...
    try {
        ...
        const instance = {
            ...
            onSignal: (nodePath, signal, callback) => {
            ...
            emitSignal: (nodePath, signal, ...args) => {
                if (!instance.js) {
                    console.warn('Godot JS interface not available');
                    return;
                }
 ...
 */
export function useGodot(canvas) {
    if (!canvas) {
        return {
            godot: null,
            isLoading: false,
 ...
 */
export function callGodot(canvas, nodePath, method, ...args) {
 ...
 */
export function onGodotSignal(canvas, nodePath, signal, callback) {
    const state = godotStates.get(canvas);
    if (!state?.instance) {
        return () => { };
    }
 ...
 */
export function disposeGodot(canvas) {
    const state = godotStates.get(canvas);
    if (state?.instance) {
        state.instance.stop();
 ...
 */
export function syncToGodot(canvas, nodePath, property, getValue) {
 ...
 */
export function syncFromGodot(canvas, nodePath, signal, setValue) {
    const state = godotStates.get(canvas);
    if (!state?.instance) {
        return () => { };
    }
    return state.instance.onSignal(nodePath, signal, (value) => {
        setValue(value);
    });
}
/**
 ...
 */
export function createGodotBridge(canvas, nodePath, options) {
    const cleanupFns = [];
    if (options.property) {
