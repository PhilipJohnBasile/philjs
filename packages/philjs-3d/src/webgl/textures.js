 ...
 */
function getDefaultTextureOptions() {
    return {
 ...
 */
export function createTextureFromImage(gl, image, options = {}, extensions) {
    ...
    const width = 'width' in image ? image.width : image.videoWidth;
    const height = 'height' in image ? image.height : image.videoHeight;
    ...
    if (opts.anisotropy && opts.anisotropy > 1 && extensions?.anisotropicFilter) {
        const maxAnisotropy = gl.getParameter(extensions.anisotropicFilter.MAX_TEXTURE_MAX_ANISOTROPY_EXT);
        gl.texParameterf(gl.TEXTURE_2D, extensions.anisotropicFilter.TEXTURE_MAX_ANISOTROPY_EXT, Math.min(opts.anisotropy, maxAnisotropy));
    }
 ...
 */
export function loadTexture(gl, url, options = {}, extensions) {
 ...
 */
export function createPlaceholderTexture(gl, color = [255, 255, 255, 255]) {
    const texture = gl.createTexture();
    if (!texture) {
        throw new Error('Failed to create texture');
    }
 ...
 */
export function createDataTexture(gl, data, width, height, options = {}) {
    const texture = gl.createTexture();
    if (!texture) {
        throw new Error('Failed to create texture');
    }
 ...
 */
export function createCubemapTexture(gl, images, options = {}) {
    const texture = gl.createTexture();
    if (!texture) {
 ...
 */
export function updateTexture(gl, textureInfo, image) {
    gl.bindTexture(gl.TEXTURE_2D, textureInfo.texture);
    gl.texImage2D(gl.TEXTURE_2D, 0, textureInfo.format, textureInfo.format, textureInfo.type, image);
    gl.bindTexture(gl.TEXTURE_2D, null);
}
/**
 * Bind a texture to a texture unit
 */
export function bindTexture(gl, textureInfo, unit = 0) {
 ...
 */
export function unbindTexture(gl, unit = 0) {
    gl.activeTexture(gl.TEXTURE0 + unit);
    gl.bindTexture(gl.TEXTURE_2D, null);
}
/**
 * Delete a texture
 */
export function deleteTexture(gl, textureInfo) {
