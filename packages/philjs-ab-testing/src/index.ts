...
export interface MetricResults {
  ...
  variants: Array<{
    variantId: string;
    value: number;
    lift: number; // % change from control
...
export class AssignmentEngine {
  ...
  constructor(storageKey: string = 'philjs_ab_assignments') {
  ...
  private loadAssignments(): void {
    ...
    try {
      ...
      if (stored) {
        const data = JSON.parse(stored) as Assignment[];
  ...
  private isInAllocation(userId: string, experimentId: string, allocation: number): boolean {
    const hash = this.hash(`${userId}:${experimentId}:allocation`);
    return hash < allocation;
  }

  private selectVariant(userId: string, experiment: Experiment): Variant | null {
...
export class FeatureFlagManager {
  ...
  getVariant<T = any>(flagId: string): T | undefined {
    if (!this.isEnabled(flagId)) return undefined;

    const flag = this.flags.get(flagId);
    return flag?.variants as T | undefined;
  }
  ...
  private matchesAudience(rules: AudienceRule[]): boolean {
...
export class StatisticalAnalyzer {
  ...
  zTest(
    ...
    variant: { conversions: number; total: number }
  ...
  tTest(
    ...
    variant: { values: number[] }
  ): { tScore: number; pValue: number; significant: boolean } {
  ...
  calculateSampleSize(
    ...
    significance: number = 0.05 // significance level
  ): number {
    const p1 = baseline;
...
export class ABTestingManager {
  ...
  getVariant(experimentId: string): Variant | null {
    ...
    const variant = this.assignmentEngine.assign(
      experiment,
      this.userId,
      this.context
    );
  ...
  getFeatureVariant<T = any>(flagId: string): T | undefined {
    return this.featureFlagManager.getVariant<T>(flagId);
  }
  ...
  trackConversion(
    ...
    this.eventTracker.trackConversion(
      experimentId,
  ...
  analyzeExperiment(
    experimentId: string,
    controlData: { conversions: number; total: number },
    variantData: { conversions: number; total: number }
  ): { lift: number; pValue: number; significant: boolean } {
...
let globalABManager: ABTestingManager | null = null;
