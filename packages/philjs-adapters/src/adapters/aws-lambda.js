...
export function awsLambdaAdapter(config = {}) {
    const { outDir = '.aws', region = 'us-east-1', runtime = 'nodejs20.x', memorySize = 1024, timeout = 30, handler = 'index.handler', architecture = 'arm64', integration = 'http-api', streaming = false, apiGateway = { type: 'HTTP', cors: true }, cloudfront, s3, layers = [], environment = {}, vpc, roleArn, reservedConcurrency, provisionedConcurrency, deadLetterQueueArn, generateSAM = true, generateCDK = false, generateTerraform = false, functionUrl, tracing, ephemeralStorageSize, } = config;
    return {
        name: 'aws-lambda',
        serverless: true,
        functionConfig: {
        ...
        async adapt() {
            ...
            writeFileSync(join(outDir, 'lambda', 'index.mjs'), generateLambdaHandler());
...
`;
     ...
     */
    function generateStreamingHandler() {
...
`;
     ...
     */
    function generateFunctionUrlHandler() {
...
`;
    ...
    async function generateCDKCode() {
        ...
        writeFileSync(join(outDir, 'cdk', 'stack.ts'), `// PhilJS AWS CDK Stack
// Generated by PhilJS Adapters
...
`);
        ...
        writeFileSync(join(outDir, 'cdk', 'app.ts'), `#!/usr/bin/env node
import * as cdk from 'aws-cdk-lib';
 ...
 */
export function getAWSLambdaContext() {
    return globalThis.__AWS_LAMBDA_CONTEXT__;
}
/**
 ...
 * Get AWS request ID
 */
export function getAWSRequestId() {
    const context = getAWSLambdaContext();
