 ...
 * ```
 */
export function bunAdapter(config: BunAdapterConfig = {}): Adapter {
  const {
    outDir = '.bun',
    port = 3000,
    hostname = '0.0.0.0',
  ...
  const sqliteConfig: SQLiteConfig | null = sqlite
...
`;
   ...
   * Generate bunfig.toml
   */
  function generateBunfigToml(): string {
 ...
 */
export async function bunHash(password: string): Promise<string> {
  if (!isBun()) {
 ...
 */
export async function bunVerify(password: string, hash: string): Promise<boolean> {
  if (!isBun()) {
 ...
 */
export function createBunHandler(config: BunAdapterConfig = {}) {
