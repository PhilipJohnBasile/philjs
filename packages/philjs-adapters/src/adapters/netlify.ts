...
export interface NetlifyBuildConfig {
  ...
  processing?: {
    ...
    html?: { prettyUrls?: boolean };
    images?: { compress?: boolean };
  };
}
 ...
 * ```
 */
export function netlifyAdapter(config: NetlifyAdapterConfig = {}): Adapter & Partial<EdgeAdapter> {
  const {
    outDir = '.netlify',
    edge = false,
    builders = true,
   ...
   */
  async function generateEdgeFunction(outputDir: string): Promise<void> {
    // Generate handler
    writeFileSync(
      join(outputDir, 'handler.ts'),
   ...
   */
  async function generateServerlessFunction(outputDir: string): Promise<void> {
    writeFileSync(
      join(outputDir, 'handler.ts'),
      generateServerlessFunctionHandler()
    );
  }
   ...
   */
  async function generateBackgroundFunction(
    outputDir: string,
    config: BackgroundFunctionConfig
  ): Promise<void> {
    writeFileSync(
      join(outputDir, `${config.name}-background.ts`),
      `// Netlify Background Function: ${config.name}
...
`
   ...
   */
  async function generateScheduledFunction(
    outputDir: string,
    config: ScheduledFunctionConfig
  ): Promise<void> {
    writeFileSync(
      join(outputDir, `${config.name}.ts`),
      `// Netlify Scheduled Function: ${config.name}
...
`;
   ...
   */
  function generateServerlessFunctionHandler(): string {
...
`;
   ...
   * Generate _redirects file
   */
  function generateRedirectsFile(): string {
    const lines: string[] = [];
...
export function getNetlifyContext(): {
  ...
  return {
    ...
    site: (globalThis as any).Netlify?.site,
    account: (globalThis as any).Netlify?.account,
  };
}
 ...
 * ```
 */
export function netlifyImageCDN(
  src: string,
  options: {
    width?: number;
    height?: number;
 ...
 */
export function getNetlifyIdentityUser(context: unknown): NetlifyUser | null {
 ...
 */
export function createFormsHandler(formName: string) {
  return {
    async submit(data: Record<string, unknown>): Promise<Response> {
