...
export function nodeAdapter(config = {}) {
    const { outDir = '.node', port = parseInt(process.env['PORT'] || '3000'), host = '0.0.0.0', https: httpsConfig, compression = true, timeout = 30000, keepAliveTimeout = 5000, headersTimeout = 60000, trustProxy = false, cluster, serveStatic = true, shutdownTimeout = 10000, generatePM2 = false, generateDockerfile = false, express = false, fastify = false, logging = true, healthCheck = '/health', metrics, } = config;
    const staticConfig = typeof serveStatic === 'object'
        ? serveStatic
    ...
    return {
        ...
        async adapt() {
            ...
            writeFileSync(join(outDir, 'server.mjs'), generateServerCode());
            ...
            if (generatePM2) {
                writeFileSync(join(outDir, 'ecosystem.config.cjs'), generatePM2Config());
            }
            // Generate Dockerfile
            if (generateDockerfile) {
                writeFileSync(join(outDir, 'Dockerfile'), generateDockerfileContent());
                writeFileSync(join(outDir, '.dockerignore'), `node_modules
npm-debug.log
.git
.gitignore
.env*
...
`);
            ...
            // Copy static assets
            if (serveStatic) {
                await copyStaticAssets(staticConfig.dir || 'public', join(outDir, 'public'));
            }
