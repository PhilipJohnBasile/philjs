...
export function awsAdapter(config = {}) {
    const { outDir = '.aws', mode = 'lambda', runtime = 'nodejs20.x', memory = 1024, timeout = 30, region = 'us-east-1', architecture = 'arm64', streaming = false, cloudfront, s3Bucket, apiGateway = { type: 'HTTP', corsEnabled: true }, } = config;
    return {
        name: 'aws',
        serverless: true,
        functionConfig: {
    ...
    async function adaptForLambdaEdge() {
        ...
        writeFileSync(join(outDir, 'template.yaml'), generateLambdaEdgeTemplate({
            runtime,
            cloudfront,
            s3Bucket,
        }));
