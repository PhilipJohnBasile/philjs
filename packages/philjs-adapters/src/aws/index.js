...
export function awsAdapter(config = {}) {
    const { outDir = '.aws', mode = 'lambda', runtime = 'nodejs20.x', memory = 1024, timeout = 30, region = 'us-east-1', architecture = 'arm64', streaming = false, cloudfront, s3Bucket, apiGateway = { type: 'HTTP', corsEnabled: true }, static: staticConfig, } = config;
    return {
        name: 'aws',
        serverless: true,
        functionConfig: {
    ...
    async function adaptForLambda() {
        ...
        writeFileSync(join(outDir, 'index.mjs'), generateLambdaHandler(streaming));
        // Create SAM template
        const samTemplateOptions = {
            runtime,
            memory,
            timeout,
            architecture,
        ...
        if (s3Bucket !== undefined)
            samTemplateOptions.s3Bucket = s3Bucket;
        writeFileSync(join(outDir, 'template.yaml'), generateSAMTemplate(samTemplateOptions));
    ...
    async function adaptForLambdaEdge() {
        ...
        writeFileSync(join(outDir, 'index.mjs'), generateLambdaEdgeHandler());
        // CloudFormation template for Lambda@Edge + CloudFront
        const edgeTemplateOptions = {
            runtime,
        };
        if (cloudfront !== undefined)
            edgeTemplateOptions.cloudfront = cloudfront;
        if (s3Bucket !== undefined)
            edgeTemplateOptions.s3Bucket = s3Bucket;
        writeFileSync(join(outDir, 'template.yaml'), generateLambdaEdgeTemplate(edgeTemplateOptions));
    }
    async function adaptForAmplify() {
        // AWS Amplify configuration
        writeFileSync(join(outDir, 'amplify.yml'), generateAmplifyConfig());
        // Server handler
        mkdirSync(join(outDir, 'amplify-ssr'), { recursive: true });
        writeFileSync(join(outDir, 'amplify-ssr', 'index.mjs'), generateAmplifyHandler());
        // Copy static assets
        const staticDir = staticConfig?.assets || 'public';
        if (existsSync(staticDir)) {
...
function generateLambdaHandler(streaming) {
    if (streaming) {
        return `
...
function generateAmplifyHandler() {
    return `
...
function generateAmplifyConfig() {
    return `version: 1
frontend:
  phases:
    preBuild:
...
export function getAWSContext() {
    return globalThis.__AWS_CONTEXT__;
}
...
// S3 URL helper
export function getS3AssetUrl(key, bucket, region = 'us-east-1') {
    return `https://${bucket}.s3.${region}.amazonaws.com/${key}`;
}
// CloudFront signed URL helper
export function signCloudFrontUrl(url, keyPairId, privateKey, expiresAt) {
