 ...
 */
export function getGeoLocation(request, platformContext) {
...
function extractCloudflareGeo(request, geo) {
    const cf = request.cf;
    if (cf) {
        if (cf.country !== undefined)
            geo.country = cf.country;
        if (cf.region !== undefined)
            geo.region = cf.region;
        if (cf.city !== undefined)
            geo.city = cf.city;
        if (cf.postalCode !== undefined)
            geo.postalCode = cf.postalCode;
        if (cf.latitude)
            geo.latitude = parseFloat(cf.latitude);
        if (cf.longitude)
            geo.longitude = parseFloat(cf.longitude);
        if (cf.timezone !== undefined)
            geo.timezone = cf.timezone;
        if (cf.continent !== undefined)
            geo.continent = cf.continent;
        if (cf.colo !== undefined)
            geo.colo = cf.colo;
        if (cf.metroCode !== undefined)
    ...
    if (geo.country) {
        ...
        if (geo.continent) {
            ...
            if (continentName !== undefined)
                geo.continentName = continentName;
        }
    }
    return geo;
}
function extractVercelGeo(request, geo) {
    const headers = request.headers;
...
function extractNetlifyGeo(request, context, geo) {
    const netlifyGeo = context?.geo;
    if (netlifyGeo) {
        if (netlifyGeo.country?.code !== undefined)
            geo.country = netlifyGeo.country.code;
        if (netlifyGeo.country?.name !== undefined)
            geo.countryName = netlifyGeo.country.name;
        if (netlifyGeo.city !== undefined)
            geo.city = netlifyGeo.city;
        if (netlifyGeo.subdivision?.code !== undefined)
            geo.region = netlifyGeo.subdivision.code;
        if (netlifyGeo.latitude !== undefined)
            geo.latitude = netlifyGeo.latitude;
        if (netlifyGeo.longitude !== undefined)
            geo.longitude = netlifyGeo.longitude;
        if (netlifyGeo.timezone !== undefined)
...
function extractDenoGeo(request, geo) {
...
function extractGenericGeo(request, geo) {
 ...
 */
export function applyGeoRouting(request, geo, config) {
 ...
 */
export function createGeoRoutingMiddleware(config) {
 ...
 */
export function findBestRegion(geo, regions) {
    ...
    if (geo.continent) {
        ...
        const continentMatch = healthyRegions.find(r => r.countries.some(c => continentCountries.includes(c)));
        if (continentMatch) {
 ...
 */
export function createLatencyRouter(config) {
    ...
    return {
        ...
        setRegionHealth(regionId, healthy) {
 ...
 */
export function selectGeoVariant(request, geo, config) {
 ...
 */
export function createVariantCookie(config, variant) {
    const cookieName = config.cookieName || `ab_${config.name}`;
 ...
 */
export function findNearestLocation(userLat, userLon, locations) {
 ...
 */
export function addGeoHeaders(headers, geo) {
    if (geo.country)
        headers.set('X-Geo-Country', geo.country);
    if (geo.region)
        headers.set('X-Geo-Region', geo.region);
    if (geo.city)
        headers.set('X-Geo-City', geo.city);
    if (geo.continent)
        headers.set('X-Geo-Continent', geo.continent);
    if (geo.timezone)
        headers.set('X-Geo-Timezone', geo.timezone);
    if (geo.isEU !== undefined)
        headers.set('X-Geo-EU', geo.isEU.toString());
    if (geo.latitude !== undefined)
        headers.set('X-Geo-Latitude', geo.latitude.toString());
    if (geo.longitude !== undefined)
        headers.set('X-Geo-Longitude', geo.longitude.toString());
}
// ============================================================================
// Exports
// ============================================================================
