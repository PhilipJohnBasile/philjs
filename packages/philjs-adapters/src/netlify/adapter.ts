...
export function netlifyAdapter(config: NetlifyConfig = {}): Adapter & Partial<EdgeAdapter & ServerlessAdapter> {
  const {
    outDir = '.netlify',
    edge = false,
  ...
  return {
    ...
    async adapt() {
      ...
      if (redirects || rewrites) {
        writeFileSync(
          join(outDir, 'publish', '_redirects'),
          generateRedirects()
        );
      }
...
`;
    ...
    writeFileSync(
      join(edgeFunctionDir, 'philjs.ts'),
      edgeFunctionCode
    );
    ...
    writeFileSync(
      join(edgeFunctionDir, 'manifest.json'),
...
`;
  ...
  function hasConditions(items: NetlifyConfig['redirects'] = []): boolean {
  ...
  function createBlobHelpers() {
    return {
      ...
      async set(key: string, data: string | Blob) {
        const { getStore } = await import('@netlify/blobs');
        const store = getStore('philjs');
...
export function getNetlifyContext() {
  return (globalThis as any).netlifyContext;
}
