...
export function netlifyAdapter(config = {}) {
    const { outDir = '.netlify', edge = false, builders = true, excludedPaths = [], headers = [], redirects = [], forms = false, identity = false, } = config;
    return {
        name: 'netlify',
        edge,
        async adapt() {
            ...
            if (edge) {
                writeFileSync(join(functionsDir, 'handler.ts'), generateEdgeFunctionHandler());
                // Edge function manifest
                writeFileSync(join(functionsDir, 'manifest.json'), JSON.stringify({
                    functions: [{
                            function: 'handler',
                            path: '/*',
                ...
                writeFileSync(join(functionsDir, 'handler.ts'), generateServerlessFunctionHandler(builders));
            }
            // Copy static assets
...
function generateServerlessFunctionHandler(useBuilder) {
