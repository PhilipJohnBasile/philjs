...
export function railwayAdapter(config = {}) {
    const { outDir = '.railway', nodeVersion = '20', port = 3000, docker, railway, nixpacks, staticFiles, compression = true, gracefulShutdown = {
        timeout: 30000,
    ...
    return {
        ...
        async adapt() {
            ...
            if (docker !== false) {
                ...
                writeFileSync(join(outDir, '.dockerignore'), generateDockerignore());
            }
            // Generate Railway.toml
            writeFileSync(join(outDir, 'railway.toml'), generateRailwayToml());
            // Generate Nixpacks configuration
            if (nixpacks) {
                writeFileSync(join(outDir, 'nixpacks.toml'), generateNixpacksToml());
            }
            // Generate server entry point
            writeFileSync(join(outDir, 'server.js'), generateServerCode());
            ...
            writeFileSync(join(outDir, 'DEPLOY.md'), generateDeploymentGuide());
