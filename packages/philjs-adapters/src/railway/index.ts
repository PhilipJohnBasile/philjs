...
export function railwayAdapter(config: RailwayConfig = {}): Adapter {
  ...
  function generateDockerfile(): string {
    // docker is guaranteed to not be false when this function is called
    const dockerConfig = typeof docker === 'object' && docker ? docker : {};
    const baseImage = dockerConfig.baseImage || `node:${dockerConfig.nodeVersion || nodeVersion}-alpine`;
    const packages = dockerConfig.packages || [];
    const buildArgs = dockerConfig.buildArgs || {};
    const envVars = dockerConfig.env || {};
...
# Expose port
EXPOSE ${port}
${dockerConfig.exposePorts?.map(p => `EXPOSE ${p}`).join('\n') || ''}
