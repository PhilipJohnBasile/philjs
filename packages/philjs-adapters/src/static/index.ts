...
export function staticAdapter(config: StaticConfig = {}): Adapter {
  const {
    outDir = 'dist',
    pages = [],
    fallback = 'index.html',
  ...
  async function prerenderPage(route: string) {
    try {
...
export async function prerender(routes: string[]): Promise<Map<string, string>> {
...
export function getStaticPaths<T extends Record<string, string>>(
  paths: T[]
): { paths: T[]; fallback: boolean } {
  return {
    paths,
    fallback: false,
  };
