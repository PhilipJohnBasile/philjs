 ...
 * ```
 */
export async function createBuildManifest(options) {
 ...
 */
async function findPrerenderedPages(dir) {
    const prerendered = [];
 ...
 * ```
 */
export async function copyStaticAssets(source, destination, options = {}) {
 ...
 * ```
 */
export async function optimizeAssets(dir, options = {}) {
    const { minifyHtml = true, minifyCss = true, minifyJs = true, compressImages = false, hashFilenames = false, inlineAssets = false, inlineThreshold = 4096, sourceMaps = false, } = options;
    let optimized = 0;
    let savedBytes = 0;
    if (!existsSync(dir)) {
 ...
 */
export function generateHashedFilename(filename, content) {
 ...
 */
export function createOutputStructure(baseDir, structure = {}) {
    const { functions = true, static: staticDir = true, prerendered = true, edge = false, } = structure;
 ...
 */
export function getCacheControl(filePath, options = {}) {
