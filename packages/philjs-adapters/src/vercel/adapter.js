...
export function vercelAdapter(config = {}) {
    const { outDir = '.vercel/output', edge = false, regions = edge ? ['iad1'] : [], memory = 1024, maxDuration = edge ? 30 : 60, isr, images, crons, kv, blob, edgeConfig, rewrites, redirects, headers, splitRoutes = false, nodeVersion = '20.x', sourceMaps = true, } = config;
    return {
        name: 'vercel',
        edge,
        ...(edge && {
    ...
    function generateConfig() {
        ...
        if (redirects && redirects.length > 0) {
            routeConfig.routes.push(...redirects.map(redirect => ({
                src: redirect.source,
        ...
        if (headers && headers.length > 0) {
            routeConfig.routes.push(...headers.map(header => ({
                src: header.source,
        ...
        if (rewrites && rewrites.length > 0) {
            routeConfig.routes.push(...rewrites.map(rewrite => ({
                src: rewrite.source,
...
export async function revalidatePath(path, type = 'page') {
