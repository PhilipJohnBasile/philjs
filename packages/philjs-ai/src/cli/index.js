...
function createProgram() {
    ...
    componentCmd
        ...
        await runWithSpinner('Generating component...', async () => {
            ...
            if (options.tests !== undefined)
                config.includeTests = options.tests;
            if (options.styles)
                config.style = { approach: options.styles };
    ...
    pageCmd
        ...
        await runWithSpinner('Generating page...', async () => {
            ...
            if (options.type)
                pageConfig.type = options.type;
            if (options.loader)
                pageConfig.dataLoading = { source: 'api', loadingStates: true };
            if (options.action !== undefined)
                pageConfig.includeActions = options.action;
    ...
    apiCmd
        ...
        await runWithSpinner('Generating API...', async () => {
            ...
            if (options.database)
                apiConfig.database = options.database;
            if (options.openapi !== undefined)
                apiConfig.openapi = options.openapi;
    ...
    program
        ...
        await runWithSpinner('Reviewing code...', async () => {
            ...
            for (const file of files) {
                ...
                const result = await engine.reviewCode(code, file, aspects);
                if (options.json) {
 ...
 */
function inferComponentName(description) {
 ...
 */
function inferPageName(description) {
