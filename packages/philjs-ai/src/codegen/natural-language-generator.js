...
Return JSON:
{
  ...
  "accessibility": ["aria-busy when loading", "disabled state"]
     ...
     */
    async refine(previousResult, refinement, options) {
...
Return the refined code.`;
     ...
     */
    async startConversation(initialDescription, options) {
     ...
     */
    async continueConversation(followUp, options) {
        if (!this.conversationContext) {
     ...
     */
    async generateVariants(description, variants, options) {
     ...
     */
    async generateFromExample(example, description, options) {
...
Return the new code that follows the example pattern.`;
    ...
    // ============================================================================
    buildGenerationPrompt(description, intent, options) {
...
- Make it reusable`,
    ...
    getSystemPrompt(intent, options) {
...
Apply requested changes incrementally.`;
    ...
    buildConversationPrompt(followUp) {
        if (!this.conversationContext) {
            return followUp;
        }
...
Apply the request to the current code and return the updated version.`;
    ...
    updateConversationContext(message, code, intent) {
        if (!this.conversationContext)
            return;
        // Add user message
        ...
        // Add artifact
        const existingArtifact = this.conversationContext.artifacts.find(a => a.name === intent.suggestedName);
        if (existingArtifact) {
    ...
    fallbackIntentParsing(description) {
    ...
    extractExplanation(response, description) {
    ...
    async generateTests(code, intent) {
...
Return only the test code.`;
    ...
    async generateSuggestions(code, intent) {
 ...
 */
export function createNaturalLanguageGenerator(provider, options) {
    return new NaturalLanguageGenerator(provider, options);
}
/**
 * Quick generation helper
 */
export async function generateFromNaturalLanguage(provider, description, options) {
    const generator = new NaturalLanguageGenerator(provider);
 ...
 */
export async function parseCodeIntent(provider, description) {
    const generator = new NaturalLanguageGenerator(provider);
    return generator.parseIntent(description);
}
//# sourceMappingURL=natural-language-generator.js.map