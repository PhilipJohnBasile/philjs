...
export function useCopilotAction(action: CopilotAction) {
    effect(() => {
        actions.update(a => [...a, action]);
        return () => {
            actions.update(a => a.filter(x => x.name !== action.name));
        };
    });
}
...
export function useCopilotContext(name: string, context: CopilotContext) {
    effect(() => {
        contexts.update(c => new Map(c).set(name, context));
        return () => {
            contexts.update(c => { const m = new Map(c); m.delete(name); return m; });
        };
    });
}
...
export function useCopilotChat() {
    ...
    const sendMessage = async (content: string) => {
        ...
        messages.update(m => [...m, { role: 'assistant', content: response }]);
        isLoading.set(false);
    };
