...
export class DocumentStore {
    documents = new Map();
    open(uri, content, version, languageId) {
        this.documents.set(uri, { content, version, languageId });
    }
    update(uri, content, version) {
...
export class LSPHandlers {
        ...
        try {
            ...
            return {
                ...
                items: completions.map((c, i) => {
                    ...
                    if (c.documentation)
                        item.documentation = { kind: 'markdown', value: c.documentation };
     ...
     */
    async handleSignatureHelp(params) {
        const doc = this.documents.get(params.textDocument.uri);
        if (!doc) {
            return null;
        }
     ...
     */
    async handleInlineCompletion(params) {
        const doc = this.documents.get(params.textDocument.uri);
        if (!doc) {
            return { items: [] };
        }
    ...
    // ============ Helper Methods ============
    getPrefix(content, position) {
        const lines = content.split('\n');
        const line = lines[position.line] || '';
        return line.slice(0, position.character);
    }
    getTextBeforePosition(content, position) {
        const lines = content.split('\n');
    ...
    getTextAfterPosition(content, position) {
        const lines = content.split('\n');
        const currentLine = lines[position.line] || '';
    ...
    getFunctionNameAtPosition(content, position) {
        const lines = content.split('\n');
        const line = lines[position.line];
        if (!line)
            return null;
    ...
    hasReactPatterns(code) {
        return (code.includes('useState') ||
            code.includes('useEffect') ||
            code.includes('useMemo') ||
            code.includes('useCallback'));
    }
    async getQuickFixes(diagnostic, content, uri) {
        const fixes = [];
 ...
 */
export function createLSPHandlers(provider) {
