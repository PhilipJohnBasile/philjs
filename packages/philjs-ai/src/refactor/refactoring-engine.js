...
export class RefactoringEngine {
    ...
    async analyze(code, options) {
        ...
        // Identify quick wins
        const quickWins = filteredSuggestions.filter(s => s.effort === 'trivial' || s.effort === 'easy' && s.impact === 'high');
        // Generate summary
     ...
     */
    async createPlan(suggestions, name) {
        ...
        const totalScore = suggestions.reduce((sum, s) => sum + effortScores[s.effort], 0);
        const totalEffort = totalScore <= 3 ? 'trivial' :
            totalScore <= 8 ? 'easy' :
        ...
        const risk = hasCritical || hasBreakingChanges ? 'high' :
     ...
     */
    async autoFix(code, suggestions) {
        ...
        for (const suggestion of sortedSuggestions) {
            try {
                ...
                if (fixedCode.includes(suggestion.before.code)) {
                    fixedCode = fixedCode.replace(suggestion.before.code, suggestion.after.code);
                    applied.push(suggestion.id);
                    if (suggestion.breakingChanges?.length) {
                        warnings.push(`Applied ${suggestion.title} which may have breaking changes: ${suggestion.breakingChanges.join(', ')}`);
                    }
                }
                else {
                    skipped.push({
     ...
     */
    async getDetailedFix(code, suggestion) {
...
Return JSON:
{
  ...
  "explanation": "detailed explanation"
     ...
     */
    async suggestForSelection(code, selection, instruction) {
...
Return JSON array of RefactoringSuggestion objects.`;
    ...
    async analyzeWithAI(code, options) {
...
Focus on practical improvements.`,
    ...
    filterSuggestions(suggestions, options) {
        ...
        if (options?.focusCategories?.length) {
            filtered = filtered.filter(s => options.focusCategories.includes(s.category));
        }
    ...
    buildCategorySummary(suggestions) {
        const categories = [
        ...
        for (const category of categories) {
            ...
            const maxSeverity = categorySuggestions.reduce((max, s) => (severityOrder[s.severity] ?? 0) > (severityOrder[max] ?? 0) ? s.severity : max, 'info');
    ...
    getVerificationSteps(suggestion) {
        const steps = ['Run TypeScript compiler to check for errors'];
    ...
    generateSummary(suggestions, qualityScore) {
 ...
 */
export async function analyzeForRefactoring(provider, code, options) {
    const engine = new RefactoringEngine(provider);
 ...
 */
export async function autoFixCode(provider, code, suggestions) {
    const engine = new RefactoringEngine(provider);
