 ...
 */
async function parseSchema(schema, type, provider) {
...
export class SchemaToComponentGenerator {
     ...
     */
    async generateCRUD(schema, options) {
        // Parse schema
        const parsedSchema = await parseSchema(schema, options.schemaType, this.provider);
        if (options.entityName) {
            parsedSchema.name = options.entityName;
        ...
        const [createForm, editForm, list, detail, deleteConfirm] = await Promise.all([
            this.generateComponent(parsedSchema, 'form', { ...options, formMode: 'create' }),
            this.generateComponent(parsedSchema, 'form', { ...options, formMode: 'edit' }),
     ...
     */
    buildComponentPrompt(schema, type, options) {
...
`;
     ...
     * Generate props interface
     */
    generatePropsInterface(schema, type) {
...
`;
     ...
     */
    async generateDeleteConfirm(schema, options) {
        const name = schema.name;
...
`;
     ...
     */
    async generateCRUDPage(schema, options) {
        const name = schema.name;
        const code = `import { signal } from 'philjs-core';
...
`;
     ...
     * Generate index file
     */
    generateIndexFile(entityName, components) {
...
`;
     ...
     * Generate documentation
     */
    generateDocumentation(schema, components) {
...
`;
     ...
     */
    generateExample(name, type, schema) {
        switch (type) {
            case 'form':
 ...
 */
export function createSchemaToComponentGenerator(provider, options) {
    return new SchemaToComponentGenerator(provider, options);
}
/**
 * Quick schema to component helper
 */
export async function generateComponentsFromSchema(provider, schema, options) {
    const generator = new SchemaToComponentGenerator(provider);
 ...
 */
export async function generateCRUDFromSchema(provider, schema, options) {
    const generator = new SchemaToComponentGenerator(provider);
 ...
 */
export async function generateFromJSONSchema(provider, jsonSchema, componentTypes) {
    const generator = new SchemaToComponentGenerator(provider);
 ...
 */
export async function generateFromGraphQL(provider, graphqlSchema, componentTypes) {
    const generator = new SchemaToComponentGenerator(provider);
...
//# sourceMappingURL=schema-to-component.js.map