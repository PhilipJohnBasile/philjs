...
const PHILJS_COMPONENTS: ComponentSuggestion[] = [
  {
    name: 'Button',
...
export class ComponentSuggester {
  ...
  private componentCache: Map<string, ComponentSuggestion[]>;
...
Return JSON array of ComponentSuggestion objects.`;
    ...
    const suggestions = extractJSON<ComponentSuggestion[]>(response);
    return suggestions || [];
  }
   ...
   */
  async detectPatterns(code: string): Promise<PatternDetection[]> {
...
Return JSON array.`;
    ...
    return extractJSON<PatternDetection[]>(response) || [];
  }
  ...
  async completeComponent(
    ...
    const staticMatches: ComponentSuggestion[] = [];
...
Return JSON array of ComponentSuggestion objects.`;
    ...
    return extractJSON<ComponentSuggestion[]>(response) || [];
  }
...
Return JSON array of ComponentSuggestion objects for appropriate children.`;
  ...
  private analyzeContext(context: SuggestionContext): ContextAnalysis {
    ...
    // Detect context type
    let contextType: ContextAnalysis['contextType'] = 'unknown';
    let parentComponent: string | undefined;
    const siblingComponents: string[] = [];
  ...
  private findMissingCommon(content: string, existingComponents: string[]): string[] {
...
- tags: relevant tags`;
  ...
  private getCodeAroundCursor(context: SuggestionContext): string {
 ...
 */
export function createComponentSuggester(
  provider: AIProvider,
 ...
 */
export async function detectUIPatterns(
  provider: AIProvider,
  code: string
