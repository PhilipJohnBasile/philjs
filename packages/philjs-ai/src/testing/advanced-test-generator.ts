...
export class AdvancedTestGenerator {
  ...
  async generateTestSuite(
    code: string,
    options?: AdvancedTestGenOptions
...
Return the integration test code.`;
  ...
  async generateE2EScenarios(
    ...
    options?: { framework?: 'playwright' | 'cypress' }
  ): Promise<E2EScenario[]> {
...
Return JSON array of E2EScenario objects.`;
    ...
    return extractJSON<E2EScenario[]>(response) || [];
  }
...
Return JSON array of A11yTest objects.`;
    ...
    return extractJSON<A11yTest[]>(response) || [];
  }
...
Return the snapshot test code.`;
  ...
  async generatePerformanceTests(
    componentCode: string,
    options?: { framework?: 'vitest' | 'jest'; thresholds?: Record<string, number> }
  ): Promise<string> {
    const framework = options?.framework || 'vitest';
...
Return JSON:
{
  ...
  private async generateMainTestFile(
    code: string,
    framework: TestFramework,
    types: TestType[],
    style: string,
...
Return the complete test file.`;
  ...
  private getTypeDescription(type: TestType): string {
  ...
  private getTestSystemPrompt(framework: TestFramework): string {
...
Include setup and teardown when needed.`;
  ...
  private extractTestCases(testFile: string, types: TestType[]): TestCaseInfo[] {
  ...
  private inferCategory(testName: string): TestCategory {
    const lower = testName.toLowerCase();

    if (lower.includes('render')) return 'rendering';
    if (lower.includes('click') || lower.includes('interact')) return 'interaction';
    if (lower.includes('state')) return 'state';
    if (lower.includes('prop')) return 'props';
    if (lower.includes('event') || lower.includes('emit')) return 'events';
    ...
    if (lower.includes('route') || lower.includes('navigate')) return 'routing';
    if (lower.includes('error') || lower.includes('fail')) return 'error-handling';
    if (lower.includes('edge') || lower.includes('empty') || lower.includes('null')) return 'edge-cases';
    if (lower.includes('accessible') || lower.includes('a11y')) return 'accessibility';
    if (lower.includes('performance') || lower.includes('fast')) return 'performance';

    return 'rendering';
  }

  private inferPriority(testName: string, category: TestCategory): TestCaseInfo['priority'] {
    if (category === 'error-handling') return 'critical';
    if (category === 'accessibility') return 'high';
    if (category === 'edge-cases') return 'high';
    if (testName.toLowerCase().includes('should')) return 'high';
    if (category === 'performance') return 'medium';

    return 'medium';
  }

  private inferTestType(testName: string, availableTypes: TestType[]): TestType {
    const lower = testName.toLowerCase();

    if (lower.includes('e2e') || lower.includes('workflow')) return 'e2e';
  ...
  private async generateMocks(code: string, framework: TestFramework): Promise<MockFile[]> {
...
Return JSON array of MockFile objects.`;
    ...
    return extractJSON<MockFile[]>(response) || [];
  }

  private async generateFixtures(code: string): Promise<FixtureFile[]> {
...
Return JSON array of FixtureFile objects.`;
    ...
    return extractJSON<FixtureFile[]>(response) || [];
  }

  private async generateTestUtils(code: string, framework: TestFramework): Promise<string> {
...
Return the test utilities code.`;
  ...
  private async getSuggestions(code: string, testCases: TestCaseInfo[]): Promise<string[]> {
    const suggestions: string[] = [];
  ...
  private generateFileName(code: string, framework: TestFramework): string {
 ...
 */
export function createAdvancedTestGenerator(
  provider: AIProvider,
 ...
 */
export async function generateUnitTestsForCode(
  provider: AIProvider,
  code: string,
 ...
 */
export async function generateE2ETestScenarios(
  provider: AIProvider,
  appDescription: string
): Promise<E2EScenario[]> {
  const generator = new AdvancedTestGenerator(provider);
