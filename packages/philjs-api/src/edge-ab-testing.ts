 ...
 */
function matchesTargeting(rules: TargetingRules | undefined, context: EdgeContext): boolean {
 ...
 * A/B testing middleware
 */
export function abTestingMiddleware(options: ABTestingOptions): EdgeMiddleware {
  const {
    experiments,
    onAssignment,
    cookiePath = '/',
 ...
 */
export function injectVariantData(html: string, assignments: Record<string, ExperimentAssignment>): string {
  const script = `
    <script>
 ...
 */
export function variantRewriteMiddleware(
  experimentId: string,
 ...
 */
export function multivariateTestingMiddleware(
