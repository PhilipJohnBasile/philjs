...
export function conditionalMiddleware(
  condition: (context: APIContext) => boolean,
...
export function geolocationMiddleware(
  options: GeolocationMiddlewareOptions = {}
): Middleware {
  const { provider, fallback, addToHeaders = false } = options;

  return async (context, next) => {
    let geo: GeolocationData = fallback || {};
    ...
    if (provider) {
      ...
      const cf = (context.request.raw as any).cf;
      if (cf) {
        // Cloudflare Workers
        geo = {
          country: cf.country,
 ...
 * ```
 */
export function abTestMiddleware(options: ABTestMiddlewareOptions): Middleware {
  ...
  return async (context, next) => {
    ...
    // Add to context
    (context as any).abTests = abTests;
...
export function rateLimitMiddleware(options: RateLimitOptions): Middleware {
  ...
  return async (context, next) => {
    ...
    const response = count > limit
      ? (onLimitExceeded?.(context) || new Response(
...
export interface SecurityHeadersOptions {
  ...
  strictTransportSecurity?: string | { maxAge: number; includeSubDomains?: boolean; preload?: boolean };
  ...
  return async (context, next) => {
    ...
    if (strictTransportSecurity) {
      const hsts = typeof strictTransportSecurity === 'string'
        ? strictTransportSecurity
        : `max-age=${strictTransportSecurity.maxAge}${
            strictTransportSecurity.includeSubDomains ? '; includeSubDomains' : ''
          }${strictTransportSecurity.preload ? '; preload' : ''}`;
 ...
 * ```
 */
export function requestIDMiddleware(options: RequestIDOptions = {}): Middleware {
  const {
 ...
 * ```
 */
export function compressionMiddleware(options: CompressionOptions = {}): Middleware {
  const {
    threshold = 1024,
