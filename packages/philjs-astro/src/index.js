 ...
 */
export function modifiedJulianDate(date) {
 ...
 */
export function equatorialToHorizontal(eq, location, date) {
 ...
 */
export function horizontalToEquatorial(hz, location, date) {
 ...
 */
export function eclipticToEquatorial(ecl, date) {
 ...
 */
export function equatorialToGalactic(eq) {
 ...
 */
export function getPlanetPosition(body, date) {
    if (body === 'sun')
...
export const MoonPhase = {
     ...
     * Get next full moon
     */
    nextFullMoon(from = new Date()) {
        const current = this.forDate(from);
        let daysToFull = SYNODIC_MONTH / 2 - current.age;
        if (daysToFull < 0)
            daysToFull += SYNODIC_MONTH;
...
export class Ephemeris {
     ...
     */
    getAngularSeparation(body1, body2) {
...
export const SolarSystem = {
     ...
     */
    findConjunctions(startDate, endDate, threshold = 5) {
        const events = [];
     ...
     */
    getVisiblePlanets(location, date = new Date()) {
...
export const StarCatalog = {
    ...
    byConstellation(constellation) {
        return BRIGHT_STARS.filter(s => s.constellation.toLowerCase() === constellation.toLowerCase());
    },
    /**
     ...
     */
    getByName(name) {
        return BRIGHT_STARS.find(s => s.name.toLowerCase() === name.toLowerCase());
    },
    /**
     * Find stars near a position
     */
    findNear(ra, dec, radius = 10) {
 ...
 */
export function createAstronomyStore(initialLocation, autoUpdate = false, updateIntervalMs = 60000) {
    const location = signal(initialLocation || { latitude: 0, longitude: 0 });
 ...
 */
export function usePlanetPosition(body) {
 ...
 */
export function useHorizontalPosition(body) {
    const store = getStore();
    return computed(() => {
 ...
 */
export function useIsVisible(body) {
 ...
 */
export function useRiseTransitSet(body) {
    const store = getStore();
    return computed(() => {
 ...
 */
export function setObserverLocation(location) {
    getStore().location.set(location);
}
/**
 * Enable/disable auto-update
 */
export function setAutoUpdate(enabled, intervalMs) {
    const store = getStore();
 ...
 */
export function createISSTracker() {
