...
export class AuthProviderFactory {
  private static providers = new Map<string, new (config: any) => AuthProvider>();
  ...
  static create(config: AuthProviderConfig): AuthProvider {
    ...
    if (!Provider) {
      throw new Error(
        `Unknown auth provider: ${config.name}. Available providers: ${Array.from(
          this.providers.keys()
        ).join(', ')}`
      );
    }
...
export abstract class BaseAuthProvider implements AuthProvider {
   ...
   */
  protected validatePassword(password: string, minLength: number = 8): boolean {
