 ...
 */
export async function loadComparisonData(frameworksPath) {
 ...
 */
export function generateComparisonChart(philjsResults, frameworkData, benchmarkName) {
    ...
    if (frameworks) {
        for (const [key, framework] of Object.entries(frameworks)) {
            ...
            if (fw['benchmarks'] && fw['benchmarks'][benchmarkName]) {
                labels.push(fw['name']);
                data.push(fw['benchmarks'][benchmarkName].mean);
            }
        }
    }
    return {
 ...
 */
export function generateAllComparisonCharts(philjsResults, frameworkData) {
    const charts = {};
    const benchmarks = [
        'create-1000-rows',
 ...
 */
export function generateChartJsConfig(chartData, options = {}) {
    const { title = '', yAxisLabel = 'Time (ms)', type = 'bar', } = options;
    const config = {
        type,
        data: chartData,
        options: {
 ...
 */
export function generateComparisonHTML(charts, philjsResults, frameworkData) {
 ...
 */
function generateStatsTable(benchmark, chartData) {
 ...
 */
export async function generateAndSaveCharts(philjsReport, outputDir, frameworksPath) {
    // Load framework data
    const frameworkData = await loadComparisonData(frameworksPath);
    // Generate charts
    const charts = generateAllComparisonCharts(philjsReport.suites.framework?.results || [], frameworkData);
    // Generate HTML
    const html = generateComparisonHTML(charts, philjsReport.suites.framework?.results || [], frameworkData);
    // Save HTML
    await fs.mkdir(outputDir, { recursive: true });
    await fs.writeFile(path.join(outputDir, 'comparison.html'), html, 'utf-8');
    // Save chart data as JSON
    await fs.writeFile(path.join(outputDir, 'comparison-data.json'), JSON.stringify(charts, null, 2), 'utf-8');
    console.log(`Comparison charts saved to ${outputDir}/comparison.html`);
}
/**
 * Generate markdown comparison table.
 */
export function generateComparisonMarkdown(philjsResults, frameworkData) {
    const frameworks = frameworkData['frameworks'];
    const benchmarks = [
        'create-1000-rows',
        'update-every-10th-row',
        ...
        if (frameworks) {
            for (const [key, framework] of Object.entries(frameworks)) {
                ...
                if (fw['benchmarks'] && fw['benchmarks'][benchmark]) {
                    results.push({
                        name: fw['name'],
                        value: fw['benchmarks'][benchmark].mean,
                    });
                }
            }
        }
