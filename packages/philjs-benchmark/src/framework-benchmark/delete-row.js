 ...
 */
function createDeletableTable(mockDOM) {
    const rows = signal([]);
 ...
 */
function createKeyedDeletableTable(mockDOM) {
    const rows = signal([]);
    const tbody = mockDOM.document.createElement('tbody');
    const rowElements = new Map();
    const create = (count) => {
 ...
 */
export const removeMultipleRows = {
    name: 'remove-10-rows',
    iterations: 100,
    fn: async () => {
        const mockDOM = createMockDOM();
...
export const deleteRowBenchmarks = [
    removeRow,
    removeRowKeyed,
    removeFirstRow,
    removeLastRow,
