 ...
 */
import { runBenchmarkSuite, formatResult, getEnvironmentInfo, getMemoryUsage, gc, } from '../utils.js';
 ...
 */
async function measureMemory(name, setup) {
    gc();
...
export async function runFrameworkBenchmarks(options = {}) {
    ...
    // Create benchmarks
    const createResults = await runBenchmarkSuite(createRowsBenchmarks.filter(b => !coreFrameworkBenchmarks.includes(b)), { ...options, verbose });
    results.push(...createResults);
    // Update benchmarks
    const updateResults = await runBenchmarkSuite(updateRowsBenchmarks.filter(b => !coreFrameworkBenchmarks.includes(b)), { ...options, verbose });
    results.push(...updateResults);
    // Swap benchmarks
    const swapResults = await runBenchmarkSuite(swapRowsBenchmarks.filter(b => !coreFrameworkBenchmarks.includes(b)), { ...options, verbose });
    results.push(...swapResults);
    // Select benchmarks
    const selectResults = await runBenchmarkSuite(selectRowBenchmarks.filter(b => !coreFrameworkBenchmarks.includes(b)), { ...options, verbose });
    results.push(...selectResults);
    // Delete benchmarks
    const deleteResults = await runBenchmarkSuite(deleteRowBenchmarks.filter(b => !coreFrameworkBenchmarks.includes(b)), { ...options, verbose });
