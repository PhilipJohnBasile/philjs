 ...
 */
function createSwappableTable(mockDOM) {
    const rows = signal([]);
 ...
 */
function createKeyedSwappableTable(mockDOM) {
    const rows = signal([]);
    const tbody = mockDOM.document.createElement('tbody');
    const rowElements = new Map();
    ...
    return {
        rows,
        tbody,
        create: (count) => rows.set(createRowsData(count)),
        swap1And998: () => {
            const currentRows = [...rows()];
            if (currentRows.length >= 999) {
                const temp = currentRows[1];
...
export const swapRowsBenchmarks = [
    swapRows,
    swapRowsKeyed,
    swapFirstLast,
    reverseRows,
...
//# sourceMappingURL=swap-rows.js.map