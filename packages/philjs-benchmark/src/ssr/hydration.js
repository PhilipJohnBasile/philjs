 ...
 */
function hydrate(vnode, container, mockDOM) {
    const cleanups = [];
    function hydrateNode(node, domNode) {
 ...
 */
function InteractiveRow({ id, label, onSelect, onDelete, }) {
    return jsx('tr', {
        children: [
 ...
 */
function InteractiveTable({ rows, onSelect, onDelete, }) {
    return jsx('table', {
        className: 'table table-hover table-striped test-data',
        children: jsx('tbody', {
            children: rows.map(row => InteractiveRow({
                ...row,
 ...
 */
function createPrerenderedDOM(count, mockDOM) {
    ...
    tbody.children = Array.from({ length: count }, (_, i) => tbody.childNodes?.[i] || tbody);
 ...
 */
export const timeToInteractive = {
...
export const hydrationBenchmarks = [
    hydrate100Rows,
    hydrate1000Rows,
    hydrateWithState,
    timeToInteractive,
