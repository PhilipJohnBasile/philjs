 ...
 */
async function* renderToStream(vnode: any): AsyncGenerator<string> {
 ...
 */
function AsyncComponent({ delay, content }: { delay: number; content: string }) {
 ...
 */
export const stream1000Rows: Benchmark = {
 ...
 */
export const timeToFirstByte: Benchmark = {
 ...
 */
export const streamWithSuspense: Benchmark = {
  name: 'stream-with-suspense',
  iterations: 20,
  fn: async () => {
    // Simulated Suspense component
    function Suspense({ fallback, children }: { fallback: any; children: any }) {
      return jsx('div', {
        'data-suspense': 'true',
        children: [
 ...
 * Chunk size analysis.
 */
export const chunkSizeAnalysis: Benchmark = {
  ...
  fn: async () => {
    ...
    (globalThis as any).__lastChunkAnalysis = {
 ...
 */
export const streamingThroughput: Benchmark = {
  ...
  fn: async () => {
    ...
    // Store for reporting
    (globalThis as any).__lastThroughput = {
      bytes: totalBytes,
      durationMs: duration,
      throughputMBps,
    };
 ...
 */
export const partialStreaming: Benchmark = {
  ...
  fn: async () => {
    ...
    const content = jsx('div', {
      id: 'content',
      children: rows.map(row =>
        jsx('div', { key: row.id, children: row.label })
      ),
    });
 ...
 */
export const outOfOrderStreaming: Benchmark = {
  name: 'out-of-order-streaming',
  iterations: 20,
  fn: async () => {
    const slots: Map<string, string> = new Map();
    const pendingSlots: Map<string, Promise<string>> = new Map();
    ...
    const results = await Promise.all(
      Array.from(pendingSlots.entries()).map(async ([id, promise]) => ({
        id,
...
export const streamingBenchmarks: Benchmark[] = [
  stream1000Rows,
  timeToFirstByte,
  streamWithSuspense,
  chunkSizeAnalysis,
