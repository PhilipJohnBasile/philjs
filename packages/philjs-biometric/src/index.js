...
export class BiometricManager {
    ...
    static async getCapabilities() {
        ...
        const [platformAuthenticator, conditionalUI, uvpa] = await Promise.all([
    ...
    async conditionalAuthenticate(abortController) {
        ...
        const credential = await navigator.credentials.get(credRequestOptions);
        if (!credential) {
            throw new Error('Conditional authentication failed');
        }
        const response = credential.response;
        const condAuthResult = {
            credentialId: credential.id,
            authenticatorData: response.authenticatorData,
            clientDataJSON: response.clientDataJSON,
            signature: response.signature
    ...
    deserializeCredential(serialized) {
...
export class PasskeyManager {
    biometricManager;
    storageKey;
    constructor(config, storageKey = 'philjs_passkeys') {
    ...
    async createPasskey(userId, userName, displayName) {
    ...
    async signInWithAutofill(abortController) {
 ...
 */
export function useBiometric(config) {
    const manager = new BiometricManager(config);
    const [getCapabilities, setCapabilities] = createState(null);
 ...
 */
export function usePasskeys(config, storageKey) {
