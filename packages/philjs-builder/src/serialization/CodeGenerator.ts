 ...
 */
function styleValueToString(value: StyleValue | string | number | undefined): string {
  if (value === undefined || value === null) return '';
  if (typeof value === 'string') return value;
  if (typeof value === 'number') return `${value}px`;
  if (typeof value === 'object' && 'value' in value) {
    const unit = value.unit || 'px';
    if (unit === 'none') return String(value.value);
 ...
 */
function getQuote(options: CodeGeneratorOptions): string {
 ...
 */
function getSemi(options: CodeGeneratorOptions): string {
 ...
 */
function formatString(value: string, options: CodeGeneratorOptions): string {
 ...
 */
function getComponentTag(type: string, options: CodeGeneratorOptions): string {
 ...
 */
function generateInlineStyles(styles: NodeStyles, options: CodeGeneratorOptions): string {
 ...
 */
function generateStyleObject(styles: NodeStyles, options: CodeGeneratorOptions, indentLevel: number): string {
 ...
 */
function generateTailwindClasses(styles: NodeStyles): string {
  ...
  if (styles.display === 'hidden' || styles.display === 'none') classes.push('hidden');
  ...
  if (styles.flexWrap === 'nowrap') classes.push('flex-nowrap');
  ...
  if (widthValue === 100) classes.push('w-full');
  if (widthValue === 50) classes.push('w-1/2');

  const heightValue = typeof styles.height === 'object' && styles.height?.unit === '%' ? styles.height.value : null;
  if (heightValue === 100) classes.push('h-full');
...
function generateProps(
  ...
  if (options.styleFormat === 'tailwind') {
    ...
    // Object style
    const hasStyles = Object.keys(node.styles).some(k => node.styles[k as keyof NodeStyles] !== undefined);
    if (hasStyles) {
...
export function generateCode(
  nodes: Record<NodeId, ComponentNode>,
  rootId: NodeId,
  userOptions: Partial<CodeGeneratorOptions> = {}
): GeneratedCode {
  const options: CodeGeneratorOptions = { ...defaultOptions, ...userOptions };
  ...
  const collectTypes = (nodeId: NodeId) => {
 ...
 */
export function generateJSXString(
  nodes: Record<NodeId, ComponentNode>,
  rootId: NodeId,
 ...
 */
export function generateInlineCSS(styles: NodeStyles): string {
 ...
 */
export function generateCSSClass(className: string, styles: NodeStyles): string {
  const cssProperties: string[] = [];
