...
export function createTemplateManager(options = {}) {
    const { storage = 'localStorage', storageKey = 'philjs_builder_templates', onSave, onLoad, onDelete, } = options;
    // Initialize signals
    const templates = signal([...builtInTemplates]);
    const categories = signal([...defaultCategories]);
    // Load from storage
 ...
 */
export function applyTemplate(template, parentId = null) {
    // Deep clone nodes with new IDs
    const nodeIdMap = new Map();
    const clonedNodes = {};
    // First pass: create new IDs
...
//# sourceMappingURL=TemplateSystem.js.map