 ...
 */
function cloneNodeWithNewIds(node, nodes, newParentId) {
    ...
    for (const childId of node.children) {
        ...
        if (childNode) {
            const { clonedNode: clonedChild, clonedNodes: childClonedNodes } = cloneNodeWithNewIds(childNode, nodes, newId);
 ...
 */
function removeNodeAndDescendants(nodeId, nodes) {
 ...
 */
export function createBuilderStore(initialState) {
    ...
    const nodes = signal(initialState?.nodes ?? { [rootNode.id]: rootNode });
    const rootId = signal(initialState?.rootId ?? rootNode.id);
    const selection = signal(initialState?.selection ?? defaultSelectionState);
    const drag = signal(initialState?.drag ?? defaultDragState);
    const resize = signal(initialState?.resize ?? defaultResizeState);
    const viewport = signal(initialState?.viewport ?? defaultViewportState);
    const canvas = signal(initialState?.canvas ?? defaultCanvasSettings);
    const components = signal(initialState?.components ?? {});
    const categories = signal(initialState?.categories ?? []);
    const ui = signal(initialState?.ui ?? defaultUIState);
    const preview = signal(initialState?.preview ?? defaultPreviewState);
    const clipboard = signal([]);
    // Event listeners
    ...
    const history = createHistoryManager({
        ...
        onUndo: (entry) => {
            batch(() => {
                if (entry.before.nodes)
                    nodes.set(entry.before.nodes);
                if (entry.before.rootId)
                    rootId.set(entry.before.rootId);
            });
        },
        onRedo: (entry) => {
            batch(() => {
                if (entry.after.nodes)
                    nodes.set(entry.after.nodes);
                if (entry.after.rootId)
                    rootId.set(entry.after.rootId);
            });
        },
    });
    // Computed values
    const selectedNodes = memo(() => {
        const sel = selection();
        const nodeMap = nodes();
        return sel.selectedIds.map((id) => nodeMap[id]).filter(Boolean);
    });
    ...
    const recordHistory = (description, type) => {
        const currentNodes = nodes();
        const currentRootId = rootId();
        return (afterNodes, afterRootId) => {
            history.push({
                id: generateId(),
                type: type,
                timestamp: Date.now(),
    ...
    const dispatch = (action) => {
        switch (action.type) {
            ...
            case 'PASTE_NODES': {
                ...
                batch(() => {
                    ...
                    for (const clipboardNode of clipboardNodes) {
                        const { clonedNode, clonedNodes } = cloneNodeWithNewIds(clipboardNode, updatedNodes, parentId);
            ...
            case 'SET_ZOOM': {
                ...
                const clampedZoom = Math.max(currentCanvas.zoom.min, Math.min(currentCanvas.zoom.max, action.payload.zoom));
                viewport.set({
                    ...viewport(),
                    zoom: clampedZoom,
                });
