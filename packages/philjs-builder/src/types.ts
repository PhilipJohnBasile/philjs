...
export type BuilderAction =
  ...
  | { type: 'UPDATE_UI'; payload: Partial<BuilderState['ui']> }
  | { type: 'UPDATE_PREVIEW'; payload: Partial<BuilderState['preview']> }
...
export type PreviewMessage =
  | { type: 'INIT'; payload: { nodes: Record<NodeId, ComponentNode>; rootId: NodeId } }
  | { type: 'UPDATE_NODE'; payload: { nodeId: NodeId; node: ComponentNode } }
  | { type: 'DELETE_NODE'; payload: { nodeId: NodeId } }
  | { type: 'FULL_REFRESH'; payload: { nodes: Record<NodeId, ComponentNode>; rootId: NodeId } }
  | { type: 'SET_DEVICE'; payload: { width: number; height: number } }
...
export type PreviewCallback =
  | { type: 'NODE_CLICKED'; payload: { nodeId: NodeId } }
  | { type: 'NODE_HOVERED'; payload: { nodeId: NodeId | null } }
  | { type: 'READY' }
...
export interface BuilderEvents {
  'node:added': { node: ComponentNode };
  'node:deleted': { nodeId: NodeId };
  'node:updated': { nodeId: NodeId; changes: Partial<ComponentNode> };
  'node:moved': { nodeId: NodeId; oldParentId: NodeId; newParentId: NodeId };
  'selection:changed': { selectedIds: NodeId[] };
  'drag:start': { source: DragSource };
  'drag:end': { source: DragSource; target: DropTarget | null };
  'resize:start': { nodeId: NodeId; handle: ResizeHandle };
  'resize:end': { nodeId: NodeId; bounds: BoundingBox };
  'viewport:changed': { viewport: ViewportState };
  'document:loaded': { document: DocumentMetadata };
  'document:saved': { document: DocumentMetadata };
 ...
 * Event listener type
 */
export type BuilderEventListener<K extends keyof BuilderEvents> = (
  event: BuilderEvents[K]
) => void;
