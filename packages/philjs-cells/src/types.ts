 ...
 */
export type CellFetcher<TData, TVariables = Record<string, unknown>> = (
 ...
 */
export type CellDataType<T> = T extends CellComponent<infer TData, unknown>
  ? TData
  : never;
 ...
 */
export type CellVariablesType<T> = T extends CellComponent<unknown, infer TVariables>
  ? TVariables
  : never;
 ...
 */
export const defaultIsEmpty = <T>(data: T): boolean => {
