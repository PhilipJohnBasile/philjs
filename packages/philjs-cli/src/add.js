...
async function checkPhilJSProject() {
    try {
...
async function addSSR() {
    ...
    await fs.writeFile(path.join(process.cwd(), 'src', `entry-server.${ext}`), generateSSREntryServer(isTS));
    console.log(pc.green(`  ✓ Created src/entry-server.${ext}`));
    await fs.writeFile(path.join(process.cwd(), 'src', `entry-client.${ext}`), generateSSREntryClient(isTS));
    console.log(pc.green(`  ✓ Created src/entry-client.${ext}`));
    await fs.writeFile(path.join(process.cwd(), `server.${ext}`), generateExpressServer(isTS));
...
async function addIslands() {
    ...
    await fs.writeFile(path.join(process.cwd(), 'src', 'islands', `ExampleIsland.${ext}`), generateExampleIsland(isTS));
...
async function addGraphQL() {
    ...
    // Create schema
    await fs.writeFile(path.join(process.cwd(), 'src', 'graphql', `schema.${ext}`), generateGraphQLSchema(isTS));
    console.log(pc.green(`  ✓ Created src/graphql/schema.${ext}`));
    // Create client
    await fs.writeFile(path.join(process.cwd(), 'src', 'graphql', `client.${ext}`), generateGraphQLClient(isTS));
...
async function addTailwind() {
    ...
    await fs.writeFile(path.join(process.cwd(), 'tailwind.config.js'), `/** @type {import('tailwindcss').Config} */
export default {
  content: [
    "./index.html",
    "./src/**/*.{js,ts,jsx,tsx}",
...
`);
    ...
    await fs.writeFile(path.join(process.cwd(), 'postcss.config.js'), `export default {
  plugins: {
    tailwindcss: {},
    autoprefixer: {},
  },
...
`);
    ...
    // Create CSS file
    await fs.writeFile(path.join(process.cwd(), 'src', 'index.css'), `@tailwind base;
@tailwind components;
@tailwind utilities;
`);
...
async function addTesting() {
    ...
    await fs.writeFile(path.join(process.cwd(), isTS ? 'vitest.config.ts' : 'vitest.config.js'), `import { defineConfig } from 'vitest/config';
...
`);
    ...
    // Create example test
    const ext = isTS ? 'tsx' : 'jsx';
    await fs.writeFile(path.join(process.cwd(), 'tests', `example.test.${ext}`), `import { describe, it, expect } from 'vitest';
...
async function addLinting() {
    ...
    await fs.writeFile(path.join(process.cwd(), '.eslintrc.json'), JSON.stringify({
        env: {
            browser: true,
            es2021: true,
            node: true,
    ...
    await fs.writeFile(path.join(process.cwd(), '.prettierrc.json'), JSON.stringify({
        semi: true,
        trailingComma: 'es5',
        singleQuote: true,
        printWidth: 100,
...
async function addPWA() {
    ...
    console.log(pc.green('  ✓ Updated package.json'));
    // Create manifest
    await fs.writeFile(path.join(process.cwd(), 'public', 'manifest.json'), JSON.stringify({
        name: pkg.name,
        short_name: pkg.name,
        start_url: '/',
        display: 'standalone',
...
async function addI18n() {
    ...
    await fs.writeFile(path.join(process.cwd(), 'src', 'i18n', `config.${ext}`), `export const defaultLocale = 'en';
...
async function addAnalytics() {
    ...
    await fs.writeFile(path.join(process.cwd(), 'src', 'lib', `analytics.${ext}`), `/**
...
async function addAuth() {
    ...
    const ext = isTS ? 'ts' : 'js';
    await fs.mkdir(path.join(process.cwd(), 'src', 'lib'), { recursive: true });
    await fs.writeFile(path.join(process.cwd(), 'src', 'lib', `auth.${ext}`), `import { signal } from '@philjs/core';
...
async function updateViteConfig(isTS, modifier) {
...
function generateSSREntryServer(isTS) {
...
function generateSSREntryClient(isTS) {
...
function generateExpressServer(isTS) {
...
function generateExampleIsland(isTS) {
...
function generateGraphQLSchema(isTS) {
...
function generateGraphQLClient(isTS) {
