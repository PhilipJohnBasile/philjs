...
export async function buildProduction(options) {
    ...
    if (budgets) {
        ...
        if (budgets.maxBundleSize && stats.totalSize > budgets.maxBundleSize) {
            console.error(pc.red(`❌ Bundle size exceeds budget: ${formatSize(stats.totalSize)} > ${formatSize(budgets.maxBundleSize)}`));
            process.exit(1);
        }
        console.log(pc.green(`✓ Bundle size: ${formatSize(stats.totalSize)} / ${formatSize(budgets.maxBundleSize || Infinity)}`));
    }
    // Bundle analysis
    if (options.analyze) {
...
async function getOutputStats(outDir) {
    const files = {};
    let totalSize = 0;
