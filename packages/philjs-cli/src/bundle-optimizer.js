...
// ============================================================================
export function bundleOptimizerPlugin(options = {}) {
    const { treeShaking = true, routeSplitting = true, chunkOptimization = true, deadCodeElimination = true, preload = 'critical', budgets = [], analyze = false, } = options;
    let config;
    const stats = {
        totalSize: 0,
        chunkCount: 0,
    ...
    return {
        ...
        transform(code, id) {
            ...
            if (treeShakeOpts.sideEffectFreeModules?.some(pattern => id.includes(pattern))) {
        ...
        config() {
            ...
            if (chunkOptimization) {
                ...
                buildOptions.rollupOptions.output.manualChunks = (id) => {
                ...
                if (chunk.type === 'chunk') {
                    ...
                    for (const [moduleId, module] of Object.entries(chunk.modules)) {
                        ...
                        if (existing) {
                            ...
                            const moduleWithImports = module;
                            stats.modules.push({
                                id: moduleId,
                                size: moduleSize,
                                chunks: [fileName],
 ...
 * Annotate pure functions for better tree-shaking
 */
function annotatePureFunctions(code) {
 ...
 */
export function detectUnusedExports(modules, imports) {
 ...
 */
export function calculateOptimalChunks(modules, options = {}) {
 ...
 */
export function findCommonDependencies(chunks, threshold = 0.5) {
 ...
 */
async function generateAnalysisReport(stats, options = {}) {
    const { outputFile = 'bundle-analysis.json', treemap = false, } = options;
    const report = {
        summary: {
 ...
 */
async function generateTreeMap(stats) {
 ...
 */
export function estimateGzipSize(content) {
