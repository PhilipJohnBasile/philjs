    ...
    const responses = await prompts([
        {
            ...
            validate: (value) => value.length > 0 && /^[a-z0-9-_]+$/i.test(value)
                ? true
...
async function generateTemplateFiles(projectPath, config) {
    const ext = config.typescript ? 'tsx' : 'jsx';
...
async function generateBasicTemplate(projectPath, config, ext) {
    // App.tsx
    await fs.writeFile(path.join(projectPath, 'src', `App.${ext}`), generateBasicApp(config));
    console.log(pc.green(`  ✓ Created src/App.${ext}`));
    // Counter component
    await fs.writeFile(path.join(projectPath, 'src', 'components', `Counter.${ext}`), generateCounterComponent(config));
    console.log(pc.green(`  ✓ Created src/components/Counter.${ext}`));
    // Routes
    await fs.writeFile(path.join(projectPath, 'src', 'routes', `index.${ext}`), generateHomeRoute(config));
...
async function generateSSRTemplate(projectPath, config, ext) {
    ...
    // SSR entry files
    const jsExt = config.typescript ? 'ts' : 'js';
    await fs.writeFile(path.join(projectPath, 'src', `entry-server.${jsExt}`), generateSSREntryServer(config));
    console.log(pc.green(`  ✓ Created src/entry-server.${jsExt}`));
    await fs.writeFile(path.join(projectPath, 'src', `entry-client.${jsExt}`), generateSSREntryClient(config));
    console.log(pc.green(`  ✓ Created src/entry-client.${jsExt}`));
    await fs.writeFile(path.join(projectPath, `server.${jsExt}`), generateExpressServer(config));
    console.log(pc.green(`  ✓ Created server.${jsExt}`));
}
async function generateSPATemplate(projectPath, config, ext) {
    await generateBasicTemplate(projectPath, config, ext);
    // Additional routes for SPA
    await fs.writeFile(path.join(projectPath, 'src', 'routes', `about.${ext}`), generateAboutRoute(config));
    console.log(pc.green(`  ✓ Created src/routes/about.${ext}`));
    await fs.writeFile(path.join(projectPath, 'src', 'routes', `_layout.${ext}`), generateLayoutComponent(config));
    console.log(pc.green(`  ✓ Created src/routes/_layout.${ext}`));
}
async function generateFullstackTemplate(projectPath, config, ext) {
    await generateSSRTemplate(projectPath, config, ext);
    // API routes
    const jsExt = config.typescript ? 'ts' : 'js';
    await fs.writeFile(path.join(projectPath, 'src', 'api', `hello.${jsExt}`), generateAPIRoute(config));
    console.log(pc.green(`  ✓ Created src/api/hello.${jsExt}`));
    // Database setup
    await fs.writeFile(path.join(projectPath, 'src', 'db', `schema.${jsExt}`), generateDBSchema(config));
...
async function generateLibraryTemplate(projectPath, config) {
    const jsExt = config.typescript ? 'ts' : 'js';
    await fs.writeFile(path.join(projectPath, 'src', 'lib', `index.${jsExt}`), generateLibraryIndex(config));
    console.log(pc.green(`  ✓ Created src/lib/index.${jsExt}`));
    await fs.writeFile(path.join(projectPath, 'rollup.config.ts'), generateRollupConfig(config));
    console.log(pc.green('  ✓ Created rollup.config.ts'));
    await fs.writeFile(path.join(projectPath, 'examples', 'basic.html'), generateLibraryExample(config));
    console.log(pc.green('  ✓ Created examples/basic.html'));
}
async function generateConfigFiles(projectPath, config) {
    // TypeScript config
    if (config.typescript) {
        await fs.writeFile(path.join(projectPath, 'tsconfig.json'), JSON.stringify(generateTSConfig(config), null, 2));
        console.log(pc.green('  ✓ Created tsconfig.json'));
    }
    // Vite config
    await fs.writeFile(path.join(projectPath, config.typescript ? 'vite.config.ts' : 'vite.config.js'), generateViteConfig(config));
    console.log(pc.green(`  ✓ Created vite.config.${config.typescript ? 'ts' : 'js'}`));
    // Tailwind config
    if (config.cssFramework === 'tailwind') {
        await fs.writeFile(path.join(projectPath, 'tailwind.config.js'), generateTailwindConfig());
        ...
        await fs.writeFile(path.join(projectPath, 'src', 'index.css'), generateTailwindCSS());
        console.log(pc.green('  ✓ Created src/index.css'));
    }
    // ESLint config
    if (config.linting) {
        await fs.writeFile(path.join(projectPath, '.eslintrc.json'), JSON.stringify(generateESLintConfig(config), null, 2));
        console.log(pc.green('  ✓ Created .eslintrc.json'));
        await fs.writeFile(path.join(projectPath, '.prettierrc.json'), JSON.stringify(generatePrettierConfig(), null, 2));
        console.log(pc.green('  ✓ Created .prettierrc.json'));
    }
    // Vitest config
    if (config.testing) {
        await fs.writeFile(path.join(projectPath, config.typescript ? 'vitest.config.ts' : 'vitest.config.js'), generateVitestConfig(config));
        console.log(pc.green(`  ✓ Created vitest.config.${config.typescript ? 'ts' : 'js'}`));
        // Example test
        const ext = config.typescript ? 'tsx' : 'jsx';
        await fs.writeFile(path.join(projectPath, 'tests', `App.test.${ext}`), generateExampleTest(config));
...
function generateBasicApp(config) {
...
function generateCounterComponent(config) {
...
function generateHomeRoute(config) {
...
function generateAboutRoute(config) {
...
function generateLayoutComponent(config) {
...
function generateSSREntryServer(config) {
...
function generateSSREntryClient(config) {
...
function generateExpressServer(config) {
...
function generateDBSchema(config) {
...
function generateRollupConfig(config) {
...
function generateLibraryExample(config) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
function generateVitestConfig(config) {
...
function generateExampleTest(config) {
