...
async function discoverRoutes(dir, base = "") {
    const routes = [];
...
declare module "philjs-router" {
  ...
  export type RouteParams<T extends RoutePath> = ${Object.keys(routeParams).length > 0
        ? Object.entries(routeParams)
            .map(([path, params]) => `T extends "${path}" ? { ${params.map((p) => `${p}: string`).join(", ")} } :`)
            .join("\n    ") + "\n    never"
        : "never"};
