...
export async function generateComponent(options) {
    const { name, directory = 'src/components', typescript = true, withTest = true, withStyles = false, } = options;
...
export async function generateRoute(options) {
    ...
    await fs.writeFile(path.join(routeDir, `index.${ext}`), routeContent);
    ...
    await fs.writeFile(path.join(routeDir, `loader.${typescript ? 'ts' : 'js'}`), loaderContent);
    ...
    if (withTest) {
        ...
        await fs.writeFile(path.join(routeDir, `index.test.${ext}`), testContent);
...
export async function generatePage(options) {
    const { name, directory = 'src/pages', typescript = true, withTest = true, } = options;
    ...
    await fs.writeFile(path.join(pageDir, `${pageName}.${ext}`), pageContent);
    console.log(pc.green(`  ✓ Created ${pageName}.${ext}`));
    // Generate test file
    if (withTest) {
        const testContent = generatePageTestTemplate(componentName, pageName, typescript);
        await fs.writeFile(path.join(pageDir, `${pageName}.test.${ext}`), testContent);
...
export async function generateHook(options) {
    const { name, directory = 'src/hooks', typescript = true, withTest = true, } = options;
    ...
    await fs.writeFile(path.join(hookDir, `${hookName}.${ext}`), hookContent);
    console.log(pc.green(`  ✓ Created ${hookName}.${ext}`));
    // Generate test file
    if (withTest) {
        const testContent = generateHookTestTemplate(hookName, typescript);
        await fs.writeFile(path.join(hookDir, `${hookName}.test.${ext}`), testContent);
 ...
 */
export async function generateCell(options) {
    const { name, directory = 'src/cells', typescript = true, withTest = true, graphql = true, } = options;
    ...
    await fs.writeFile(path.join(cellDir, `${cellName}.${ext}`), cellContent);
    console.log(pc.green(`  ✓ Created ${cellName}.${ext}`));
    // Generate test file
    if (withTest) {
        const testContent = generateCellTestTemplate(cellName, entityName, typescript);
        await fs.writeFile(path.join(cellDir, `${cellName}.test.${ext}`), testContent);
 ...
 * Store Generator
 */
export async function generateStore(options) {
    const { name, directory = 'src/stores', typescript = true, withTest = true, } = options;
    ...
    await fs.writeFile(path.join(storeDir, `${storeName}.${ext}`), storeContent);
    ...
    if (withTest) {
        ...
        await fs.writeFile(path.join(storeDir, `${storeName}.test.${ext}`), testContent);
        console.log(pc.green(`  ✓ Created ${storeName}.test.${ext}`));
    }
}
// Template generators
function generateComponentTemplate(name, ts, withStyles) {
...
function generateTestTemplate(name, ts) {
...
function generateRouteTemplate(name, routePath, ts) {
...
function generateLoaderTemplate(name, ts) {
...
function generateRouteTestTemplate(name, routePath, ts) {
...
function generatePageTemplate(name, pageName, ts) {
    const propsType = ts ? `\nexport interface ${name}Props {\n  // Page props\n}\n` : '';
    return `/**
 * ${name} - Page component
 */
...
function generatePageTestTemplate(name, pageName, ts) {
    return `import { describe, it, expect } from 'vitest';
import { render, screen } from 'philjs-testing';
...
function generateHookTemplate(name, ts) {
...
function generateHookTestTemplate(name, ts) {
    return `import { describe, it, expect } from 'vitest';
...
function generateStoreTemplate(storeName, pascalName, ts) {
...
function generateStoreTestTemplate(storeName, pascalName, ts) {
...
function generateCellTemplate(cellName, entityName, ts, graphql) {
...
function generateCellTestTemplate(cellName, entityName, ts) {
    const entityLower = entityName.toLowerCase();
    const entityPlural = entityLower + 's';
