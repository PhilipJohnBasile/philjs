...
import { createContext, toPascalCase, toKebabCase, toCamelCase, extractRouteParams, isDynamicRoute, } from './template-engine.js';
/**
 * Generate an API route
 */
export async function generateApi(options) {
    const { name, directory = 'src/api', typescript = true, withTest = true, methods = ['GET', 'POST', 'PUT', 'DELETE'], } = options;
...
function generateRouteTemplate(context) {
    const { typescript, isDynamic, routeParams, methods, resourceName, resourcePascal, } = context;
    ...
    if (hasGet) {
        handlers += `
/**
 * GET /${context.kebabName}${isDynamic ? `/:${routeParams[0]}` : ''}
...
export async function GET(request${requestType}${isDynamic ? `, params${paramsType}` : ''}) {
  try {
    ${isDynamic ? `const { ${routeParams[0]} } = params;` : ''}
    // Implement your ${isDynamic ? 'fetch single' : 'fetch all'} logic here
    // Example: const data = await db.query(...)
    ${isDynamic
            ? `const data${typescript ? `: ${resourcePascal}Data` : ''} = { id: ${routeParams[0]}, /* ... */ };`
...
`;
    ...
    if ((hasPut || hasPatch) && isDynamic) {
        handlers += `
/**
 * ${hasPut ? 'PUT' : 'PATCH'} /${context.kebabName}/:${routeParams[0]}
...
`;
    ...
    if (hasDelete && isDynamic) {
        handlers += `
/**
 * DELETE /${context.kebabName}/:${routeParams[0]}
...
function generateTypesTemplate(context) {
...
function generateTestTemplate(context) {
  ...
  describe('GET /${kebabName}/:${routeParams[0]}', () => {
    ...
    it('returns 404 for non-existent ${resourcePascal}', async () => {
      ...
      const response = await GET(request, { ${routeParams[0]}: 'invalid' });
...
`;
    ...
    if ((methodsArr.includes('PUT') || methodsArr.includes('PATCH')) && isDynamic) {
        ...
        tests += `
  describe('${method} /${kebabName}/:${routeParams[0]}', () => {
    it('updates a ${resourcePascal}', async () => {
      ...
      const response = await ${method}(request, { ${routeParams[0]}: '123' });
...
`;
    ...
    if (methodsArr.includes('DELETE') && isDynamic) {
        tests += `
  describe('DELETE /${kebabName}/:${routeParams[0]}', () => {
    it('deletes a ${resourcePascal}', async () => {
      const request = new Request('http://localhost/${kebabName}/123', {
        method: 'DELETE',
      });
      const response = await DELETE(request, { ${routeParams[0]}: '123' });
...
`;
    ...
    const imports = methodsArr.filter(m => {
        if (m === 'POST' && isDynamic)
            return false;
        if ((m === 'PUT' || m === 'PATCH' || m === 'DELETE') && !isDynamic)
            return false;
        return true;
    }).join(', ');
