...
export async function generateModel(options) {
    const { name, fields = [], provider = 'prisma', schemaPath, typescript = true, } = options;
 ...
 */
async function generatePrismaModel(modelName, fields, schemaPath) {
 ...
 */
function generatePrismaModelDefinition(name, fields) {
 ...
 */
async function generateDrizzleModel(modelName, fields, typescript) {
 ...
 */
function generateDrizzleModelContent(name, fields, typescript) {
    const tableName = toSnakeCase(name) + 's';
 ...
 */
function formatDrizzleField(field) {
    const imports = [];
    const columnName = toSnakeCase(field.name);
    let typeFunc;
 ...
 */
async function updateDrizzleIndex(modelsDir, fileName, modelName, typescript) {
