 ...
 */
export async function generatePage(options: PageOptions): Promise<string[]> {
  const {
    name,
    directory = 'src/pages',
    typescript = true,
...
function generatePageTemplate(context: TemplateContext): string {
...
function generateLoaderTemplate(context: TemplateContext): string {
  const { pascalName, typescript, isDynamic, routeParams } = context;
  const componentName = `${pascalName}Page`;

  const paramsType = isDynamic && typescript
    ? `{ params }: { params: { ${(routeParams as string[]).map(p => `${p}: string`).join('; ')} } }`
    : '';
...
export async function loader(${paramsType})${returnType} {
  ...
  ${isDynamic ? `// Access params: ${(routeParams as string[]).map(p => `params.${p}`).join(', ')}` : ''}
