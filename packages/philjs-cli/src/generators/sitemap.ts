 ...
 * Generate sitemap
 */
export async function generateSitemap(options: SitemapGeneratorOptions): Promise<void> {
 ...
 */
function generateSitemapGeneratorFile(options: SitemapGeneratorOptions): string {
  const output = options.output || 'public/sitemap.xml';
 ...
 */
function generateRobotsTxtGenerator(options: SitemapGeneratorOptions): string {
  const site = options.site || 'https://example.com';
