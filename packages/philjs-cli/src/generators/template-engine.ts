...
export function renderTemplate(template: string, context: TemplateContext): string {
  ...
  result = result.replace(
    ...
    (_, arrayName, itemTemplate) => {
      const items = context[arrayName] as unknown[];
  ...
  result = result.replace(/\{\{(\w+)\.(\w+)\}\}/g, (_, obj, prop) => {
    ...
    if (value && typeof value === 'object' && prop in (value as Record<string, unknown>)) {
      return String((value as Record<string, unknown>)[prop]);
    }
    return '';
  });
