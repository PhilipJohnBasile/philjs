...
import { createCompiler, transform, analyzeCode, validateConfig, getDefaultConfig, version } from './index';
describe('PhilJS Compiler', () => {
    ...
    describe('validateConfig', () => {
        ...
        it('should detect invalid include patterns', () => {
            const errors = validateConfig({
                include: [123] // Invalid: not a string
            });
        ...
        it('should detect invalid exclude patterns', () => {
            const errors = validateConfig({
                exclude: [null] // Invalid: not a string
            });
            expect(errors.length).toBeGreaterThan(0);
        ...
        it('should detect invalid plugins', () => {
            const errors = validateConfig({
                plugins: [
                    { name: 'test', transform: 'not-a-function' }
                ]
            });
            expect(errors.length).toBeGreaterThan(0);
