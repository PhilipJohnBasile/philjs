...
export class DeadCodeEliminator {
    ...
    eliminate(ast, analysis) {
        ...
        const bindingIndex = new Map(analysis.bindings.map((b) => [b.name, b]));
        const unusedBindings = new Set(analysis.bindings
            .filter((b) => !b.isUsed)
            .map((b) => b.name));
        let initialSize = 0;
        let finalSize = 0;
    ...
    markAsPure(path) {
        ...
        const hasPureComment = path.node.leadingComments.some((comment) => comment.value === '#__PURE__');
        if (!hasPureComment) {
     ...
     */
    static /*#__PURE__*/ formatReport(report) {
        const lines = [
 ...
 */
export const createDeadCodeEliminator = /*#__PURE__*/ function createDeadCodeEliminator(config) {
