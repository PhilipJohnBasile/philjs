...
describe('Vite Plugin - PhilJS Compiler', () => {
  ...
  describe('configResolved hook', () => {
    it('should initialize in development mode', () => {
      const plugin = philJSCompiler({ verbose: false }) as Plugin;
      const config = {
        mode: 'development',
    ...
    it('should log configuration in verbose mode', () => {
      ...
      const plugin = philJSCompiler({ verbose: true }) as Plugin;
      const config = {
        mode: 'development',
        build: { sourcemap: true },
      } as any;
  ...
  describe('transform hook', () => {
    ...
    it('should log transformation details in verbose mode', async () => {
      const verbosePlugin = philJSCompiler({ verbose: true }) as Plugin;
  ...
  describe('buildEnd hook', () => {
    it('should log summary in verbose mode after processing files', async () => {
      const consoleSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
      const plugin = philJSCompiler({ verbose: true, cache: true }) as Plugin;
    ...
    it('should not log when no files processed', () => {
      ...
      const summaryLogged = consoleSpy.mock.calls.some(call =>
        call.some(arg => typeof arg === 'string' && arg.includes('Build Summary'))
      );
  ...
  describe('handleHotUpdate hook', () => {
    let plugin: Plugin;
    let mockModule: ModuleNode;
    let mockContext: HmrContext;

    beforeEach(() => {
      plugin = philJSCompiler({ verbose: false, cache: true }) as Plugin;
  ...
  describe('closeBundle hook', () => {
    ...
    it('should clear cache in production mode', () => {
      const plugin = philJSCompiler({ verbose: false, cache: true }) as Plugin;
