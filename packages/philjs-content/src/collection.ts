...
export function defineCollection<
  TType extends CollectionType,
  TSchema extends ZodType,
>(config: CollectionConfig<TType, TSchema>): CollectionDefinition<TType, TSchema> {
  return {
    type: config.type,
 ...
 * ```
 */
export function reference<TCollection extends string>(
  collection: TCollection
): ReferenceSchema<TCollection> {
  return z.object({
    collection: z.literal(collection),
    id: z.string(),
  }) as ReferenceSchema<TCollection>;
}

/**
 * Global content store for managing loaded collections
 */
const globalStore: ContentStore = {
  collections: new Map(),
  configs: new Map(),
  initialized: false,
};
 ...
 */
export function initializeStore(collections: CollectionsConfig): void {
 ...
 */
export function getCollectionConfig(name: string): CollectionDefinition | undefined {
 ...
 */
export function transformDates(data: Record<string, unknown>): Record<string, unknown> {
 ...
 */
export function idFromPath(filePath: string, basePath: string): string {
 ...
 * ```
 */
export function defineCollections<T extends CollectionsConfig>(
  collections: T
): T {
  return collections;
}
