...
import { z } from 'zod';
import { getContentStore, getCollectionConfig, validateEntryData, transformDates, slugFromPath, idFromPath, } from './collection.js';
 ...
 */
export function setContentDir(dir) {
    contentDir = dir;
}
/**
 ...
 */
export function setCollectionsConfig(config) {
    collectionsConfig = config;
}
/**
 * Get file patterns for a collection type
 */
function getFilePatterns(type) {
    if (type === 'content') {
        return ['**/*.md', '**/*.mdx'];
    }
 ...
 */
async function parseContentFile(filePath, collectionName, basePath) {
 ...
 */
async function parseDataFile(filePath, collectionName, basePath) {
    const content = await readFile(filePath, 'utf-8');
    const stats = await stat(filePath);
 ...
 */
async function loadCollectionEntries(collectionName, config) {
        ...
        for (const filePath of files) {
            try {
                ...
                if (result.success) {
                    ...
                    console.warn(`[philjs-content] Validation failed for ${filePath}:`, result.error.format());
                }
            }
            catch (error) {
 ...
 * ```
 */
export async function getCollection(collection, filterOrOptions) {
    // Get collection config
    const config = collectionsConfig?.[collection] ?? getCollectionConfig(collection);
    if (!config) {
        throw new Error(`[philjs-content] Collection "${collection}" not found. ` +
 ...
 * ```
 */
export async function getEntries(references) {
    const entries = [];
...
export function hasCollection(collection) {
    return (collectionsConfig?.[collection] !== undefined ||
...
export async function getEntryBySlug(collection, slug) {
    ...
    if (config.type !== 'content') {
        throw new Error(`[philjs-content] getEntryBySlug only works with content collections. ` +
            `"${collection}" is a data collection.`);
    }
    const entries = await loadCollectionEntries(collection, config);
    return entries.find((entry) => entry.type === 'content' && entry.slug === slug);
}
/**
 ...
 * ```
 */
export async function getCollectionTags(collection, tagField = 'tags') {
 ...
 * ```
 */
export async function getEntriesByTag(collection, tag, tagField = 'tags') {
 ...
 * ```
 */
export async function getAdjacentEntries(collection, currentId, options) {
 ...
 * ```
 */
export async function groupBy(collection, keyFn, filter) {
    const entries = await getCollection(collection, filter);
    // ES2024: Use native Object.groupBy
    return Object.groupBy(entries, keyFn);
}
/**
 ...
 * ```
 */
export async function countEntries(collection, filter) {
    const entries = await getCollection(collection, filter);
