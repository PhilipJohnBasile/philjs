 ...
 */
function extractHeadings(content: string): ContentHeading[] {
 ...
 */
function buildTableOfContents(headings: ContentHeading[]): TOCEntry[] {
  const toc: TOCEntry[] = [];
  const stack: TOCEntry[][] = [toc];
...
export async function renderContent(
  ...
  const Content = (props: { components?: MDXComponents }) => {
 ...
 * ```
 */
export function createContentRenderer(defaultOptions: MDXCompileOptions = {}) {
  return async function render(
    content: string,
    frontmatter: Record<string, unknown> = {},
    options: MDXCompileOptions = {}
  ): Promise<RenderResult> {
    const mergedOptions: MDXCompileOptions = {
      ...defaultOptions,
      ...options,
      components: {
 ...
 * ```
 */
export async function processForSearch(content: string): Promise<string> {
  // Remove code blocks
