...
import type { z, ZodType, ZodObject, ZodRawShape } from 'zod';
...
export interface CollectionConfig<
  TType extends CollectionType = CollectionType,
 ...
 */
export type InferCollectionData<T extends CollectionDefinition> =
  T['schema'] extends ZodType ? z.infer<T['schema']> : never;
...
export type MDXComponents = Record<
  string,
  (props: MDXComponentProps) => unknown
>;
 ...
 */
export type CollectionFilter<TEntry extends CollectionEntry> = (
  entry: TEntry
) => boolean;
 ...
 */
export type CollectionSort<TEntry extends CollectionEntry> = (
  a: TEntry,
  b: TEntry
) => number;
...
export type CollectionEntryType<
  TConfig extends CollectionsConfig,
  TName extends CollectionNames<TConfig>,
> = TConfig[TName]['type'] extends 'content'
  ? ContentEntry<InferCollectionData<TConfig[TName]>>
  : DataEntry<InferCollectionData<TConfig[TName]>>;
...
export type ContentWatchEvent =
  | { type: 'add'; path: string; collection: string }
  | { type: 'change'; path: string; collection: string }
  | { type: 'unlink'; path: string; collection: string };
