 ...
 */
export function calculateReadingTime(text, options = {}) {
 ...
 */
export function generateExcerpt(text, options = {}) {
 ...
 */
export function extractTableOfContents(headings, options = {}) {
    const minDepth = options.minDepth || 1;
    const maxDepth = options.maxDepth || 6;
    const filteredHeadings = headings.filter(h => h.depth >= minDepth && h.depth <= maxDepth);
    return buildTOCTree(filteredHeadings, minDepth);
}
/**
 * Build hierarchical TOC tree from flat heading list
 */
function buildTOCTree(headings, baseDepth) {
    const root = [];
    const stack = [];
 ...
 */
export function findRelatedPosts(currentPost, allPosts, options = {}) {
    ...
    const currentTags = currentData[tagField] || [];
    const currentTitle = currentData[titleField] || '';
    // Calculate similarity scores
    const scored = allPosts
        .filter(post => post.id !== currentPost.id)
        .map(post => {
        const postData = post.data;
        const postTags = postData[tagField] || [];
        const postTitle = postData[titleField] || '';
        let score = 0;
        ...
        const currentWords = new Set(currentTitle.toLowerCase().split(/\s+/).filter(w => w.length > 3));
        const postWords = new Set(postTitle.toLowerCase().split(/\s+/).filter(w => w.length > 3));
 ...
 */
export function generateTagCloud(entries, options = {}) {
    const tagField = options.tagField || 'tags';
 ...
 */
export function renderTagCloud(cloud, options = {}) {
 ...
 */
export function groupByField(entries, field) {
 ...
 */
export function groupByDate(entries, options = {}) {
    ...
    // Sort groups
    const sorted = new Map(Array.from(groups.entries()).sort((a, b) => {
        return sort === 'desc'
 ...
 */
export function getFirstWords(text, count) {
    return text
        .split(/\s+/)
        .slice(0, count)
        .join(' ');
