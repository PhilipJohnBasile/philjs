 ...
 * ```
 */
export function contentPlugin(userOptions = {}) {
    const options = {
        ...defaultOptions,
        ...userOptions,
        imageOptions: {
    ...
    async function loadContentConfig() {
        ...
        if (!existsSync(configPath)) {
            console.warn(`[philjs-content] No content config found at ${configPath}`);
            return null;
        }
        try {
    ...
    async function generateTypes() {
        ...
        for (const [name, config] of Object.entries(collectionsConfig)) {
            ...
            try {
                ...
                const ids = files.map((f) => f.replace(/\.(md|mdx|json|yaml|yml)$/i, ''));
                lines.push(`    ${name}: {`);
     ...
     */
    async function processContentFile(filePath, collectionName, collectionConfig) {
        try {
            ...
            const id = relative(collectionDir, filePath).replace(/\.(md|mdx|json|yaml|yml)$/i, '');
            return {
                path: filePath,
...
`;
    ...
    return {
        ...
        async configureServer(devServer) {
            ...
            console.log(`[philjs-content] Processed ${result.entries.length} entries in ${result.duration}ms`);
            // Generate types
            await writeTypes();
            // Set up file watching
            setupWatcher();
        ...
        async buildStart() {
            ...
            // Build all content
            const result = await buildContent();
            console.log(`[philjs-content] Built ${result.entries.length} entries in ${result.duration}ms`);
            // Generate types
            await writeTypes();
        },
        async buildEnd() {
