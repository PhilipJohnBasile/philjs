...
describe("Data Layer - Query Invalidation", () => {
  ...
  it("should invalidate by string key", async () => {
  ...
  it("should invalidate by array key", async () => {
  ...
  it("should invalidate by predicate function", async () => {
    const userFetcher = vi.fn().mockResolvedValue("user");
    const postFetcher = vi.fn().mockResolvedValue("post");

    createQuery({ key: "user-1", fetcher: userFetcher, staleTime: 5000 });
    createQuery({ key: "post-1", fetcher: postFetcher, staleTime: 5000 });
...
describe("Data Layer - Prefetching", () => {
  ...
  it("should prefetch and cache data", async () => {
    ...
    expect(queryCache.get("prefetch-test")).toBeDefined();
  });
