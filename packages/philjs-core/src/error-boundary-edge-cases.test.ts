...
describe('Error Boundary - Dynamic Children', () => {
  ...
  it('should recover when children are replaced', () => {
    const children = signal<any>({ type: 'div', props: { children: 'ok' } });
  ...
  it('should suggest fixes for property access on null', () => {
    ...
    const boundary = ErrorBoundary({
      ...
      onError: (errorInfo) => {
        ...
        const hasOptionalChaining = errorInfo.suggestions.some(s =>
          s.description.includes('optional chaining')
        );
...
describe('Error Retry Mechanism', () => {
  it('should increment retry count on each retry', () => {
    ...
    const fallback = (error: ErrorInfo, retry: () => void) => {
      retryFn = retry;
      errorCount++;
...
describe('Error Info Structure', () => {
  it('should include all required fields in ErrorInfo', () => {
    const onError = vi.fn((errorInfo: ErrorInfo) => {
...
describe('Suggestion Generation - Network Errors', () => {
  it('should suggest error handling for network errors', () => {
    ...
    const boundary = ErrorBoundary({
      children: { type: 'div', props: {} },
      onError: (errorInfo) => {
        const hasErrorHandling = errorInfo.suggestions.some(s =>
          s.description.toLowerCase().includes('error handling')
        );
  ...
  it('should suggest checking network connection', () => {
    ...
    const boundary = ErrorBoundary({
      children: { type: 'div', props: {} },
      onError: (errorInfo) => {
        const hasNetworkCheck = errorInfo.suggestions.some(s =>
...
describe('Suggestion Generation - Render Errors', () => {
    ...
    const boundary = ErrorBoundary({
      ...
      onError: (errorInfo) => {
        if (errorInfo.category === 'render') {
          const hasPropsCheck = errorInfo.suggestions.some(s =>
            s.description.toLowerCase().includes('props')
          );
  ...
  it('should suggest JSX syntax check for render errors', () => {
    ...
    const boundary = ErrorBoundary({
      ...
      onError: (errorInfo) => {
        if (errorInfo.category === 'render') {
          const hasJSXCheck = errorInfo.suggestions.some(s =>
            s.description.toLowerCase().includes('jsx')
          );
