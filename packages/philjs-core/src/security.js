...
export function sanitizeHtml(html, options = {}) {
    ...
    const { allowedTags = DEFAULT_ALLOWED_TAGS, allowedAttributes = DEFAULT_ALLOWED_ATTRIBUTES, allowedSchemes = DEFAULT_ALLOWED_SCHEMES, } = options;
    ...
    sanitized = sanitized.replace(/<(\/?)?([a-z][a-z0-9]*)((?:\s+[^>]*)?)\s*\/?>/gi, (match, closing, tagName, attrs) => {
 ...
 * ```
 */
export function safeJsonParse(json, reviver) {
 ...
 */
function fallbackSecureToken(length) {
 ...
 * ```
 */
export async function createCspNonce() {
