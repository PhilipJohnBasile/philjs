...
export function serialize(
  ...
  if (typeof value === 'object') {
    const obj: Record<string, SerializedState> = {};
...
export function deserialize(
  ...
  switch (serialized.type) {
    ...
    case 'error': {
      if (!preserveTypes) return serialized.value;
    ...
    case 'array':
      return serialized.value.map((item: SerializedState) =>
        deserialize(item, options)
      );
 ...
 */
export function toJSON(value: any, options: SerializationOptions = {}): string {
 ...
 */
export function fromJSON(json: string, options: SerializationOptions = {}): any {
 ...
 */
export function serializeForHydration(
  state: Record<string, any>,
  options: SerializationOptions = {}
): string {
  const hydrationData: Record<string, SerializedState> = {};
...
export function persistentSignal<T>(
  ...
  const sig = signal<T>(restored !== null ? restored : initialValue);
...
export function urlSignal<T>(
  ...
  let restoredValue: T = initialValue;
  if (urlValue) {
    try {
      restoredValue = JSON.parse(urlValue);
    } catch {
      restoredValue = urlValue as any;
    }
  }
 ...
 */
function getOrCreateSignalId(sig: Signal<any>): string {
  for (const [id, registeredSig] of signalRegistry) {
    if (registeredSig === sig) return id;
  }
 ...
 */
export function getSerializationStats(value: any): {
  size: number;
  signalCount: number;
  objectCount: number;
  arrayCount: number;
  ...
  function analyze(val: any, depth: number): void {
 ...
 */
export function deepClone<T>(value: T, options: SerializationOptions = {}): T {
