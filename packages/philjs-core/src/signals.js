...
let currentNestedDisposers = null;
// ============================================================================
 ...
 */
const hmrStateRegistry = isDev ? new Map() : null;
/**
 ...
 * ```
 */
export function linkedSignal(computation, options = {}) {
 ...
 * ```
 */
export function snapshotHMRState(options = {}) {
 ...
 * ```
 */
export function restoreHMRState(options = {}) {
    if (!isDev || !activeSignals || !hmrStateRegistry) {
        return;
    }
    const startTime = performance.now();
    ...
    if (options.verbose) {
        const duration = performance.now() - startTime;
        console.log(`[PhilJS HMR] Restored ${restoredCount} signals` +
 ...
 * ```
 */
export function rollbackHMRState(snapshot, options = {}) {
    if (!isDev || !activeSignals || !hmrStateRegistry) {
        return;
    }
    const startTime = performance.now();
 ...
 * ```
 */
export function cleanupHMREffects(options = {}) {
    if (!isDev || !activeEffects) {
        return;
    }
    const startTime = performance.now();
 ...
 * ```
 */
export function isHMRInProgress() {
