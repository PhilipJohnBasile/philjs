...
export function exampleGlobalCustomTypes() {
  ...
  const parsed = sj.parse<typeof data>(json);
...
export async function exampleCompression() {
  ...
  // Decompress
  const decompressed = await deserializeWithDecompression<typeof data>(compressed, {
    compression: NativeCompression,
  });
...
export function exampleRPCIntegration() {
  ...
  const clientData = deserialize<UserData>(received);
...
export function exampleSSRLoader() {
  ...
  const hydrated = deserialize<LoaderData>(serialized);
