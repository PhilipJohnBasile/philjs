...
export class UsageAnalytics {
  private usage = new Map<string, ComponentUsage>();
  private renderTimes = new Map<string, number[]>();
  private propPatterns = new Map<string, Map<string, any[]>>();
   ...
   */
  private trackPropUsage(component: string, props: Record<string, any>): void {
    const usage = this.getOrCreateUsage(component);
   ...
   */
  private getOrCreateUsage(component: string): ComponentUsage {
  ...
  detectDeadCode(options: {
    ...
    for (const [component, usage] of this.usage) {
      let reason: DeadCodeReport["reason"] | null = null;
      let confidence = 0;
      let suggestion = "";
   ...
   */
  private hasCircularDependency(component: string, visited = new Set<string>()): boolean {
    if (visited.has(component)) return true;
      ...
      for (const [prop, stats] of usage.propsUsage) {
        ...
        if (mostCommon && mostCommon.percentage > 80) {
          suggestions.push({
            component,
            type: "default-prop",
            description: `${mostCommon.percentage.toFixed(1)}% of uses pass "${
              mostCommon.value
            }" for prop "${prop}". Consider making it the default.`,
            impact: "low",
            autoFixAvailable: true,
            codeChanges: [{
              file: component,
              before: `function ${component}({ ${prop}, ...props })`,
              after: `function ${component}({ ${prop} = ${JSON.stringify(
                mostCommon.value
              )}, ...props })`,
            }],
          });
   ...
   */
  getDependencyGraph(): {
    nodes: Array<{ id: string; size: number; active: boolean }>;
  ...
  exportUsageData(): Record<string, ComponentUsage> {
    const data: Record<string, ComponentUsage> = {};
