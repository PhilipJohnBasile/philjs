 ...
 */
export function virtualModulesPlugin(config = {}) {
    const { routesDir = './src/routes', contentDir = './src/content', pluginsDir = './src/plugins', appConfig = {}, basePath = '', } = config;
    let root = '';
    // Virtual module IDs
    ...
    return {
        ...
        async handleHotUpdate({ file, server }) {
            ...
            if (file.startsWith(routesPath)) {
                const module = server.moduleGraph.getModuleById(resolvedVirtualModuleIds.get(VIRTUAL_ROUTES));
                if (module) {
                    return [module];
                }
            }
            if (file.startsWith(contentPath)) {
                const module = server.moduleGraph.getModuleById(resolvedVirtualModuleIds.get(VIRTUAL_CONTENT));
                if (module) {
                    return [module];
                }
            }
            if (file.startsWith(pluginsPath)) {
                const module = server.moduleGraph.getModuleById(resolvedVirtualModuleIds.get(VIRTUAL_PLUGINS));
                if (module) {
                    return [module];
                }
            }
 ...
 */
function generateRoutesModule(routes, routesPath, root) {
 ...
 */
function generateContentModule(files, contentPath, root) {
    const imports = files
        .map((file, index) => {
 ...
 */
function generateConfigModule(config, basePath) {
 ...
 */
function generatePluginsModule(files, pluginsPath, root) {
    const imports = files
        .map((file, index) => {
 ...
 */
export function generateVirtualModuleTypes() {
 ...
 */
export async function writeVirtualModuleTypes(outPath) {
