...
export class PQC {
    ...
    static n = 4; // Dimension of secret vector
    static q = 32; // Modulus
    static start_error = 1; // Noise bound
    static generateKeypair() {
        ...
        const A = Array.from({ length: this.n }, () => Array.from({ length: this.n }, () => Math.floor(Math.random() * this.q)));
    ...
    static decrypt(ciphertext, sk) {
        // Decrypt: v - s^T * u
        let dot_su = 0;
        for (let i = 0; i < this.n; i++)
            dot_su += sk[i] * ciphertext.u[i];
