 ...
 */
export function calculateSpring(from, to, config = springPresets['default'], precision = 0.01) {
 ...
 */
export function springAnimation(property, from, to, config = springPresets['default'], unit = '') {
 ...
 */
export function springEasing(config = springPresets['default']) {
 ...
 */
export function slide(direction, distance = '100%') {
    const transforms = {
 ...
 */
export function rotate(degrees = 360, options = {}) {
 ...
 */
export function bounce(intensity = 'medium') {
    const configs = {
        light: [0, -10, 0, -5, 0],
 ...
 */
export function pulse(scale = 1.05) {
    return `
@keyframes pulse {
 ...
 */
export function calculateStagger(count, config = {}) {
 ...
 */
export function staggerAnimation(selector, animation, count, config = {}) {
    const delays = calculateStagger(count, config);
    return delays.map((delay, i) => `${selector}:nth-child(${i + 1}) { animation-delay: ${delay.toFixed(2)}s; }`).join('\n');
}
/**
 ...
 */
export function parallel(animations, options = {}) {
    const { duration = 0.3, delay = 0, easing = 'ease' } = options;
    return animations.map(anim => `${anim} ${duration}s ${easing} ${delay}s`).join(', ');
}
/**
 ...
 * Play FLIP animation
 */
export function playFLIP(element, first, last, options = {}) {
 ...
 */
export function batchFLIP(elements, getNewState, options = {}) {
 ...
 */
export function motionSafe(animation, fallback) {
    return `
...
// Utility Functions
// =============================================================================
function generateKeyframes(name, from, to) {
...
export function generateAnimationUtilities() {
    ...
    Object.entries(motionPresets).forEach(([name, preset]) => {
        const presetWithIterations = preset;
