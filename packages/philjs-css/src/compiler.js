 ...
 */
function stringifyCSS(nodes, minify = false) {
    const results = [];
    const space = minify ? '' : ' ';
 ...
 * ```
 */
export async function extractUsageFromFiles(patterns) {
    const usage = {
        classes: new Set(),
        ids: new Set(),
        tags: new Set(),
 ...
 */
export function extractUsageFromHTML(html) {
    const usage = {
        classes: new Set(),
        ids: new Set(),
        tags: new Set(),
 ...
 */
export function extractUsageFromJSX(source) {
    const usage = {
        classes: new Set(),
        ids: new Set(),
        tags: new Set(),
 ...
 * ```
 */
export function purgeUnusedCSS(css, usage) {
 ...
 */
export function atomicDeduplication(css) {
    const nodes = parseCSS(css);
    const declarationUsage = new Map();
 ...
 */
export function extractCriticalCSS(fullCSS, aboveFoldSelectors) {
 ...
 * ```
 */
export function splitCSSByRoute(routeSelectors) {
 ...
 */
export function generateSourceMap(css, sourceFile) {
 ...
 */
export function generateOptimizationReport(originalCSS, optimizedCSS, options = {}) {
