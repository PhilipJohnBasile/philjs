 ...
 */
function generateSourceMap(css) {
 ...
 * ```
 */
export function extractCSS(config) {
    const sections = [];
 ...
 */
export async function extractToFile(filePath, config) {
 ...
 */
export function createRollupPlugin(config) {
    return {
        name: 'philjs-css',
        async generateBundle() {
            if (!config?.outputPath)
 ...
 */
export function createWebpackPlugin(config) {
    return class PhilJSCSSPlugin {
        apply(compiler) {
            compiler.hooks.emit.tapAsync('PhilJSCSSPlugin', async (compilation, callback) => {
                try {
                    const css = extractCSS({
                        minify: config?.minify ?? true,
                        sourceMap: config?.sourceMap ?? true,
 ...
 * ```
 */
export function extractCriticalCSS(html, config) {
 ...
 * ```
 */
export function analyzeCSSBundle() {
    const css = extractCSS();
