 ...
 */
export function attachGestures(element, handlers, config = {}) {
    ...
    function handlePointerMove(e) {
        ...
        if (pointers.size >= 2) {
            ...
            if (Math.abs(newScale - state.scale) > mergedConfig.pinch.threshold) {
                state.scale = Math.max(mergedConfig.pinch.minScale, Math.min(mergedConfig.pinch.maxScale, newScale));
    ...
    function handlePointerUp(e) {
        ...
        // Swipe detection
        if (distance >= mergedConfig.swipe.threshold &&
...
export function createGestureAnimation(options) {
    ...
    if (gesture === 'pan') {
        ...
        handlers.onPinch = (e) => {
            ...
            element.style.setProperty(property, `${Math.max(min, Math.min(max, value))}${unit}`);
        };
    }
 ...
 */
export function swipeToDismiss(element, options) {
    ...
    return attachGestures(element, {
        onPan: (e) => {
            ...
            element.style.opacity = `${1 - Math.abs(currentX) / (threshold * 2)}`;
        },
        onPanEnd: (e) => {
...
function createGestureEvent(type, state, target, originalEvent) {
    return {
        type,
        state: { ...state },
        target,
...
function createSwipeEvent(state, velocity, target, originalEvent) {
    return {
...
function createPinchEvent(state, center, target, originalEvent) {
    return {
...
function createRotateEvent(state, center, target, originalEvent) {
    return {
...
function createPanEvent(state, delta, offset, target, originalEvent) {
    return {
