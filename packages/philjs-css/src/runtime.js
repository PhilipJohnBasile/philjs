 ...
 */
export function injectStyle(className, cssText) {
 ...
 */
export function removeStyle(className) {
    if (!sheetManager.rules.has(className)) {
        return;
    }
 ...
 */
export function getCriticalSSRStyles(usedClasses) {
    const allStyles = styleRegistry.getStyles();
 ...
 * ```
 */
export function createDynamicStyle(initialStyles) {
 ...
 * ```
 */
export function createReactiveStyle(getStyles) {
 ...
 * ```
 */
export function createThemeToggle(lightTheme, darkTheme) {
 ...
 * ```
 */
export function syncWithSystemTheme(lightTheme, darkTheme) {
    if (typeof window === 'undefined') {
        return () => { };
    }
 ...
 */
export function removeCSSVariable(name, element = document.documentElement) {
    if (typeof document === 'undefined') {
        return;
    }
 ...
 * ```
 */
export function batchStyleUpdates(fn) {
    // Collect all updates
    const pendingRules = new Map();
    const originalInject = injectStyle;
    // Override inject to collect
    globalThis.__philjsBatchedInject = (className, cssText) => {
    ...
    try {
        ...
        delete globalThis.__philjsBatchedInject;
    }
    // Apply all at once
 ...
 * ```
 */
export function prefetchStyles(styles) {
    if (typeof document === 'undefined') {
        return;
    }
