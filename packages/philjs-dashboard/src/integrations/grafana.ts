 ...
 */
function encodePrometheusTimeSeries(timeseries: PrometheusTimeSeries[]): string {
...
export class GrafanaExporter {
  private config: Required<GrafanaConfig>;
  private metricsQueue: PrometheusTimeSeries[] = [];
  private logsQueue: LokiLogEntry[] = [];
  private tracesQueue: TempoSpan[] = [];
  ...
  exportWebVitals(webVitals: WebVitalsMetrics): void {
    ...
    if (!this.config.prometheusUrl) return;

    const timestamp = Date.now();
    const baseLabels = this.getBaseLabels();

    const metrics: Array<{ name: string; value: number | null; help: string }> = [
  ...
  log(
    ...
    const existingStream = this.logsQueue.find(
      (s) => JSON.stringify(s.stream) === JSON.stringify(streamLabels)
    );
  ...
  exportTrace(spans: Span[]): void {
    if (this.isDestroyed) return;
    if (!this.config.tempoUrl) return;
  ...
  private getBaseLabels(): Record<string, string> {
    return {
      job: this.config.job,
      instance: this.config.instance,
      ...this.config.labels,
  ...
  private async sendMetrics(timeseries: PrometheusTimeSeries[]): Promise<void> {
  ...
  private async sendLogs(streams: LokiLogEntry[]): Promise<void> {
  ...
  private async sendTraces(spans: TempoSpan[]): Promise<void> {
...
export function createGrafanaExporter(config: GrafanaConfig): GrafanaExporter {
 ...
 */
export function generateWebVitalsDashboard(
  job: string = 'philjs-dashboard'
): GrafanaDashboard {
  return {
