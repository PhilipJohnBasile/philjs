...
export class DrizzleMigrationIntegration {
     ...
     */
    static async importDrizzleMigrations(drizzleDir) {
     ...
     */
    static async exportToDrizzleSchema(migrations) {
     ...
     */
    static tableToMigration(tableDef) {
        return {
    ...
    static columnToBuilder(table, name, column) {
        switch (column.type) {
            case 'serial':
            case 'integer':
