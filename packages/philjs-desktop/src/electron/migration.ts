...
export const ElectronToTauriMapper = {
   ...
   */
  checkCompatibility(api: string): { supported: boolean; alternative?: string } {
    const compatibility: Record<string, { supported: boolean; alternative?: string }> = {
      // Fully supported
...
export function createMigrationHelper() {
  return {
     ...
     */
    analyzeCode(code: string): Array<{ api: string; line: number; suggestion: string }> {
      const issues: Array<{ api: string; line: number; suggestion: string }> = [];
