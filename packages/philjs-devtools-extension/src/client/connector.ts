...
let componentRoot: ComponentNode | null = null;
...
export function connectDevTools(): void {
  ...
  if (!(window as any).__PHILJS_DEVTOOLS__) {
    // Create hook for extension to detect
    (window as any).__PHILJS_DEVTOOLS_HOOK__ = {
 ...
 */
function sendToDevTools(message: DevToolsMessage): void {
  if (!isConnected) return;
 ...
 */
function handleDevToolsMessage(event: MessageEvent): void {
 ...
 */
function hookIntoSignals(): void {
  const philjs = (window as any).__PHILJS__;
  if (!philjs?.internals?.signals) return;

  const originalCreate = philjs.internals.signals.create;
  philjs.internals.signals.create = function(initialValue: unknown, options?: any) {
    ...
    signal.set = function(newValue: unknown) {
 ...
 */
function hookIntoComponents(): void {
  const philjs = (window as any).__PHILJS__;
  if (!philjs?.internals?.components) return;

  // Hook into component mounting
  const originalMount = philjs.internals.components.mount;
  philjs.internals.components.mount = function(component: any, container: any) {
  ...
  philjs.internals.components.update = function(component: any) {
 ...
 */
function hookIntoPerformance(): void {
  // Web Vitals
...
function updateComponentTree(): void {
  const philjs = (window as any).__PHILJS__;
  if (!philjs?.internals?.getComponentTree) return;
 ...
 */
function handleSelectComponent(componentId: string): void {
  const philjs = (window as any).__PHILJS__;
  if (!philjs?.internals?.getComponentById) return;
 ...
 */
function handleHighlightComponent(componentId: string | null): void {
 ...
 */
function handleModifySignal(signalId: string, value: unknown): void {
  const philjs = (window as any).__PHILJS__;
  if (!philjs?.internals?.getSignalById) return;
