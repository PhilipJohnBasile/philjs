...
export function showOverlay() {
    ...
    if (typeof window !== 'undefined' && window.__PHILJS_PROD__)
        return;
...
function updateStats() {
    ...
    if (islandCount)
        islandCount.textContent = String(islands.length);
    if (hydratedCount)
        hydratedCount.textContent = String(hydrated.length);
    if (routePath && routeParams && routeError) {
        const info = window.__PHILJS_ROUTE_INFO__?.current;
