...
export class StatePanel {
  ...
  constructor(container: HTMLElement, maxHistorySize: number = 50) {
  ...
  public recordStateChange(path: string, value: unknown): void {
    ...
    this.container.querySelectorAll('.state-expand-toggle').forEach(toggle => {
      toggle.addEventListener('click', (e) => {
        ...
        if (content) {
          ...
          (content as HTMLElement).style.display = isHidden ? 'block' : 'none';
  ...
  private renderSignal(signal: SignalInfo): string {
  ...
  private renderStateTree(obj: Record<string, unknown>, path: string[] = [], depth: number = 0): string {
    let html = '';
        ...
        <div style="padding: 4px 0; padding-left: ${indent}px; border-bottom: 1px solid #2a2a2a;">
          ...
          ${isObject ? `
            <div class="state-children" style="margin-top: 4px;">
              ${this.renderStateTree(value as Record<string, unknown>, currentPath, depth + 1)}
            </div>
          ` : ''}
        </div>
      `;
