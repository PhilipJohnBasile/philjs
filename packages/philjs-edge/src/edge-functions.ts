    ...
    if (config?.timeout) {
      response = await Promise.race([
        ...
        new Promise<Response>((_, reject) =>
          setTimeout(() => reject(new Error('Function timeout')), config.timeout)
        ),
      ]).catch(error => {
 ...
 * Compose middleware
 */
export function compose(...middlewares: EdgeMiddleware[]): EdgeMiddleware {
...
export function applyMiddleware(
  ...
  return (ctx: EdgeContext) => composed(ctx, () => handler(ctx));
}
...
function defaultLogFormat(ctx: EdgeContext, duration: number, status: number): string {
 ...
 * Timing middleware
 */
export function timing(): EdgeMiddleware {
  return async (ctx, next) => {
    const start = Date.now();
    const response = await next();
    const duration = Date.now() - start;
 ...
 */
export function bodyParser(): EdgeMiddleware {
  return async (ctx, next) => {
...
export function auth(
  authenticate: (ctx: EdgeContext) => Promise<{ valid: boolean; user?: unknown }>
): EdgeMiddleware {
  return async (ctx, next) => {
...
export function apiKeyAuth(
  validateKey: (key: string) => Promise<{ valid: boolean; user?: unknown }>,
...
function handleCORS(request: Request, config: CORSConfig): Response {
...
function addCORSHeaders(response: Response, request: Request, config: CORSConfig): Response {
  const newResponse = new Response(response.body, response);
...
export function stream(
  generator: () => AsyncGenerator<Uint8Array | string>,
  init?: ResponseInit
): Response {
