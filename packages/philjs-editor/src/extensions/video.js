 ...
 */
export function createVideoExtensions(options = {}) {
    const { youtube = true, vimeo = true, customVideo = true, width = 640, height = 360, allowFullscreen = true, } = options;
    const extensions = [];
    if (youtube) {
    ...
    if (vimeo) {
        extensions.push(Vimeo.configure({
            width,
            height,
            allowFullscreen,
        }));
