...
export class InMemoryQueue implements EmailQueue {
  private jobs: Map<string, EmailQueueJob> = new Map();
   ...
   */
  async process(
    handler: (job: EmailQueueJob) => Promise<EmailResult>
  ): Promise<void> {
    if (this.processing) {
      return;
    }
   ...
   * Get a job by ID
   */
  async getJob(id: string): Promise<EmailQueueJob | null> {
  ...
  private getPendingJobs(): EmailQueueJob[] {
    const now = new Date();
    const pending: EmailQueueJob[] = [];
...
export function createQueue(
  options?: ConstructorParameters<typeof InMemoryQueue>[0]
): InMemoryQueue {
