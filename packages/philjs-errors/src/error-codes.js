...
export const ERROR_CATALOG = {
    ...
    'PHIL-001': {
        ...
        message: (ctx) => `Cannot read signal '${ctx?.['signalName'] || 'unknown'}' during its own update. This creates an infinite loop.`,
        suggestions: [
            {
    ...
    'PHIL-002': {
        ...
        message: (ctx) => `Circular dependency detected in signal graph: ${ctx?.['cycle']?.join(' -> ') || 'unknown cycle'}`,
        suggestions: [
            {
    ...
    'PHIL-003': {
        ...
        message: (ctx) => `${ctx?.['count'] || 'Multiple'} signals updated consecutively without batching. This may cause unnecessary re-renders.`,
        suggestions: [
            {
    ...
    'PHIL-004': {
        ...
        message: (ctx) => `Effect at ${ctx?.['location'] || 'unknown location'} does not return a cleanup function. This may cause memory leaks.`,
        suggestions: [
            {
    ...
    'PHIL-005': {
        ...
        message: (ctx) => `Memo computation at ${ctx?.['location'] || 'unknown location'} returns undefined. Ensure your memo function returns a value.`,
        suggestions: [
            {
    ...
    'PHIL-100': {
        ...
        message: (ctx) => `Hydration mismatch detected at ${ctx?.['path'] || 'unknown'}. Server HTML does not match client render.`,
        suggestions: [
            {
    ...
    'PHIL-101': {
        ...
        message: (ctx) => `Browser API '${ctx?.['api'] || 'unknown'}' called during server-side rendering. This API is not available on the server.`,
        suggestions: [
            {
    ...
    'PHIL-102': {
        ...
        message: (ctx) => `Required data for SSR not available. Expected '${ctx?.['dataKey'] || 'data'}' to be present.`,
        suggestions: [
            {
    ...
    'PHIL-200': {
        ...
        message: (ctx) => `Invalid route pattern: '${ctx?.['pattern'] || 'unknown'}'. ${ctx?.['reason'] || 'Check syntax'}.`,
        suggestions: [
            {
    ...
    'PHIL-201': {
        ...
        message: (ctx) => `Route parameter '${ctx?.['paramName'] || 'unknown'}' is required but not provided in navigation.`,
        suggestions: [
            {
    ...
    'PHIL-202': {
        ...
        message: (ctx) => `No route matches path '${ctx?.['path'] || 'unknown'}'. Did you forget to add a catch-all route?`,
        suggestions: [
            {
    ...
    'PHIL-300': {
        ...
        message: (ctx) => `Invalid JSX syntax at ${ctx?.['location'] || 'unknown location'}: ${ctx?.['details'] || 'syntax error'}`,
        suggestions: [
            {
    ...
    'PHIL-301': {
        ...
        message: (ctx) => `Unsupported feature: ${ctx?.['feature'] || 'unknown'}. ${ctx?.['reason'] || 'This feature is not yet supported by PhilJS'}.`,
        suggestions: [
            {
    ...
    'PHIL-302': {
        ...
        message: (ctx) => `Potential optimization issue detected: ${ctx?.['issue'] || 'unknown'}`,
        suggestions: [
            {
    ...
    'PHIL-400': {
        ...
        message: (ctx) => `Error rendering component '${ctx?.['componentName'] || 'Unknown'}': ${ctx?.['error'] || 'unknown error'}`,
        suggestions: [
            {
    ...
    'PHIL-401': {
        ...
        category: 'component',
        title: 'Invalid Props',
        message: (ctx) => `Invalid props passed to component '${ctx?.['componentName'] || 'Unknown'}': ${ctx?.['invalidProps']?.join(', ') || 'unknown'}`,
        suggestions: [
            {
    ...
    'PHIL-500': {
        ...
        message: (ctx) => `Cannot read property '${ctx?.['property'] || 'unknown'}' of ${ctx?.['valueType'] || 'null/undefined'}`,
        suggestions: [
            {
    ...
    'PHIL-501': {
        ...
        message: (ctx) => `Async operation failed: ${ctx?.['operation'] || 'unknown'}. ${ctx?.['error'] || 'Check error details'}`,
        suggestions: [
            {
 ...
 */
export function createPhilJSError(code, context, originalError) {
    const definition = getErrorDefinition(code);
    if (!definition) {
        const error = new Error(`Unknown PhilJS error code: ${code}`);
        error.code = code;
    ...
    const error = (originalError ? Object.create(originalError) : new Error(message));
...
export function formatError(error, options = {}) {
    const { includeStack = true, includeSuggestions = true, includeDocLink = true, } = options;
