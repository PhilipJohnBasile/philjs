 ...
 */
function createOverlayElement(error) {
...
export function initErrorOverlay() {
    ...
    window.__PHILJS_HIDE_ERROR_OVERLAY__ = hideErrorOverlay;
    // Intercept unhandled errors in development
    if (process.env['NODE_ENV'] === 'development') {
        window.addEventListener('error', (event) => {
            // Check if this is a PhilJS error
            if (event.error?.code?.startsWith('PHIL-')) {
        ...
        window.addEventListener('unhandledrejection', (event) => {
            // Check if this is a PhilJS error
            if (event.reason?.code?.startsWith('PHIL-')) {
                event.preventDefault();
 ...
 */
export function isErrorOverlayVisible() {
 ...
 */
export function updateErrorOverlay(error) {
