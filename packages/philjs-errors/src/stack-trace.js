 ...
 */
export function getPrimaryLocation(error) {
    ...
    if (!primaryFrame || !primaryFrame.fileName)
        return null;
    return {
...
export function formatStackTrace(error, options = {}) {
    const { maxFrames = 10, showFramework = false, showNodeModules = false, highlightUserCode = true, } = options;
    const processed = processStack(error);
 ...
 */
export async function getCodeSnippet(location, options = {}) {
 ...
 */
export async function loadSourceMap(fileName) {
 ...
 */
export async function applySourceMap(location) {
 ...
 */
export function enhanceErrorStack(error) {
    const processed = processStack(error);
    ...
    error.__originalStack = error.stack;
 ...
 */
export function formatErrorForDev(error) {
    const processed = processStack(error);
