 ...
 */
export function toXML<T>(data: T, options: XMLOptions = {}): string {
  const {
    rootElement = 'root',
    declaration = true,
    version = '1.0',
 ...
 */
export function arrayToXML<T extends Record<string, unknown>>(
  data: T[],
  options: XMLOptions = {}
): string {
  const {
 ...
 */
export async function* streamToXML<T extends Record<string, unknown>>(
  data: T[] | AsyncIterable<T>,
  options: StreamingXMLOptions = {}
...
function valueToXML(
    ...
    return value
      .map(item => {
        if (typeof item === 'object' && item !== null) {
          return objectToXML(item as Record<string, unknown>, itemElement, options, depth, indent);
        }
  ...
  if (typeof value === 'object') {
    return objectContentToXML(value as Record<string, unknown>, options, depth, indent);
  }
  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    if (Array.isArray(transformedValue)) {
      ...
      for (const item of transformedValue) {
        if (typeof item === 'object' && item !== null) {
          lines.push(objectToXML(item as Record<string, unknown>, elemName, options, depth, indent));
        } else {
          lines.push(`${prefix}<${elemName}>${escapeXML(formatValue(item, options))}</${elemName}>`);
        }
      }
    } else if (typeof transformedValue === 'object') {
      lines.push(objectToXML(transformedValue as Record<string, unknown>, elemName, options, depth, indent));
    } else {
 ...
 */
function formatValue(value: unknown, options: XMLOptions): string {
  if (value === null || value === undefined) {
    return '';
  }
