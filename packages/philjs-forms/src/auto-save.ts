...
export function createAutoSave(config: AutoSaveConfig): AutoSaveController {
  ...
  function restore(): FormDraft | null {
    ...
    try {
      const draft: FormDraft = JSON.parse(decryptData(savedDraft));
  ...
  function getVersions(): FormDraft[] {
    return [...state.versions];
  }

  function restoreVersion(version: number): FormDraft | null {
...
export function useAutoSave<T extends Record<string, unknown>>(
  formData: T,
  config: Omit<AutoSaveConfig, 'key'> & { key: string }
): {
 ...
 */
export function createIndexedDBStorage(dbName: string = 'formDrafts'): Promise<Storage> {
 ...
 */
export function getRecoveryMessage(draft: FormDraft): string {
