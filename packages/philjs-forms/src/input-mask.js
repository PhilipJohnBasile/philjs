 ...
 */
export function parseMaskPattern(pattern) {
    const result = [];
    let escaped = false;
 ...
 */
export function unmask(maskedValue, pattern) {
    const maskPattern = typeof pattern === 'string' ? parseMaskPattern(pattern) : pattern;
    let rawValue = '';
    let maskIndex = 0;
    for (const char of maskedValue) {
        if (maskIndex >= maskPattern.length)
            break;
 ...
 */
export function phoneMask(value, options = {}) {
...
export function currencyMask(value, options = {}) {
    const { symbol = '$', symbolPosition = 'prefix', thousandsSeparator = ',', decimalSeparator = '.', decimalPlaces = 2, allowNegative = false, } = options;
    // Extract sign
    ...
    const numericValue = parseFloat((isNegative ? '-' : '') + integerPart + (decimalPart ? '.' + decimalPart : ''));
    return {
        maskedValue: formatted,
 ...
 * Time mask
 */
export function timeMask(value, options = {}) {
 ...
 * ZIP code mask
 */
export function zipCodeMask(value, options = {}) {
 ...
 */
export function createMaskInputHandler(maskFn, onChange) {
    return {
        onInput: (event) => {
