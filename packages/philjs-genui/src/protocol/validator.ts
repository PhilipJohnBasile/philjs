...
export function validateMessage(message: unknown): ValidationResult {
  ...
  if (result.success) {
    return {
      valid: true,
      errors: [],
      data: result.data as A2UIMessage,
    };
  }

  const errors: A2UIError[] = result.error.issues.map((issue) => ({
    code: 'INVALID_MESSAGE' as A2UIErrorCode,
...
export function validateComponent(component: unknown): ValidationResult {
  ...
  const errors: A2UIError[] = result.error.issues.map((issue) => ({
    code: 'INVALID_COMPONENT' as A2UIErrorCode,
    message: issue.message,
    details: {
      path: issue.path.join('.'),
      code: issue.code,
