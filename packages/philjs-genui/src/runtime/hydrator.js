 ...
 */
import { ASTValidator } from '../sandbox/ast-validator.js';
/**
 * GenUI Hydrator
...
export class GenUIHydrator {
     ...
     */
    hydrateRenderPayload(payload, container) {
    ...
    createLayoutElement(layout) {
        ...
        switch (layout.type) {
            ...
            case 'grid':
                ...
                if (layout.columns)
                    element.style.gridTemplateColumns = String(layout.columns);
                if (layout.rows)
                    element.style.gridTemplateRows = String(layout.rows);
                break;
            case 'flow':
        ...
        if (layout.gap)
            element.style.gap = String(layout.gap);
        if (layout.align)
            element.style.alignItems = layout.align;
        if (layout.justify) {
    ...
    renderComponent(component, context) {
        ...
        if (component.each) {
            ...
            const items = this.resolvePath(context.signals, component.each.source);
            ...
            case 'signal':
                ...
                if (signal) {
                    if (handler.action === 'set') {
                        ...
                        signal.set(handler.value(signal.get()));
                    }
     ...
     */
    evaluateTransform(transform, value, context) {
        ...
        try {
            ...
            if (transform === 'trim' && typeof value === 'string')
                return value.trim();
            if (transform === 'length' && (typeof value === 'string' || Array.isArray(value)))
                return value.length;
            return value;
        }
        catch {
     ...
     */
    setElementProperty(element, prop, value) {
     ...
     */
    applyComponentDecorations(element, component) {
 ...
 */
export function createHydrator(registry, options) {
