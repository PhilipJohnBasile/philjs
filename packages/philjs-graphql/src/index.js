...
export function createMutation(client, mutation) {
    ...
    const execute = async (variables) => {
        try {
            ...
            const response = await client.mutate(mutationOptions);
            if (response.errors && response.errors.length > 0) {
                throw new Error(response.errors[0].message);
            }
            data.set(response.data);
...
export { SubscriptionClient, SubscriptionHandle, createSubscriptionClient, useSubscription, } from './subscription.js';
// Export persisted query utilities
export { PersistedQueryManager, PersistedQueryRegistry, createPersistedQueryManager, createPersistedQueryRegistry, shouldRetryWithFullQuery, buildPersistedQueryRequest, extractQueryHash, generatePersistedQueryManifest, } from './persisted.js';
// Export fragment utilities
export { FragmentRegistry, defineFragment, maskFragment, unmaskFragment, isMaskedFragment, spreadFragment, composeFragments, buildQueryWithFragments, useFragment, withFragment, getComponentFragment, mergeFragmentData, selectFromFragment, getFragmentRegistry, createFragmentRegistry, fragment, inlineFragment, FragmentUtils, } from './fragments.js';
// Export optimistic update utilities
export { OptimisticUpdateManager, OptimisticResponseBuilder, MutationQueue, LastWriteWinsResolver, FirstWriteWinsResolver, CustomConflictResolver, createOptimisticUpdateManager, buildOptimisticResponse, createMutationQueue, } from './optimistic.js';
// Export code generation utilities
export { GraphQLCodegen, BatchCodegen, createCodegen, createBatchCodegen, runCodegen, extractOperationInfo, extractFragmentInfo, } from './codegen-enhanced.js';
