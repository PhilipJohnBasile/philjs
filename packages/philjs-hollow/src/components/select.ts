...
export class HollowSelect extends HollowElement {
  ...
  protected override template(): string {
    ...
    const options = this.getProp<SelectOption[]>('options', []);
  ...
  private handleOutsideClick(event: Event): void {
    if (!this.contains(event.target as Node) && this._isOpen) {
      this._isOpen = false;
      this._searchQuery = '';
      this.scheduleRenderManual();
    }
  ...
  private getFilteredOptions(): SelectOption[] {
    const options = this.getProp<SelectOption[]>('options', []);
    if (!this._searchQuery) return options;
  ...
  private scrollToHighlighted(): void {
    requestAnimationFrame(() => {
      const highlighted = this.query<HTMLElement>(`.option[data-index="${this._highlightedIndex}"]`);
  ...
  private selectValue(value: string): void {
    ...
    this.emit('hollow-change', {
      ...
      option: this.getProp<SelectOption[]>('options', []).find((opt) => opt.value === value),
    });
   ...
   */
  private scheduleRenderManual(): void {
    queueMicrotask(() => {
      this.render();
    });
  }
   ...
   */
  public focus(): void {
    const trigger = this.query<HTMLElement>('.select-trigger');
    trigger?.focus();
  }
}
