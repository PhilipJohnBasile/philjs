...
export class HollowTabs extends HollowElement {
  ...
  protected override template(): string {
    ...
    const tabs = this.getProp<TabDefinition[]>('tabs', []);
  ...
  private handleKeyDown(event: KeyboardEvent): void {
    ...
    if (enabledTabs[newIndex]) {
      ...
      requestAnimationFrame(() => {
        const tabEl = this.query<HTMLElement>(`[data-tab-id="${enabledTabs[newIndex]!.id}"]`);
        tabEl?.focus();
      });
    }
  }
   ...
   */
  public setTabs(tabs: TabDefinition[]): void {
...
export class HollowTab extends HollowElement {
  static override observedAttributes = ['active', 'disabled'];
...
export class HollowTabPanel extends HollowElement {
  static override observedAttributes = ['tab', 'active'];
