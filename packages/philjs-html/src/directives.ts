...
directive('if', (el, expression, context) => {
  ...
  return effect(() => {
    ...
    if (value && !currentEl) {
      currentEl = template.cloneNode(true) as HTMLElement;
...
directive('on', (el, expression, context) => {
  ...
  const listener = (e: Event) => {
    ...
    const dataValues = Object.values(context.data).map((s: any) =>
...
directive('model', (el, expression, context) => {
  ...
  const handler = () => {
    if (input.type === 'checkbox') {
      ...
      if (input.checked) sig.set?.(input.value);
    } else {
      sig.set?.(input.value);
    }
  };
 ...
 */
function evaluateExpression(expression: string, context: DirectiveContext): any {
  try {
    const fn = new Function(
      ...Object.keys(context.data),
      '$el',
 ...
 */
export function processElement(el: HTMLElement, context: DirectiveContext): void {
  const cleanups: (() => void)[] = [];
 ...
 */
export function initDirectives(root: HTMLElement = document.body): void {
