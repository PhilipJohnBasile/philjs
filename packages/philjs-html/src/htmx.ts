 ...
 */
export function configure(options: Partial<HtmxConfig>): void {
 ...
 */
function setupRequest(el: HTMLElement, method: string, url: string): void {
  const trigger = parseTrigger(el.getAttribute('hx-trigger') || getDefaultTrigger(el));

  const handler = async (e: Event) => {
    // Check trigger conditions
    if (!shouldTrigger(trigger, e, el)) return;
 ...
 */
async function makeRequest(el: HTMLElement, method: string, url: string): Promise<void> {
  const target = getTarget(el);
  const swap = (el.getAttribute('hx-swap') || config.defaultSwap) as SwapStrategy;
  ...
  if (!el.dispatchEvent(beforeEvent)) return;
...
function getDefaultTrigger(el: HTMLElement): string {
...
function shouldTrigger(trigger: Trigger, e: Event, el: HTMLElement): boolean {
  // Check filter
  if (trigger.filter) {
    try {
...
function getTarget(el: HTMLElement): HTMLElement | null {
  const targetSelector = el.getAttribute('hx-target');

  if (!targetSelector) return el;
...
function getValues(el: HTMLElement): Record<string, string> {
...
function getHeaders(el: HTMLElement): Record<string, string> {
...
function getIndicator(el: HTMLElement): HTMLElement | null {
...
function handleResponseHeaders(headers: Headers, el: HTMLElement): void {
  // HX-Redirect
...
function swapContent(target: HTMLElement, html: string, strategy: SwapStrategy): void {
  // Parse HTML
 ...
 */
function boostElement(el: HTMLElement): void {
  // Boost links
  if (el.tagName === 'A') {
 ...
 */
export function initHtmx(root: HTMLElement = document.body): void {
  process(root);
