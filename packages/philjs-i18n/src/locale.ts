 ...
 */
export function detectLocaleFromCookie(cookieName = 'locale'): string | null {
 ...
 */
export function detectLocaleFromStorage(key = 'locale'): string | null {
 ...
 */
export function detectLocaleFromDocument(): string | null {
  if (typeof document === 'undefined') return null;
...
export interface LocaleDetectionOptions {
  ...
  urlOptions?: { pathIndex?: number; paramName?: string };
...
export function detectLocale(options: LocaleDetectionOptions = {}): string {
  const {
    sources = ['url', 'cookie', 'storage', 'document', 'browser'],
    availableLocales,
    fallback = 'en-US',
    urlOptions,
    cookieName,
 ...
 */
export function saveLocaleToStorage(locale: string, key = 'locale'): void {
 ...
 */
export function updateDocumentLocale(locale: string): void {
  if (typeof document === 'undefined') return;
  document.documentElement.lang = locale;
}

// ============================================================================
// Reactive Locale Manager
// ============================================================================
...
export function createLocaleManager(
  ...
  return {
    ...
    isAvailable: (locale: string) =>
      availableLocales.includes(normalizeLocale(locale)),
  };
}
