 ...
 */
export function startKeyboardListening() {
    if (isListening)
        return;
    isListening = true;
 ...
 */
function makeShortcutKey(shortcut) {
    const parts = [];
    if (shortcut.ctrl)
        parts.push('ctrl');
    if (shortcut.shift)
...
export class ComponentNavigator {
    ...
    nextSibling(current) {
        if (!current || !current.parentElement)
            return null;
        const siblings = Array.from(current.parentElement.children).filter((el) => this.elements.includes(el));
