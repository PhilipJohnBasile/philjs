...
let onSelectCallback: ((component: ComponentInfo) => void) | null = null;
 ...
 * Hide search box
 */
export function hideSearchBox(): void {
  if (searchBox) {
    searchBox.remove();
    searchBox = null;
    searchInput = null;
 ...
 */
export function isSearchBoxVisible(): boolean {
...
function handleSearchKeyDown(event: KeyboardEvent): void {
  ...
  const resultElements = Array.from(
    searchResults.querySelectorAll('.philjs-search-result')
  ) as HTMLElement[];

  if (resultElements.length === 0) return;

  const selectedIndex = resultElements.findIndex((el) =>
    el.style.background.includes('rgba(59, 130, 246, 0.2)')
  );
 ...
 */
function selectResultByIndex(elements: HTMLElement[], index: number): void {
 ...
 */
function selectSearchResult(component: ComponentInfo): void {
  if (onSelectCallback) {
 ...
 */
function stylesToString(styles: Record<string, string | number>): string {
 ...
 */
export function getSearchStats(components: ComponentInfo[]): {
  total: number;
  islands: number;
  hydrated: number;
  withSignals: number;
