 ...
 */
export function updateTooltip(componentInfo: ComponentInfo): void {
  if (!tooltip) return;
 ...
 */
function buildTooltipContent(info: ComponentInfo): string {
 ...
 */
function positionTooltip(position?: { x: number; y: number }): void {
  if (!tooltip) return;
...
function setupTooltipEvents(): void {
  ...
  if (sourceLink) {
    sourceLink.addEventListener('click', (e) => {
      ...
      // Try to open in IDE
      if (typeof window !== 'undefined' && (window as any).__PHILJS_OPEN_IN_IDE__) {
        (window as any).__PHILJS_OPEN_IN_IDE__(href);
      } else {
...
function makeDraggable(element: HTMLElement): void {
  ...
  const header = element.querySelector('[style*="tooltipHeader"]') as HTMLElement;
  if (!header) return;
 ...
 */
export function getCurrentTooltipComponent(): ComponentInfo | null {
