...
export class IntentResolver {
    ...
    learn(intent, correctedImplementation) {
...
// Intent DSL
// ============================================================================
export function intent(description) {
...
class IntentBuilder {
    ...
    must(constraint, validate) {
        this._intent.constraints = this._intent.constraints || [];
        const c = { type: 'must', description: constraint };
        if (validate)
            c.validate = validate;
    ...
    should(constraint, validate) {
        this._intent.constraints = this._intent.constraints || [];
        const c = { type: 'should', description: constraint };
        if (validate)
            c.validate = validate;
        this._intent.constraints.push(c);
        return this;
    }
    mustNot(constraint, validate) {
        this._intent.constraints = this._intent.constraints || [];
        const c = { type: 'must-not', description: constraint };
        if (validate)
            c.validate = validate;
        this._intent.constraints.push(c);
        return this;
    ...
    async resolve(resolver) {
        ...
        if (!r)
            throw new Error('No intent resolver available');
...
export function initIntent(config) {
...
export function useIntent(description) {
