...
export class Monitor {
    ...
    async getHealth() {
        ...
        if (metrics.throughput < this.options.healthThresholds.processingRateWarning &&
            queueDepth > 0) {
            if (status === 'healthy')
                status = 'degraded';
    ...
    async getJobDetails(jobId) {
        ...
        const duration = startTime && job.finishedAt
     ...
     */
    recordJobCompletion(jobId, _success) {
        const startTime = this.jobTimings.get(jobId);
        if (startTime) {
...
<body>
  <div class="container">
    ...
    calculateThroughput() {
        ...
        const jobsDiff = recent.metrics.completedJobs - previous.metrics.completedJobs;
        if (timeDiff === 0) {
            return 0;
        }
