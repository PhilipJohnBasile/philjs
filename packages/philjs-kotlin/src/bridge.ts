...
export class BridgeRuntime implements Disposable {
  private messageId = 0;
  private readonly pendingCalls = new Map<string, PendingCall<unknown>>();
  private readonly eventHandlers = new Map<string, Set<EventHandler>>();
  private readonly onceHandlers = new Map<string, Set<EventHandler>>();
  private readonly config: Required<BridgeRuntimeConfig>;
   ...
   */
  private defaultSendMessage(message: string): void {
    if (typeof window !== 'undefined' && 'PhilJSNative' in window) {
      (window as Window & { PhilJSNative: { postMessage: (msg: string) => void } })
  ...
  private setupMessageReceiver(): void {
    if (typeof window !== 'undefined') {
      (window as Window & { __philjs_bridge_receive__?: (msg: string) => void })
   ...
   */
  private handleCall(message: BridgeMessage): void {
    const eventName = message.method;
    if (!eventName) return;
    ...
    if (handlers && handlers.size > 0) {
      ...
      if (handler) {
        try {
          const result: unknown = handler(message.params);
          // If result is a promise, wait for it
          if (result !== null && typeof result === 'object' && 'then' in result && typeof result.then === 'function') {
            (result as Promise<unknown>).then(
              (res) => this.sendResponse(message.id, res),
              (err) => this.sendError(message.id, err)
            );
          } else {
  ...
  dispose(): void {
    ...
    if (typeof window !== 'undefined') {
      delete (window as Window & { __philjs_bridge_receive__?: unknown }).__philjs_bridge_receive__;
    }
 ...
 */
export function createBridge(config?: BridgeRuntimeConfig): BridgeRuntime {
 ...
 */
export function useBridge(config?: BridgeRuntimeConfig): BridgeRuntime {
  return new BridgeRuntime(config);
}
