 ...
 */
export function createDiffer(): Differ {
  return {
    diff(oldHtml: string, newHtml: string): DOMPatch[] {
...
function parseHtml(html: string): VNode {
  ...
  const root: VNode = { type: 'element', tag: 'root', children: [] };
 ...
 */
function diffVdom(oldNode: VNode, newNode: VNode, path: string): DOMPatch[] {
  ...
  if (oldNode.type === 'element' && newNode.type === 'element') {
    ...
    oldChildren.forEach((child, i) => {
      if (child.key) oldKeyMap.set(child.key, { node: child, index: i });
    });
    newChildren.forEach((child, i) => {
      if (child.key) newKeyMap.set(child.key, { node: child, index: i });
    });
...
function diffKeyedChildren(
  ...
  newChildren: VNode[],
  path: string,
  oldKeyMap: Map<string, { node: VNode; index: number }>,
  newKeyMap: Map<string, { node: VNode; index: number }>
): DOMPatch[] {
  const patches: DOMPatch[] = [];
 ...
 */
export function applyPatches(container: HTMLElement, patches: DOMPatch[]): void {
