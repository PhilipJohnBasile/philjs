...
class MessageComponent {
    ...
    `;
        ...
        if (message.isStreaming) {
            const contentEl = this.element.querySelector(`#content-${message.id}`);
            if (contentEl) {
...
class ChatContainer {
    ...
    addMessage(message) {
        ...
        this.messagesContainer.insertBefore(component.getElement(), this.thinkingIndicator.getElement());
        this.scrollToBottom();
    }
    ...
    injectStyles() {
        if (document.getElementById('philjs-llm-ui-styles'))
            return;
...
function useChat(sendFn, options) {
...
function useStreamingText(config) {
...
export { 
// Core classes
ChatContainer, MessageComponent, ChatInput, ThinkingIndicator, StreamingText, MarkdownRenderer, 
// Hooks
useChat, useStreamingText };
