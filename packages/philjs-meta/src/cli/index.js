...
const log = {
    ...
    success: (msg) => console.log(`${colors.green}ready${colors.reset} - ${msg}`),
...
export async function dev(options = {}) {
    const { root = process.cwd(), port = 3000, host = 'localhost', hmr = true, verbose = false, } = options;
    ...
    const server = http.createServer(async (req, res) => {
        ...
        const context = createMiddlewareContext(new Request(url.toString(), {
            method: req.method ?? 'GET',
            headers: Object.entries(req.headers).reduce((acc, [key, value]) => {
                if (value)
                    acc.append(key, Array.isArray(value) ? value[0] : value);
                return acc;
            }, new Headers()),
        }));
        try {
...
export async function build(options = {}) {
    const { root = process.cwd(), minify = true, sourcemap = false, analyze = false, verbose = false, } = options;
...
export async function start(options = {}) {
    const { root = process.cwd(), port = 3000, host = '0.0.0.0', verbose = false, } = options;
    ...
    middlewareChain.use(async (_ctx, next) => {
...
export async function generate(options = {}) {
    const { root = process.cwd(), outDir = 'out', fallback = true, verbose = false, } = options;
 ...
 */
function generateDevHtml(pattern, pathname, hmr) {
    const hmrScript = hmr
        ? `
    <script>
 ...
 */
function generateProductionHtml(pattern) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
 ...
 * Generate 500 HTML
 */
function generate500Html(error) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
