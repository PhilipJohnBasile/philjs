 ...
 */
export function createSitemapEntry(path, options) {
 ...
 */
export function splitSitemap(entries, maxUrls = 50000) {
    const chunks = [];
 ...
 */
export function generateSitemapFromRoutes(routes, baseUrl, options) {
    ...
    return routes
        ...
        if (priority[route] !== undefined)
            entry.priority = priority[route];
        if (changefreq[route] !== undefined)
            entry.changefreq = changefreq[route];
        return entry;
    });
}
