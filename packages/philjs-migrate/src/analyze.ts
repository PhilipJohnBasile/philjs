...
function detectFramework(packageJson: any): 'react' | 'vue' | 'svelte' | 'unknown' {
...
function getFrameworkVersion(packageJson: any, framework: string): string | undefined {
  const deps = { ...packageJson.dependencies, ...packageJson.devDependencies };
...
function detectFeatures(packageJson: any): FrameworkFeatures {
  const deps = { ...packageJson.dependencies, ...packageJson.devDependencies };
  ...
  // State management
  if (deps.redux || deps['@reduxjs/toolkit']) features.stateManagement.push('Redux');
  if (deps.zustand) features.stateManagement.push('Zustand');
  if (deps.jotai) features.stateManagement.push('Jotai');
  if (deps.recoil) features.stateManagement.push('Recoil');
  if (deps.mobx) features.stateManagement.push('MobX');
  if (deps.pinia) features.stateManagement.push('Pinia');
  if (deps.vuex) features.stateManagement.push('Vuex');

  // CSS frameworks
  if (deps.tailwindcss) features.cssFramework.push('Tailwind');
  if (deps['styled-components']) features.cssFramework.push('Styled Components');
  if (deps['@emotion/react']) features.cssFramework.push('Emotion');
  if (deps.sass || deps['node-sass']) features.cssFramework.push('Sass');

  // Testing
  if (deps.jest) features.testingFramework.push('Jest');
  if (deps.vitest) features.testingFramework.push('Vitest');
  if (deps.cypress) features.testingFramework.push('Cypress');
  if (deps.playwright || deps['@playwright/test']) features.testingFramework.push('Playwright');

  return features;
}

async function detectPackageManager(projectPath: string): Promise<'npm' | 'yarn' | 'pnpm' | 'unknown'> {
  try {
