...
export class AngularTransform {
    ...
    transformImports(code, result) {
        ...
        transformed = transformed.replace(/import\s+\{([^}]+)\}\s+from\s+['"]@angular\/core['"]/g, (match, imports) => {
        ...
        transformed = transformed.replace(/import\s+\{([^}]+)\}\s+from\s+['"]@angular\/router['"]/g, (match, imports) => {
        ...
        transformed = transformed.replace(/import\s+\{([^}]+)\}\s+from\s+['"]@ngrx\/store['"]/g, (match, imports) => {
    ...
    transformAngularSignals(code, result) {
        let transformed = code;
        ...
        transformed = transformed.replace(/(\w+)\s*=\s*signal\(([^)]+)\)/g, (match, name, initialValue) => {
            result.warnings.push({
        ...
        transformed = transformed.replace(/(\w+)\s*=\s*computed\(\s*\(\)\s*=>\s*([^)]+)\)/g, (match, name, expression) => {
            result.warnings.push({
        ...
        transformed = transformed.replace(/effect\(\s*\(\)\s*=>\s*\{/g, (match) => {
            result.warnings.push({
    ...
    transformComponent(code, result) {
        let transformed = code;
        ...
        if (componentMatch) {
            ...
            transformed = transformed.replace(/@Component\(\{[\s\S]*?\}\)\s*export\s+class\s+(\w+)[^{]*\{/, (match, name) => {
    ...
    transformDependencyInjection(code, result) {
        let transformed = code;
        // inject() → useContext()
        transformed = transformed.replace(/private\s+readonly\s+(\w+)\s*=\s*inject\((\w+)\)/g, (match, varName, serviceName) => {
            result.manualReview.push({
                line: this.getLineNumber(code, match),
                type: 'inject',
        ...
        transformed = transformed.replace(/constructor\s*\(\s*private\s+(\w+):\s*(\w+)\s*\)/g, (match, varName, typeName) => {
            result.manualReview.push({
                line: this.getLineNumber(code, match),
        ...
        transformed = transformed.replace(/@Injectable\(\{[\s\S]*?\}\)\s*export\s+class\s+(\w+)/g, (match, serviceName) => {
            result.manualReview.push({
                line: this.getLineNumber(code, match),
                type: 'injectable',
    ...
    transformRxJS(code, result) {
        let transformed = code;
        // BehaviorSubject → signal
        transformed = transformed.replace(/new\s+BehaviorSubject(?:<[^>]+>)?\(([^)]+)\)/g, (match, initialValue) => {
            result.warnings.push({
        ...
        transformed = transformed.replace(/new\s+Subject(?:<[^>]+>)?\(\)/g, (match) => {
            result.manualReview.push({
                line: this.getLineNumber(code, match),
                type: 'subject',
        ...
        // .subscribe() → effect()
        transformed = transformed.replace(/\.subscribe\(\s*\(([^)]*)\)\s*=>\s*\{/g, (match, params) => {
            result.manualReview.push({
                line: this.getLineNumber(code, match),
                type: 'subscribe',
    ...
    transformLifecycle(code, result) {
        let transformed = code;
        // ngOnInit → onMount
        transformed = transformed.replace(/ngOnInit\s*\(\)\s*\{/g, (match) => {
            result.warnings.push({
        ...
        transformed = transformed.replace(/ngOnDestroy\s*\(\)\s*\{/g, (match) => {
            result.warnings.push({
        ...
        transformed = transformed.replace(/ngAfterViewInit\s*\(\)\s*\{/g, (match) => {
            result.warnings.push({
    ...
    transformTemplateSyntax(code, result) {
        let transformed = code;
