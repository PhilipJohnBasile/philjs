...
export class DioxusTransform {
    ...
    transformImports(code, result) {
        ...
        // Main dioxus import
        transformed = transformed.replace(/use\s+dioxus::prelude::\*;/g, 'use philjs::prelude::*;');
        transformed = transformed.replace(/use\s+dioxus::\{([^}]+)\};/g, (match, imports) => {
            const mapping = {
                'prelude::*': 'prelude::*',
                'Element': 'Element',
                'Props': '',
        ...
        // Dioxus router
        transformed = transformed.replace(/use\s+dioxus_router::\{([^}]+)\};/g, (match, imports) => {
    ...
    transformHooks(code, result) {
        let transformed = code;
        // use_state → Signal
        transformed = transformed.replace(/let\s+(\w+)\s*=\s*use_state\(\s*(?:cx,\s*)?\|\|\s*([^)]+)\)/g, (match, name, initialValue) => {
            result.warnings.push({
        ...
        transformed = transformed.replace(/let\s+(\w+)\s*=\s*use_state\(&cx,\s*\|\|\s*([^)]+)\)/g, (match, name, initialValue) => {
            return `let ${name} = Signal::new(${initialValue})`;
        });
        // use_ref → Signal (for mutable references)
        transformed = transformed.replace(/let\s+(\w+)\s*=\s*use_ref\(\s*(?:cx,\s*)?\|\|\s*([^)]+)\)/g, (match, name, initialValue) => {
            result.warnings.push({
        ...
        // use_memo → Memo
        transformed = transformed.replace(/let\s+(\w+)\s*=\s*use_memo\(\s*(?:cx,\s*)?\|[^|]*\|\s*([^)]+)\)/g, (match, name, expression) => {
            return `let ${name} = Memo::new(|| ${expression})`;
        });
        // use_effect → Effect
        transformed = transformed.replace(/use_effect\(\s*(?:cx,\s*)?\|[^|]*\|\s*\{/g, (match) => {
            result.warnings.push({
        ...
        transformed = transformed.replace(/let\s+(\w+)\s*=\s*use_coroutine\(/g, (match, name) => {
            result.manualReview.push({
        ...
        // use_future → Resource
        transformed = transformed.replace(/let\s+(\w+)\s*=\s*use_future\(\s*(?:cx,\s*)?\|\|/g, (match, name) => {
            result.warnings.push({
    ...
    transformRsx(code, result) {
        let transformed = code;
        // rsx! → view!
        // The syntax is similar but has some differences
        transformed = transformed.replace(/rsx!\s*\{/g, (match) => {
            result.warnings.push({
    ...
    transformComponents(code, result) {
        let transformed = code;
        // Dioxus component with Scope
        transformed = transformed.replace(/pub\s+fn\s+(\w+)\(\s*cx:\s*Scope(?:<[^>]*>)?\s*\)\s*->\s*Element/g, (match, name) => {
            result.warnings.push({
        ...
        transformed = transformed.replace(/pub\s+fn\s+(\w+)\(\s*cx:\s*Scope(?:<[^>]*>)?,\s*([^)]+)\)\s*->\s*Element/g, (match, name, props) => {
    ...
    transformContext(code, result) {
        let transformed = code;
        // use_context_provider → provide_context
        transformed = transformed.replace(/use_context_provider\(\s*(?:cx,\s*)?\|\|\s*([^)]+)\)/g, 'provide_context($1)');
        // use_context → use_context (same in PhilJS, just remove cx)
        transformed = transformed.replace(/use_context::<([^>]+)>\(\s*cx\s*\)/g, 'use_context::<$1>()');
        return transformed;
    }
    handlePlatformSpecific(code, result) {
        // Desktop-specific
...
//# sourceMappingURL=dioxus.js.map