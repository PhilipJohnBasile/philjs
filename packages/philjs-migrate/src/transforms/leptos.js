...
export class LeptosTransform {
    ...
    transformImports(code, result) {
        ...
        // Main leptos import
        transformed = transformed.replace(/use\s+leptos::\*;/g, 'use philjs::prelude::*;');
        transformed = transformed.replace(/use\s+leptos::\{([^}]+)\};/g, (match, imports) => {
        ...
        // Leptos router
        transformed = transformed.replace(/use\s+leptos_router::\{([^}]+)\};/g, (match, imports) => {
            result.warnings.push({
    ...
    transformSignalAccess(code, result) {
        let transformed = code;
    ...
    transformContext(code, result) {
        ...
        transformed = transformed.replace(/provide_context\(\s*cx,\s*/g, 'provide_context(');
...
//# sourceMappingURL=leptos.js.map