...
export class SvelteTransform {
    ...
    async transformSvelteComponent(code, result) {
...
`;
    ...
    transformScript(script, result) {
        let transformed = script;
        // Remove Svelte imports
        transformed = transformed.replace(/import\s+\{[^}]*\}\s+from\s+['"]svelte['"]\s*;?\n?/g, '');
        // Transform reactive declarations (let x = value)
        // In Svelte, top-level let declarations are reactive
        transformed = transformed.replace(/let\s+(\w+)\s*=\s*([^;]+);/g, (match, name, value) => {
        ...
        transformed = transformed.replace(/\$:\s*(\w+)\s*=\s*([^;]+);/g, (match, name, expression) => {
        ...
        transformed = transformed.replace(/\$:\s*([^;]+);/g, (match, statement) => {
            return `effect(() => {\n    ${statement.trim()};\n  });`;
        });
        // Transform $: blocks
        transformed = transformed.replace(/\$:\s*\{([\s\S]*?)\}/g, (match, body) => {
            return `effect(() => {\n${body}\n  });`;
        });
        // Transform lifecycle
...
${props.map(p => `  ${p.name}${p.defaultValue ? '?' : ''}: ${p.type || 'any'};`).join('\n')}
    ...
    templateToJSX(template, result) {
        ...
        jsx = jsx.replace(/\{#each\s+(\w+)\s+as\s+(\w+)(?:,\s*(\w+))?\s*(?:\(([^)]+)\))?\}/g, (match, array, item, index, key) => {
        ...
        jsx = jsx.replace(/bind:value=\{(\w+)\}/g, 'value={$1.get()} onInput={(e) => $1.set(e.target.value)}');
        // Svelte bind:checked → checked + onChange
        jsx = jsx.replace(/bind:checked=\{(\w+)\}/g, 'checked={$1.get()} onChange={(e) => $1.set(e.target.checked)}');
        // Svelte on:event → onEvent
        jsx = jsx.replace(/on:(\w+)=/g, (match, event) => {
            return `on${event.charAt(0).toUpperCase() + event.slice(1)}=`;
        });
        // Svelte class:name → conditional className
        jsx = jsx.replace(/class:(\w+)=\{([^}]+)\}/g, (match, className, condition) => {
            result.warnings.push({
