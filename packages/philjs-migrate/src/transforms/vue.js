...
export class VueTransform {
    ...
    async transformVueSFC(code, result) {
...
`;
    ...
    transformCompositionAPI(code, result) {
        let transformed = code;
        // Transform imports
        transformed = this.transformImports(transformed, result);
        // Transform ref
        transformed = transformed.replace(/const\s+(\w+)\s*=\s*ref(?:<[^>]+>)?\(([^)]*)\)/g, (match, name, initialValue) => {
            result.warnings.push({
        ...
        // Transform reactive
        transformed = transformed.replace(/const\s+(\w+)\s*=\s*reactive(?:<[^>]+>)?\(([^)]*)\)/g, (match, name, initialValue) => {
            result.warnings.push({
        ...
        // Transform computed
        transformed = transformed.replace(/const\s+(\w+)\s*=\s*computed\(\s*\(\)\s*=>\s*([^)]+)\)/g, 'const $1 = computed(() => $2)');
        // Transform watch to effect
        transformed = transformed.replace(/watch\(\s*(\w+),\s*\(([^)]*)\)\s*=>\s*\{([\s\S]*?)\}\s*\)/g, (match, watched, params, body) => {
            result.warnings.push({
    ...
    transformImports(code, result) {
        return code.replace(/import\s+\{([^}]+)\}\s+from\s+['"]vue['"]/g, (match, imports) => {
    ...
    transformVueScript(script, result) {
        // Remove setup() wrapper, keep content
        let transformed = script.replace(/export\s+default\s+defineComponent\(\{[\s\S]*?setup\(\)\s*\{([\s\S]*?)\s*return\s*\{[\s\S]*?\}[\s\S]*?\}\s*\}\)/, '$1');
        // Transform Vue composition API
        transformed = this.transformImports(transformed, result);
        return transformed;
    }
    extractSetupBody(script, result) {
