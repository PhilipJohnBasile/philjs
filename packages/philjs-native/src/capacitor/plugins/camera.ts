 ...
 */
export const cameraPermission: Signal<CameraPermissions> = signal({
  camera: 'prompt',
  photos: 'prompt',
});
 ...
 * Last captured photo
 */
export const lastPhoto: Signal<CameraPhoto | null> = signal(null);
 ...
 */
const WebCamera = {
  async getPhoto(options: CameraPhotoOptions): Promise<CameraPhoto> {
  ...
  async checkPermissions(): Promise<CameraPermissions> {
    ...
    try {
      ...
      return {
        camera: result.state as CameraPermissionState,
...
export const CapacitorCamera = {
   ...
   */
  async getPhoto(options: CameraPhotoOptions = {}): Promise<CameraPhoto> {
    const defaultOptions: CameraPhotoOptions = {
      quality: 90,
      allowEditing: false,
      resultType: 'uri',
      source: 'prompt',
    ...
    try {
      const photo = await callPlugin<CameraPhotoOptions, CameraPhoto>(
        'Camera',
        'getPhoto',
        mergedOptions
      );
      ...
      throw new Error(`Camera error: ${(error as Error).message}`);
    }
  },
  ...
  async pickImages(options: {
    ...
    try {
      const result = await callPlugin<typeof options, { photos: CameraPhoto[] }>(
        'Camera',
        'pickImages',
        options
      );
      ...
      throw new Error(`Pick images error: ${(error as Error).message}`);
    }
  },
  ...
  async checkPermissions(): Promise<CameraPermissions> {
    ...
    try {
      const permissions = await callPlugin<never, CameraPermissions>(
        'Camera',
        'checkPermissions'
      );
      cameraPermission.set(permissions);
  ...
  async requestPermissions(
    permissions?: { permissions: Array<'camera' | 'photos'> }
    ...
    try {
      const result = await callPlugin<typeof permissions, CameraPermissions>(
        'Camera',
        'requestPermissions',
        permissions
      );
   ...
   */
  async pickVideo(): Promise<{ path: string; webPath: string; duration?: number }> {
    if (!isNativePlatform()) {
      return new Promise((resolve, reject) => {
        const input = document.createElement('input');
        input.type = 'file';
    ...
    try {
      ...
      throw new Error(`Pick video error: ${(error as Error).message}`);
    }
  },
};
 ...
 */
export function useLastPhoto(): CameraPhoto | null {
  return lastPhoto();
}
