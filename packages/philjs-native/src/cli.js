 ...
 */
function validateSimulatorName(name) {
...
async function main() {
    switch (command) {
        case 'init':
            ...
            break;
        case 'run':
            await runApp(args[1], parseOptions(args.slice(2)));
            break;
        case 'build':
...
async function initProject(name) {
    ...
    writeFileSync(join(projectDir, 'philjs.config.json'), JSON.stringify(config, null, 2));
    // Create package.json
    const packageJson = {
        name: projectName,
        version: '1.0.0',
 ...
 */
async function runApp(platform, options) {
    if (!platform) {
 ...
 * Run on iOS
 */
async function runIOS(config, options) {
    // Check for macOS
    ...
    const simulator = options['simulator'] || 'iPhone 15';
 ...
 * Run on Android
 */
async function runAndroid(config, options) {
 ...
 * Run on web
 */
async function runWeb(config, options) {
    // Validate port
...
async function buildIOS(config, outDir) {
    if (process.platform !== 'darwin') {
...
async function buildAndroid(config, outDir) {
    // Validate output directory
    if (!validatePath(outDir)) {
        console.error('  Error: Invalid output directory');
        process.exit(1);
...
async function buildWeb(config, outDir) {
    // Validate output directory
    if (!validatePath(outDir)) {
        console.error('  Error: Invalid output directory');
        process.exit(1);
...
async function initIOSProject(config) {
    const iosDir = join(process.cwd(), 'ios');
