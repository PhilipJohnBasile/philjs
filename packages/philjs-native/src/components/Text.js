    ...
    if (platform === 'web') {
        ...
        return {
            ...
            props: {
                ...
                onContextMenu: props.onLongPress ? (e) => {
                    e.preventDefault();
                    props.onLongPress();
                } : undefined,
 ...
 */
function convertTextStyle(style, platform) {
    const result = {};
    ...
    if (platform === 'web' && style.textShadowColor) {
        const offsetX = style.textShadowOffset?.width || 0;
        const offsetY = style.textShadowOffset?.height || 0;
 ...
 */
function convertTextValue(key, value) {
