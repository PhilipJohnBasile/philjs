...
export interface ScreenOptions {
  ...
  drawerLabel?: string;
  drawerIcon?: (props: { focused: boolean; color: string; size: number }) => any;
}
 ...
 * Event listeners
 */
const eventListeners = new Map<NavigationEvent, Set<(data: any) => void>>();
 ...
 */
export function createStackNavigator(options?: StackNavigatorOptions) {
  const screens: Screen[] = [];
  const stackState = signal<NavigationState | null>(null);

  /**
   * Register a screen
   */
  function Screen<T extends RouteParams>(config: Screen<T>): void {
    screens.push(config as Screen);
  }
   ...
   * Navigator component
   */
  function Navigator(props: { children?: any }): any {
 ...
 */
function renderWebStack(
  state: NavigationState,
  screen: Screen,
  currentRoute: Route,
  navigation: Navigation,
  screenOptions: ScreenOptions,
  navOptions?: StackNavigatorOptions
): any {
  return {
    type: 'div',
    props: {
 ...
 */
export function createTabNavigator(options?: TabNavigatorOptions) {
  const screens: Screen[] = [];
  const activeTab = signal(0);
  const tabState = signal<NavigationState | null>(null);
 ...
 */
export function createDrawerNavigator(options?: DrawerNavigatorOptions) {
  const screens: Screen[] = [];
  const isOpen = signal(false);
  const activeScreen = signal(0);
  const drawerState = signal<NavigationState | null>(null);
...
function emitEvent(event: NavigationEvent, data: any): void {
 ...
 */
export function useRoute<T extends RouteParams = RouteParams>(): Route<T> {
  const state = navigationState();
  if (!state) return { key: '', name: '', params: {} as T };
  return state.routes[state.index] as Route<T>;
}
