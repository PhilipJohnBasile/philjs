 ...
 */
export const memoryPressure: Signal<'normal' | 'moderate' | 'critical'> = signal<'normal' | 'moderate' | 'critical'>('normal');

/**
 * Start memory monitoring
 */
export function startMemoryMonitor(interval = 5000): () => void {
  let timer: any = null;
 ...
 */
export function createVirtualizedList(config: VirtualizationConfig): {
  range: Signal<VirtualizedRange>;
  updateScroll: (scrollTop: number) => void;
  getItemStyle: (index: number) => { position: string; top: number; height: number };
} {
 ...
 */
export function preload<T>(loader: () => Promise<T>): LazyModule<T> {
...
const interactionQueue: Array<() => void> = [];
