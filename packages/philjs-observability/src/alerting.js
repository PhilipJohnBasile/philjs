...
export class AlertManager {
    ...
    async sendEmailNotification(channel, alert) {
        ...
        const from = channel.config['from'] ?? 'alerts@philjs.dev';
        if (!endpoint) {
        ...
        const text = channel.config['text']
            ?? `${alert.message}\nSeverity: ${alert.severity}\nValue: ${alert.value}\nThreshold: ${alert.threshold}`;
        const html = channel.config['html']
        ...
        if (apiKey) {
            const headerName = channel.config['authHeader'] ?? 'Authorization';
            const prefix = channel.config['authPrefix'] ?? 'Bearer';
