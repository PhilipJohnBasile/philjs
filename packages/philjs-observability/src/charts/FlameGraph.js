 ...
 */
export function spansToFlameGraph(spans) {
...
function getColor(depth, scheme) {
    switch (scheme) {
        case 'warm':
...
export function FlameGraph(props) {
    const { root, width = 800, height = 400, cellHeight = 20, colorScheme = 'warm', showLabels = true, minWidth = 5, className = '', } = props;
    const rects = [];
    function renderNode(node, x, y, nodeWidth, depth) {
