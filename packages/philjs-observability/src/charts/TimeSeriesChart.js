...
export function TimeSeriesChart(props) {
    const { series, width = 600, height = 300, showLegend = true, showGrid = true, xAxisLabel, yAxisLabel, className = '', } = props;
    // Calculate bounds
    ...
    const scaleX = (timestamp) => padding.left + ((timestamp - minTime) / (maxTime - minTime)) * chartWidth;
    ...
    const legend = showLegend
        ? series
            .map((s, i) => `<g transform="translate(${padding.left + i * 100}, ${height - 10})">
