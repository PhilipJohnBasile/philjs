    ...
    while (i < args.length) {
        ...
        switch (arg) {
            ...
            case '--prefix': {
                const val = args[++i];
                if (val !== undefined)
                    result.typePrefix = val;
                break;
            }
            case '--suffix': {
                const val = args[++i];
                if (val !== undefined)
                    result.typeSuffix = val;
                break;
            }
            case '--enums-as-union':
 ...
 */
function jsonSchemaToTS(schema, options) {
 ...
 */
function formatTypeName(name, options) {
 ...
 */
export function generateTypes(spec, options) {
    const lines = [];
    const zodLines = [];
    const clientLines = [];
    // Header
    ...
    if (options.generateClient) {
        ...
        for (const { operationId, method, path, operation } of operations) {
            ...
            if (hasParams)
                clientLines.push('        params: req.params,');
            if (hasQuery)
                clientLines.push('        query: req.query,');
            if (hasBody)
                clientLines.push('        body: req.body,');
            clientLines.push('      });');
            clientLines.push('    },');
        }
        clientLines.push('  };');
 ...
 */
function generateZodSchema(name, schema, options) {
    ...
    switch (parsed.command) {
        ...
        case 'generate':
            ...
            try {
                ...
                if (parsed.generateClient !== undefined)
                    options.generateClient = parsed.generateClient;
                if (parsed.generateZod !== undefined)
                    options.generateZod = parsed.generateZod;
                if (parsed.typePrefix !== undefined)
                    options.typePrefix = parsed.typePrefix;
                if (parsed.typeSuffix !== undefined)
                    options.typeSuffix = parsed.typeSuffix;
                if (parsed.enumsAsUnion !== undefined)
                    options.enumsAsUnion = parsed.enumsAsUnion;
                if (parsed.readonlyTypes !== undefined)
                    options.readonlyTypes = parsed.readonlyTypes;
