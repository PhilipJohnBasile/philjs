...
function generateSwaggerHTML(options) {
    const { spec, specUrl, title = 'API Documentation', favicon = 'https://unpkg.com/swagger-ui-dist@5.10.3/favicon-32x32.png', customCss = '', customJs = '', config = {}, } = options;
 ...
 * ```
 */
export function createSwaggerRoutes(spec, options = {}) {
    return {
        docs: swaggerUI({ ...options, spec }),
        spec: () => new Response(JSON.stringify(spec, null, 2), {
            headers: {
 ...
 * ```
 */
export function specHandler(spec) {
...
function generateReDocHTML(options) {
    const { spec, specUrl, title = 'API Documentation', favicon = 'https://redocly.github.io/redoc/favicon.png', customCss = '', config = {}, } = options;
    const specSource = spec
        ? typeof spec === 'string'
            ? `spec-url="${spec}"`
            : `spec='${JSON.stringify(spec)}'`
 ...
 * ```
 */
export function createDocsRoutes(spec, options = {}) {
