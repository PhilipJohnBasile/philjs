 ...
 */
import { findCommonDependencies, getAllDependencies, calculateCohesion, groupByDepth, } from './dependency-graph.js';
/**
...
export const hybridStrategy = {
    ...
    bundle: (graph, options) => {
        ...
        const lazySymbols = Array.from(graph.symbols.values()).filter((s) => s.isLazy);
        for (const symbol of lazySymbols) {
            chunks.set(`lazy_${symbol.id}`, [symbol]);
        }
        // 2. ES2024: Group non-lazy symbols by route using Map.groupBy()
        const regularSymbols = Array.from(graph.symbols.values()).filter((s) => !s.isLazy);
 ...
 */
function findRelatedSymbols(graph, symbolId) {
 ...
 */
export function bundleSymbols(graph, options, strategyName = 'hybrid') {
