...
export function buildDependencyGraph(symbols) {
    ...
    for (const symbol of symbols) {
        for (const depName of symbol.dependencies) {
            ...
            const depSymbol = Array.from(graph.symbols.values()).find((s) => s.name === depName);
            if (depSymbol) {
                // Add dependency edge
 ...
 */
export function getAllDependencies(graph, symbolId, visited = new Set()) {
 ...
 */
export function getAllDependents(graph, symbolId, visited = new Set()) {
    if (visited.has(symbolId)) {
        return visited;
    }
    visited.add(symbolId);
...
export function topologicalSort(graph) {
    ...
    function visit(symbolId) {
        if (tempMarked.has(symbolId)) {
            throw new Error(`Circular dependency detected involving ${symbolId}`);
        }
        if (visited.has(symbolId)) {
            return;
        }
 ...
 */
export function findLeafNodes(graph) {
    const leafNodes = [];
 ...
 */
export function calculateDepth(graph, symbolId) {
    const deps = graph.dependencies.get(symbolId);
    if (!deps || deps.size === 0) {
        return 0;
    }
 ...
 */
export function groupByDepth(graph) {
 ...
 */
export function findCommonDependencies(graph, symbolIds) {
    if (symbolIds.length === 0)
        return new Set();
    const allDeps = symbolIds.map((id) => getAllDependencies(graph, id, new Set()));
    ...
    for (let i = 1; i < allDeps.length; i++) {
        ...
        if (deps) {
            common = new Set(Array.from(common).filter((id) => deps.has(id)));
        }
    }
    return common;
}
 ...
 */
export function calculateCohesion(graph, symbolIds) {
