...
export function transform(
  ...
  const lazyHandlerReplacements = new Map<
    t.CallExpression,
    { symbolId: string; modulePath: string }
  >();
 ...
 */
export function extractLazyChunks(
  source: string,
  filePath: string
): Map<string, string> {
    ...
    CallExpression(path) {
      if (
        ...
        if (path.node.callee.name === '$$' && args.length >= 2) {
          ...
          handlerNode = args[0] as t.Expression;
        } else {
          return;
        }

        // Generate a module for this handler
        const chunkAst = t.file(
          t.program([
 ...
 */
export function generateLazyImports(
  symbolIds: string[],
  baseUrl = '/lazy'
): string {
 ...
 */
export function injectHandlerRegistrations(
  source: string,
  ...
  const lastImportIndex = program.body.findIndex(
    (node, idx) =>
      !t.isImportDeclaration(node) &&
...
export function generateManifest(
  symbols: Map<string, { modulePath: string; dependencies: string[] }>
): string {
  const manifest = {
