 ...
 * Vite plugin for PhilJS optimizer
 */
export function philjsOptimizer(options = {}) {
    let config;
    let isBuild = false;
    ...
    const { strategy = 'hybrid', include = ['**/*.tsx', '**/*.ts', '**/*.jsx', '**/*.js'], exclude = ['node_modules/**', '**/*.test.*', '**/*.spec.*'], sourcemap = true, baseUrl = '/lazy', ...optimizerOptions } = options;
    return {
 ...
 */
function shouldInclude(id, patterns) {
 ...
 */
function shouldExclude(id, patterns) {
    const patternsArray = Array.isArray(patterns) ? patterns : [patterns];
    return patternsArray.some((pattern) => {
        const regex = new RegExp(pattern.replace(/\*\*/g, '.*').replace(/\*/g, '[^/]*'));
        return regex.test(id);
 ...
 */
function generateChunkCode(symbols) {
