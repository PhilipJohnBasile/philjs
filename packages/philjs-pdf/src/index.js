...
export class PDFGenerator {
     ...
     * ```
     */
    async generateFromHtml(htmlOrUrl, options = {}) {
     ...
     * ```
     */
    async generateFromComponent(Component, props = {}, options = {}) {
     ...
     */
    async generateBlobFromComponent(Component, props = {}) {
        const { pdf } = await import('@react-pdf/renderer');
        // @ts-expect-error - React types not available, using dynamic import
        const React = await import('react');
        const element = React.createElement(Component, props);
     ...
     * ```
     */
    async addWatermark(pdfBytes, options) {
        const pdfDoc = await PDFDocument.load(pdfBytes);
        const pages = pdfDoc.getPages();
        const font = await pdfDoc.embedFont(StandardFonts.HelveticaBold);
        const { text, fontSize = 50, rotation = -45, color = [0.75, 0.75, 0.75], opacity = 0.3, position = 'center', } = options;
     ...
     * ```
     */
    async protect(pdfBytes, options) {
        const pdfDoc = await PDFDocument.load(pdfBytes);
        ...
        if (this.options.debug) {
            console.warn('PDF protection with pdf-lib is limited. Consider using PDFKit for full encryption.');
        }
        return pdfDoc.save();
    }
    /**
     ...
     * ```
     */
    async compress(pdfBytes, options = {}) {
        const pdfDoc = await PDFDocument.load(pdfBytes);
 ...
 */
export async function mergePdfs(pdfs, options) {
    const generator = new PDFGenerator();
