...
export interface PerformanceBudget {
  ...
  // Runtime budgets
  maxMemory?: number; // MB
  maxLongTasks?: number; // count
  maxLayoutShifts?: number; // count
  maxNetworkRequests?: number; // count
  maxDOMNodes?: number; // count
...
export interface BudgetConfig {
  ...
  warningThreshold?: number; // Percentage of budget to trigger warning (e.g., 0.8 = 80%)
  errorThreshold?: number; // Percentage of budget to trigger error (e.g., 1.0 = 100%)
...
export class BudgetChecker {
  ...
  private async reportViolations(violations: BudgetViolation[]): Promise<void> {
    try {
  ...
  private calculateWebVitalsScore(metrics: PerformanceMetrics): number {
    ...
    if (metrics.lcp !== undefined) {
      ...
      else totalScore += 50 * (4000 / metrics.lcp);
      checks++;
    }
    ...
    if (metrics.fid !== undefined) {
      ...
      else totalScore += 50 * (300 / metrics.fid);
      checks++;
    }
    ...
    if (metrics.cls !== undefined) {
      ...
      else totalScore += Math.max(0, 50 * (0.25 / metrics.cls));
      checks++;
    }
    ...
    if (metrics.ttfb !== undefined) {
      ...
      else totalScore += 50 * (1800 / metrics.ttfb);
      checks++;
    }
    ...
    if (metrics.fcp !== undefined) {
      ...
      else totalScore += 50 * (3000 / metrics.fcp);
      checks++;
    }
  ...
  private penalize(actual: number, budget: number): number {
    const ratio = actual / budget;
    if (ratio <= 1) return 0;
...
export class BuildBudgetChecker {
  ...
  check(): { passed: boolean; violations: BudgetViolation[]; summary: string } {
    const violations: BudgetViolation[] = [];
  ...
  private generateSummary(violations: BudgetViolation[]): string {
  ...
  getArtifactReport(): string {
    const lines: string[] = ['Build Artifacts:'];
...
let globalChecker: BudgetChecker | null = null;
let globalObserver: PerformanceObserverManager | null = null;

/**
 * Hook for performance budgets
 */
export function usePerformanceBudget(config: BudgetConfig): {
  ...
  const [getMetrics, setMetrics] = createState<PerformanceMetrics>({});
  const [getScore, setScore] = createState<PerformanceScore | null>(null);
...
export function usePerformanceMetric(name: string): {
  ...
  const [getValue, setValue] = createState<number | undefined>(undefined);
...
export function perfBudgetPlugin(budget: PerformanceBudget): any {
  ...
  return {
    ...
    generateBundle(_options: any, bundle: any) {
      for (const [fileName, chunk] of Object.entries(bundle)) {
        ...
        else if (/\.(png|jpg|jpeg|gif|svg|webp)$/i.test(fileName)) type = 'image';
        else if (/\.(woff|woff2|ttf|eot)$/i.test(fileName)) type = 'font';
