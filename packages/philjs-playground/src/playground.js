 ...
 */
export function createPlayground(container, config = {}) {
    const { initialCode = exampleCode.helloWorld, theme = 'dark', layout = 'horizontal', autoRun = true, showConsole = true, showShare = true, onCompile, onError, } = config;
    ...
    // Run button
    const runButton = header.querySelector('.philjs-playground__run');
    runButton?.addEventListener('click', runCode);
    // Share button
    const shareButton = header.querySelector('.philjs-playground__share');
        ...
        try {
            ...
            if (result.success) {
                ...
                preview.render(result.output, {
                    onConsole: (type, ...args) => {
                        consoleView.log(type, args.map(String).join(' '));
                    },
                    onError: (error) => {
...
function createHeader(showShare) {
...
function injectStyles() {
    if (document.getElementById('philjs-playground-styles'))
        return;
