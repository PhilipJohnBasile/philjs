 ...
 */
function _generateClientInit(config) {
 ...
 */
function getTrackingCall(provider, eventName, properties) {
    switch (provider) {
        case "google-analytics":
        case "ga4":
 ...
 */
export function createAnalyticsPlugin(userConfig) {
            ...
            try {
                ...
                if (hasIndex) {
                    ...
                    if (!indexContent.includes(mergedConfig.trackingId)) {
                        const updatedContent = indexContent.replace("</head>", `  ${script}\n  </head>`);
