 ...
 */
export const currentLocale = signal<string>('en');

/**
 * Translations store
 */
const translations = signal<Record<string, TranslationMap>>({});

/**
 * Locale configs store
 */
const localeConfigs = signal<Map<string, LocaleConfig>>(new Map());
...
export function initI18n(config: {
  ...
  const configs = new Map<string, LocaleConfig>();
 ...
 */
export function setLocale(locale: string, options?: { persist?: boolean; storageKey?: string }): void {
...
export function t(
  ...
  options?: { locale?: string; count?: number }
): string {
  ...
  if (typeof value !== 'string') {
    ...
    if (typeof value === 'object' && options?.count !== undefined) {
      const pluralValue = resolvePlural(value as PluralRules, options.count, locale);
      if (pluralValue) {
        value = pluralValue;
      } else {
        return key;
  ...
  if (params) {
    return interpolate(value as string, { ...params, count: options?.count });
  }
 ...
 */
function resolvePlural(forms: PluralRules, count: number, locale: string): string | null {
 ...
 */
export function getI18nContext(): I18nContextValue {
  return {
    get locale() {
