...
function createVitePlugin(options) {
    const { translationsDir, virtualModuleId = 'virtual:philjs-i18n', generateTypes = true, typesOutputPath = './src/i18n.d.ts', } = options;
 ...
 */
async function generateTypesFile(root, translationsDir, outputPath) {
 ...
 */
function generateTypeFromTranslations(obj, prefix = '') {
    const lines = [];
    if (prefix === '') {
    ...
    for (const [key, value] of Object.entries(obj)) {
        ...
        if (typeof value === 'string') {
            ...
            const nested = generateTypeFromTranslations(value, fullKey);
            lines.push(nested);
        }
    }
    if (prefix === '') {
 ...
 * Create i18n plugin
 */
export function createI18nPlugin(userConfig) {
        ...
        async setup(pluginConfig, ctx) {
            ...
            try {
                ...
                if (!exists) {
                    ...
                    await ctx.fs.writeFile(`${localesDir}/${mergedConfig.defaultLocale}.json`, JSON.stringify(defaultTranslations, null, 2));
            ...
            const localeArray = mergedConfig.locales.map(l => typeof l === 'string' ? `'${l}'` : JSON.stringify(l));
            const clientCode = `/**
 * i18n initialization
