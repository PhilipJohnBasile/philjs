 ...
 */
export function generateRobotsContent(robots: RobotsDirectives): string {
 ...
 */
export function generateTwitterTags(twitter: TwitterTags): string[] {
  const tags: string[] = [];
 ...
 */
export function generateJsonLd(data: JsonLd | JsonLd[]): string {
 ...
 */
export function generateSEOHead(seo: PageSEO): string {
  const parts: string[] = [];
 ...
 */
export function mergeSEO(...configs: (PageSEO | undefined)[]): PageSEO {
 ...
 */
export function updateHead(seo: PageSEO): void {
  ...
  const existingMeta = head.querySelectorAll(
    'meta[data-philjs-seo], link[data-philjs-seo], script[data-philjs-seo]'
  );
