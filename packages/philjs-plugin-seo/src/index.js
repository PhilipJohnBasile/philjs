 ...
 */
function generateRobotsTxt(config, baseUrl) {
    const lines = [];
    // Rules
...
function createVitePlugin(config) {
    ...
    return {
        ...
        generateBundle() {
            ...
            // Generate robots.txt
            if (config.robots) {
                const robotsConfig = typeof config.robots === 'object'
                    ? config.robots
                    : {
        ...
        transformIndexHtml(html) {
            ...
            if (config.defaults || config.openGraph || config.twitter || config.jsonLd) {
                const { generateSEOHead, } = require('./head.js');
 ...
 * Create SEO plugin
 */
export function createSEOPlugin(userConfig = {}) {
