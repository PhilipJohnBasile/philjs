 ...
 */
const defaultConfig: TailwindPluginConfig = {
  jit: true,
  darkMode: "class",
 ...
 */
const tailwindConfigTemplate = (config: TailwindPluginConfig) => `/** @type {import('tailwindcss').Config} */
export default {
 ...
 */
export function createTailwindPlugin(userConfig: TailwindPluginConfig = {}): Plugin {
  ...
  return {
    ...
    async setup(pluginConfig: TailwindPluginConfig, ctx: PluginContext) {
      ...
      if (!hasTailwindConfig) {
        ...
        await ctx.fs.writeFile(
          "tailwind.config.js",
          tailwindConfigTemplate(mergedConfig)
        );
      } else {
      ...
      async buildEnd(ctx, result) {
        if (result.success) {
          ...
          if (cssOutputs.length > 0) {
            ...
            ctx.logger.success(
              `Tailwind CSS processed: ${(totalSize / 1024).toFixed(2)} KB`
            );
          }
        }
      },
...
export const tailwindUtils = {
   ...
   */
  async detectContentPaths(rootDir: string): Promise<string[]> {
   ...
   */
  withStates(base: string, states: string[] = ["hover", "focus", "active"]): string {
    const classes = [base];
