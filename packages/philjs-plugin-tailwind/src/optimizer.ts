...
export class CSSOptimizer {
   ...
   */
  private deduplicateRules(css: string): string {
  ...
  private mergeMediaQueries(css: string): string {
    ...
    if (mediaQueries.size === 0) return css;
   ...
   */
  private mergeProperties(existing: string, incoming: string): string {
...
export class CriticalCSSExtractor {
   ...
   */
  extractCritical(css: string, html: string): { critical: string; deferred: string } {
    ...
    for (const sel of selectors) {
      ...
      if (classMatch) {
        for (const cls of classMatch) {
          if (classes.has(cls.substring(1))) return true;
        }
      }
      ...
      if (idMatch) {
        for (const id of idMatch) {
          if (ids.has(id.substring(1))) return true;
        }
      }
 ...
 */
export function extractCriticalCSS(css: string, html: string): {
  critical: string;
  deferred: string;
} {
...
export function analyzeCSSStats(css: string): CSSStats {
  const ruleRegex = /([^{}]+)\{([^{}]+)\}/g;
  let ruleCount = 0;
  let selectorCount = 0;
