...
export class ManifestGenerator {
  ...
  private generateDefaultIcons(): PWAIcon[] {
  ...
  private addIOSMeta(): void {
    ...
    if (this.config.icons) {
      const appleTouchIcon = this.config.icons.find(i =>
        i.sizes === '180x180' || i.sizes === '192x192'
      );
      if (appleTouchIcon) {
...
export class PWAManager {
  ...
  private swGenerator: ServiceWorkerGenerator;
...
let globalPWA: PWAManager | null = null;
...
export function usePWA(config: PWAConfig): {
  ...
  const [getState, setState] = createState<PWAState>(globalPWA.getState());
...
export function useOnlineStatus(): boolean {
  const [getOnline, setOnline] = createState(
...
export function pwaPlugin(config: PWAConfig): any {
  ...
  return {
    ...
    configureServer(server: any) {
      // Serve manifest
      server.middlewares.use('/manifest.json', (_req: any, res: any) => {
