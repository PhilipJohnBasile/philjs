...
export function generateQRCode(options) {
    const { data, style = {}, logo, errorCorrectionLevel = 'M', gradient, } = options;
    const { width = 200, height = 200, margin = 4, backgroundColor = '#ffffff', foregroundColor = '#000000', cornerRadius = 0, dotStyle = 'square', cornerStyle = 'square', } = style;
...
function calculateVersion(dataLength, ecLevel) {
...
function isOverlappingFinderPattern(row, col, size) {
    return (row < 9 && col < 9) ||
        (row < 9 && col >= size - 8) ||
        (row >= size - 8 && col < 9);
}
...
function generateGradientDef(gradient, id) {
...
function drawModule(x, y, size, fill, style, radius) {
 ...
 */
export async function generateQRCodeCanvas(options) {
    const svg = generateQRCode(options);
