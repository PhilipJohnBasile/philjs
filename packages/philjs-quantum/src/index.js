...
export class QuantumSimulator {
    state;
    maxQubits;
    constructor(numQubits, maxQubits = 16) {
    ...
    initializeState(numQubits) {
        ...
        const amplitudes = new Array(size);
    ...
    applySingleQubitGate(qubit, matrix) {
        ...
        for (let i = 0; i < size; i++) {
            ...
            if (bit === 0) {
                ...
                newAmplitudes[i] = Complex.add(Complex.mul(matrix[0][0], a0), Complex.mul(matrix[0][1], a1));
                newAmplitudes[j] = Complex.add(Complex.mul(matrix[1][0], a0), Complex.mul(matrix[1][1], a1));
            }
        }
    ...
    measure(qubit) {
        ...
        for (let i = 0; i < size; i++) {
            const bit = (i >> qubit) & 1;
            if (bit === result) {
                this.state.amplitudes[i] = Complex.scale(this.state.amplitudes[i], 1 / normFactor);
            }
            else {
...
export class QuantumAnnealingOptimizer {
     ...
     */
    optimize(costFunction, numVariables, options = {}) {
        const { maxIterations = 10000, initialTemp = 100, finalTemp = 0.001, coolingRate = 0.99 } = options;
...
export class QAOAOptimizer {
     ...
     */
    optimize(costHamiltonian, params) {
    ...
    applyCostLayer(hamiltonian, gamma) {
...
let globalRNG = null;
export function initQuantum(config) {
...
export function useQuantumSimulator(numQubits) {
    return new QuantumSimulator(numQubits, globalConfig.maxQubits);
}
export function useQuantumAnnealing() {
    return new QuantumAnnealingOptimizer();
}
export function useQAOA(numQubits) {
    return new QAOAOptimizer(numQubits);
}
// Utility hooks
export function useQuantumId(length = 16) {
...
export function useQuantumShuffle(array) {
    const rng = useQuantumRandom();
