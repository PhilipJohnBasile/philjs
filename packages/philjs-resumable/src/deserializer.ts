 ...
 * import { deserializeState, resumeFromDOM } from '@philjs/resumable';
 *
 * // Parse state from script tag
 * const state = deserializeState(document.getElementById('__PHIL_STATE__'));
 *
 * // Resume all serialized elements
 * await resumeFromDOM(document.body, state);
 * ```
 */

import type { SerializedValue, SerializedElement, SerializedSignal } from './serializer.js';
import { parseQRL, type QRL } from './qrl.js';
...
export interface DeserializedComponent {
  /** Component ID */
  id: string;
  /** Component QRL for lazy loading */
  qrl: QRL;
  /** Deserialized props */
 ...
 *
 * Handles all serializable types including primitives, objects, arrays,
 * Date, Map, Set, BigInt, RegExp, Error, signals, and QRLs.
 */
export function deserializeValue(
  ...
  switch (type) {
    ...
    case 'qrl': {
      // Parse the QRL string into a QRL object
      const qrlString = data as string;
      return parseQRL(qrlString);
    }
 ...
 *
 * Parses the JSON state embedded in the HTML and creates live objects
 * for signals, elements, and components.
 */
...
function deserializeElement(
  ...
  const handlers = new Map<string, QRL>();

  // Parse handler QRLs
  for (const handler of serialized.handlers) {
    handlers.set(handler.event, parseQRL(handler.qrl));
  }
 ...
 *
 * Finds the actual DOM elements for each serialized element/component
 * and updates the state with live references.
 */
 ...
 *
 * Connects deserialized signals to DOM elements so that
 * when a signal value changes, the DOM is updated.
 */
 ...
 *
 * This is the main entry point for client-side resumption.
 * It parses serialized state, resolves DOM references, and
 ...
 *
 * Used for inline state serialization in data-qstate attributes.
 */
...
/**
 * Resolve a signal reference to a live signal.
 *
 * If the signal exists in the state, returns the live signal object.
 * Otherwise creates a new signal with the provided initial value.
 */
export function resolveSignalRef<T>(
  ref: { $signalRef$: string; value: T },
  state: DeserializedState | null,
  signalFactory?: (initial: T) => unknown
): unknown {
  if (!state) {
    return signalFactory?.(ref.value) ?? ref.value;
  }

  const signal = state.signals.get(ref.$signalRef$);
  if (signal) {
    return signal;
  }
