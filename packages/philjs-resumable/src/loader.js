...
import { deserializeValue, } from './serializer.js';
// ============================================================================
// Loader State
// ============================================================================
const defaultConfig = {
 ...
 */
export function registerLazyComponent(id, loader) {
    const component = {
        id,
        loader,
        status: 'idle',
 ...
 */
export function registerLazyComponents(components) {
 ...
 */
export function getLazyComponent(id) {
    return componentRegistry.get(id);
}
/**
 * Check if a component is registered
 */
export function hasLazyComponent(id) {
 ...
 */
export function prefetchChunk(chunkPath) {
    if (chunkCache.has(chunkPath))
        return;
 ...
 */
export async function loadFromQRL(qrl) {
 ...
 * ```
 */
export async function loadAndHydrate(element, options) {
 ...
 */
async function hydrateComponent(element, Component, props) {
    ...
    try {
        ...
        element.dispatchEvent(new CustomEvent('phil:hydrated', {
            bubbles: true,
...
export function prefetchComponent(id) {
    const component = componentRegistry.get(id);
    if (!component || component.status !== 'idle')
        return;
 ...
 */
export function prefetchVisibleComponents() {
    if (typeof document === 'undefined')
        return;
 ...
 */
export async function loadAndInvokeHandler(qrl, event, element) {
    try {
 ...
 */
export function queueLoad(id, element, qrl) {
        ...
        document.addEventListener(eventType, async (event) => {
            ...
            while (element && element !== document.body) {
                ...
                if (qid && clientState?.elements[qid]) {
                    const elementState = clientState.elements[qid];
                    const handler = elementState.handlers.find((h) => h.event === eventType);
                    if (handler) {
 ...
 */
export async function waitForLoads() {
