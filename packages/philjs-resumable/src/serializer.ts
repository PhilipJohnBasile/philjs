...
export function serializeValue(value: unknown): SerializedValue {
  ...
  if (typeof value === 'object' && 'peek' in value && typeof (value as { peek: unknown }).peek === 'function') {
    const signalValue = (value as { peek: () => unknown }).peek();
    const signalId = '$id$' in value ? String((value as { $id$: string }).$id$) : generateId();
    return {
      type: 'signal',
      data: {
        id: signalId,
...
export function deserializeValue(serialized: SerializedValue): unknown {
  switch (serialized.type) {
    ...
    case 'bigint':
      return BigInt(serialized.data as string);

    case 'date':
      return new Date(serialized.data as string);

    case 'regexp': {
      const { source, flags } = serialized.data as { source: string; flags: string };
      return new RegExp(source, flags);
    }

    case 'error': {
      const { name, message, stack } = serialized.data as {
        name: string;
        message: string;
        stack?: string;
      };
    ...
    case 'map': {
      const entries = serialized.data as [SerializedValue, SerializedValue][];
      return new Map(entries.map(([k, v]) => [deserializeValue(k), deserializeValue(v)]));
    }

    case 'set': {
      const values = serialized.data as SerializedValue[];
    ...
    case 'object': {
      const entries = serialized.data as Record<string, SerializedValue>;
 ...
 */
export function deserializeFromAttribute(attr: string): unknown {
  try {
    const serialized = JSON.parse(atob(attr)) as SerializedValue;
