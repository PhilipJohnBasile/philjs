...
export class BlockRenderer {
  ...
  private createBlockElement(block: Block, editor: EditorInstance | null, readOnly: boolean): HTMLElement {
    switch (block.type) {
      case 'paragraph':
  ...
  private renderContent(content: TextNode[]): DocumentFragment {
  ...
  private createMarkElement(mark: { type: string; attrs?: Record<string, unknown> }, child: Node): HTMLElement {
    let el: HTMLElement;
  ...
  private createParagraph(block: Block): HTMLParagraphElement {
    const p = document.createElement('p');
    p.appendChild(this.renderContent(block.content as TextNode[]));
    return p;
  }

  private createHeading(block: Block, level: 1 | 2 | 3): HTMLHeadingElement {
    const h = document.createElement(`h${level}`) as HTMLHeadingElement;
    h.appendChild(this.renderContent(block.content as TextNode[]));
    return h;
  }

  private createList(block: Block, tag: 'ul' | 'ol', editor: EditorInstance | null, readOnly: boolean): HTMLElement {
  ...
  private createTodoList(block: Block, editor: EditorInstance | null, readOnly: boolean): HTMLDivElement {
  ...
  private createQuote(block: Block): HTMLQuoteElement {
    const quote = document.createElement('blockquote');
    quote.appendChild(this.renderContent(block.content as TextNode[]));
    return quote;
  }
  ...
  private createEmbed(block: Block): HTMLDivElement {
    const div = document.createElement('div');
  ...
  private createCallout(block: Block): HTMLDivElement {
    ...
    content.appendChild(this.renderContent(block.content as TextNode[]));
    div.appendChild(content);

    return div;
  }

  private createToggle(block: Block, editor: EditorInstance | null, readOnly: boolean): HTMLDetailsElement {
    const details = document.createElement('details');
    const summary = document.createElement('summary');
    summary.appendChild(this.renderContent(block.content as TextNode[]));
  ...
  private createTable(block: Block): HTMLTableElement {
    ...
    for (const row of block.children || []) {
      ...
      for (const cell of row.children || []) {
        ...
        td.appendChild(this.renderContent(cell.content as TextNode[]));
        tr.appendChild(td);
      }
