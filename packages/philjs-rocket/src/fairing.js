 ...
 */
export function createFairing(name, hooks) {
...
export class CustomFairing extends PhilJsFairing {
    ...
    constructor(name, hooks) {
        ...
        if (hooks.onIgnite)
            this.onIgnite = hooks.onIgnite;
        if (hooks.onLiftoff)
            this.onLiftoff = hooks.onLiftoff;
        if (hooks.onRequest)
            this.onRequest = hooks.onRequest;
        if (hooks.onResponse)
            this.onResponse = hooks.onResponse;
        if (hooks.onShutdown)
            this.onShutdown = hooks.onShutdown;
    }
    toRustCode() {
        const kinds = [];
        if (this.hooks.onIgnite)
            kinds.push('Kind::Ignite');
        if (this.hooks.onLiftoff)
            kinds.push('Kind::Liftoff');
        if (this.hooks.onRequest)
            kinds.push('Kind::Request');
        if (this.hooks.onResponse)
            kinds.push('Kind::Response');
        if (this.hooks.onShutdown)
            kinds.push('Kind::Shutdown');
        return `
use rocket::{Rocket, Build, fairing::{Fairing, Info, Kind}, Request, Response, Data};
 ...
 */
export function composeFairings() {
    return new FairingComposer();
}
//# sourceMappingURL=fairing.js.map