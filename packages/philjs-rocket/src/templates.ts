...
export class TemplateEngine {
  ...
  private registerBuiltinHelpers(): void {
    // Date formatting
    this.registerHelper('formatDate', (...args: unknown[]) => {
      const date = args[0] as Date | string;
    ...
    // URL encoding
    this.registerHelper('urlencode', (...args: unknown[]) => {
      return encodeURIComponent(String(args[0]));
    });

    // HTML escaping
    this.registerHelper('escape', (...args: unknown[]) => {
    ...
    // Truncate text
    this.registerHelper('truncate', (...args: unknown[]) => {
      const str = String(args[0]);
      const length = (args[1] as number) || 100;
      const suffix = (args[2] as string) || '...';
...
<div class="form-errors" role="alert">
    <ul>
        ${entries.flatMap(([field, msgs]) =>
          msgs.map(msg => `<li><strong>${field}:</strong> ${msg}</li>`)
        ).join('')}
    </ul>
</div>
`.trim();
 ...
 */
export function createTemplateEngine(options?: Partial<TemplateEngineConfig>): TemplateEngine {
