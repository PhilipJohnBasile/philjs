 ...
 */
let currentNavigationStart = null;
let currentNavigationMetrics = {};
// ============================================================================
// Public API
// ============================================================================
/**
 ...
 */
export function initRouterDevTools(config) {
 ...
 */
export function trackNavigation(path, params, searchParams, isHistoryNav = false) {
    const state = devToolsState();
    if (!state.enabled || !state.config.autoTrack)
        return;
 ...
 */
export function completeNavigation(metrics) {
    const state = devToolsState();
    if (!state.enabled || !currentNavigationStart)
        return;
 ...
 */
export function trackLoader(routeId, duration) {
    const state = devToolsState();
    if (!state.enabled)
        return;
 ...
 */
export function updateRouteTree(routes) {
    const state = devToolsState();
    if (!state.enabled)
        return;
 ...
 */
export function updateRouteState(snapshot) {
    const state = devToolsState();
    if (!state.enabled)
        return;
 ...
 */
export function recordRouteMatch(debugInfo) {
    const state = devToolsState();
    if (!state.enabled)
        return;
 ...
 */
export function RouterDevTools(props) {
    if (props?.config) {
    ...
    return {
        ...
        props: {
            id: "philjs-devtools",
            class: `philjs-devtools philjs-devtools--${state.config.position} ${state.minimized ? "philjs-devtools--minimized" : ""} philjs-devtools--${state.config.theme}`,
 ...
 */
function DevToolsExpanded(state) {
    return {
        type: "div",
        props: {
    ...
    return {
        ...
        props: {
            ...
            children: tabs.map((tab) => ({
                type: "button",
                props: {
                    class: `philjs-devtools__tab ${state.activeTab === tab.id ? "philjs-devtools__tab--active" : ""}`,
 ...
 */
function DevToolsContent(state) {
    let content;
 ...
 */
function RouteTreeNodeView(node, depth) {
    return {
        type: "div",
        props: {
            class: `philjs-devtools__tree-node ${node.isActive ? "philjs-devtools__tree-node--active" : ""}`,
            ...
            children: [
                ...
                ...(node.children || []).map((child) => RouteTreeNodeView(child, depth + 1)),
            ],
        },
    };
}
 ...
 */
function StateInspectorView(state) {
    if (!state) {
        return {
            type: "div",
            props: {
 ...
 */
function MatchingView(debugInfo) {
    if (!debugInfo) {
        return {
            type: "div",
            props: {
                ...
                },
                {
                    ...
                    props: {
                        ...
                        children: debugInfo.attempts.map((attempt) => ({
                            type: "div",
                            props: {
                                class: `philjs-devtools__matching-attempt ${attempt.matched
...
function InspectorSection(title, data) {
    return {
        type: "div",
        props: {
...
// Utility Functions
// ============================================================================
function getDevToolsStyle(config) {
    return `--devtools-size: ${config.size}px`;
}
function generateId() {
    return `${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
}
function updateRoutePerformance(routeId, duration) {
    const state = devToolsState();
