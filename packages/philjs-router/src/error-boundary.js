 ...
 */
const routeErrorsSignal = signal(new Map());
/**
 * Current route's error for useRouteError hook.
 */
const currentRouteErrorSignal = signal(null);
/**
 * Stack of error contexts for the current route hierarchy.
 */
const errorStackSignal = signal([]);
// ============================================================================
 ...
 */
export function isRouteErrorResponse(error) {
 ...
 */
export async function createRouteErrorResponse(response) {
    let data;
 ...
 */
export function throwResponse(data, init) {
 ...
 */
export function throwForbidden(message) {
 ...
 */
export function throwServerError(message) {
...
export function useRouteError() {
    ...
    if (!error) {
        throw new Error("[PhilJS Router] useRouteError must be used inside an ErrorBoundary component.");
    }
    return error;
}
/**
 ...
 */
export function useRouteErrorById(routeId) {
 ...
 */
export function useHasRouteError() {
 ...
 */
export function setRouteError(routeId, error) {
 ...
 */
export function setCurrentRouteError(error) {
 ...
 */
export function markErrorHandled(routeId) {
    const errors = new Map(routeErrorsSignal());
 ...
 */
export function setErrorStack(stack) {
        ...
        props: {
            ...
            children: [
                ...
                },
                {
                    type: "pre",
                    props: {
                        style: "text-align: left; background: #f7fafc; padding: 16px; border-radius: 4px; overflow: auto;",
 ...
 */
export async function catchRouteError(routeId, fn) {
    try {
        return await fn();
    }
    catch (error) {
 ...
 */
export function handleRouteError(error, matches) {
 ...
 */
export async function withErrorRecovery(fn, options = {}) {
    const { maxRetries = 3, retryDelay = 1000, exponentialBackoff = true, retryOn = () => true, } = options;
    let lastError;
 ...
 */
export function createRetryHandler(statusCodes) {
    return (error) => {
        if (isRouteErrorResponse(error)) {
