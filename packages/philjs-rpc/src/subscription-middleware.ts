 ...
 * ```
 */
export function createSubscriptionAuthMiddleware(
  ...
  return {
    onSubscribe: async (ctx: SubscriptionContext, path: string) => {
      // Check authentication
      if (isAuthenticated) {
        const authenticated = await isAuthenticated(ctx);
        if (!authenticated) {
          throw new RPCErrorClass({
            code: unauthorizedCode as any,
 ...
 * ```
 */
export function createSubscriptionRateLimitMiddleware(
  ...
  const {
    ...
    windowMs = 1000,
    getUserId = (ctx) => ctx.user as string | undefined,
  } = options;
  ...
  const subscriptionEvents = new Map<string, number[]>();

  return {
    onSubscribe: (ctx: SubscriptionContext) => {
    ...
    onData: (ctx: SubscriptionContext, data: unknown) => {
      // Track event rate
      const now = Date.now();
    ...
    onUnsubscribe: (ctx: SubscriptionContext) => {
      // Clean up tracking
...
// ============================================================================
// Subscription Filtering Middleware
// ============================================================================
 ...
 * ```
 */
export function createSubscriptionFilterMiddleware<TData = unknown>(
  options: SubscriptionFilterOptions<TData>
): SubscriptionMiddleware {
  const { filter } = options;

  return {
    onData: async (ctx: SubscriptionContext, data: unknown) => {
      const shouldEmit = await filter(ctx, data as TData);
...
// ============================================================================
// Subscription Multiplexing Middleware
// ============================================================================
 ...
 * ```
 */
export function createMultiplexingMiddleware(
  options: MultiplexingOptions
): SubscriptionMiddleware {
  const {
    getKey,
    maxSubscriptionsPerKey = 10000,
  } = options;

  const keySubscriptions = new Map<string, Set<string>>();

  return {
    onSubscribe: (ctx: SubscriptionContext, path: string, input: unknown) => {
 ...
 * ```
 */
export function composeSubscriptionMiddleware(
  middlewares: SubscriptionMiddleware[]
): SubscriptionMiddleware {
  return {
    onSubscribe: async (ctx: SubscriptionContext, path: string, input: unknown) => {
    ...
    onError: async (ctx: SubscriptionContext, error: RPCError) => {
      for (const middleware of middlewares) {
    ...
    onComplete: async (ctx: SubscriptionContext) => {
      for (const middleware of middlewares) {
    ...
    onUnsubscribe: async (ctx: SubscriptionContext) => {
      for (const middleware of middlewares) {
