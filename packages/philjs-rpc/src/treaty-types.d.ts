 ...
 */
export type ExtractPathParamNames<T extends string> = T extends `${infer _Start}:${infer Param}/${infer Rest}` ? Param | ExtractPathParamNames<Rest> : T extends `${infer _Start}:${infer Param}` ? Param : never;
/**
 ...
 */
export type ExtractBody<TInput> = TInput extends {
    body: infer B;
} ? B : TInput extends {
    data: infer D;
} ? D : TInput extends {
    query: unknown;
} ? never : TInput extends {
    params: unknown;
} ? never : TInput extends void ? never : TInput;
/**
 * Extract headers from input type.
 */
export type ExtractHeaders<TInput> = TInput extends {
    headers: infer H;
} ? H : never;
/**
 * Extract cookies from input type.
 */
export type ExtractCookies<TInput> = TInput extends {
    cookies: infer C;
} ? C : never;
/**
 * Extract the output type from a procedure.
 */
export type InferOutput<TProcedure> = TProcedure extends ProcedureDefinition<ProcedureType, unknown, infer TOutput, unknown> ? TOutput : never;
/**
 * Extract the input type from a procedure.
 */
export type InferInput<TProcedure> = TProcedure extends ProcedureDefinition<ProcedureType, infer TInput, unknown, unknown> ? TInput : never;
/**
 * Extract the procedure type.
 */
export type InferProcedureType<TProcedure> = TProcedure extends ProcedureDefinition<infer TType, unknown, unknown, unknown> ? TType : never;
/**
 * Extract possible error types from a procedure.
 */
export type InferErrors<TProcedure> = TProcedure extends ProcedureDefinition<ProcedureType, unknown, unknown, unknown> ? TProcedure extends {
    _errors?: infer E;
} ? E : Error : never;
/**
 * Union of all possible error codes.
 */
export type ErrorCode = 'BAD_REQUEST' | 'UNAUTHORIZED' | 'FORBIDDEN' | 'NOT_FOUND' | 'METHOD_NOT_ALLOWED' | 'TIMEOUT' | 'CONFLICT' | 'PRECONDITION_FAILED' | 'PAYLOAD_TOO_LARGE' | 'UNPROCESSABLE_ENTITY' | 'TOO_MANY_REQUESTS' | 'CLIENT_CLOSED_REQUEST' | 'INTERNAL_SERVER_ERROR' | 'NETWORK_ERROR';
/**
 ...
 */
export type ExtractFiles<TInput> = TInput extends {
    files: infer F;
} ? F : TInput extends {
    file: infer F;
} ? F : never;
 ...
 */
export type ProcedureTypeToMethod<T extends ProcedureType> = T extends 'query' ? 'GET' : T extends 'mutation' ? 'POST' : never;
/**
 ...
 */
export type BuildPath<TSegments extends readonly string[], TSep extends string = '.'> = TSegments extends readonly [infer First extends string, ...infer Rest extends readonly string[]] ? Rest extends readonly [] ? First : `${First}${TSep}${BuildPath<Rest, TSep>}` : never;
/**
 * Extract all paths from a router.
 */
export type ExtractPaths<TRouter extends Router, TPrefix extends string = ''> = {
    [K in keyof TRouter]: TRouter[K] extends ProcedureDefinition<ProcedureType, unknown, unknown, unknown> ? TPrefix extends '' ? K & string : `${TPrefix}.${K & string}` : TRouter[K] extends Router ? ExtractPaths<TRouter[K], TPrefix extends '' ? K & string : `${TPrefix}.${K & string}`> : never;
}[keyof TRouter];
/**
 ...
 */
export type ValidInput<T> = T extends void ? never : T extends undefined ? never : T;
/**
...
//# sourceMappingURL=treaty-types.d.ts.map