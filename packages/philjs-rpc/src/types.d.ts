 ...
 */
export type InferSchemaOutput<S> = S extends ZodType<infer T, ZodTypeDef, unknown> ? T : S extends {
 ...
 */
export type ProcedureHandler<TInput, TOutput, TContext = ProcedureContext> = (opts: ProcedureOptions<TInput, TContext>) => TOutput | Promise<TOutput>;
/**
 * A defined procedure with input schema and handler.
 */
export interface ProcedureDefinition<TType extends ProcedureType = ProcedureType, TInput = unknown, TOutput = unknown, TContext = ProcedureContext> {
    _type: TType;
    _input: TInput;
    _output: TOutput;
    _context: TContext;
 ...
 */
export type MiddlewareFn<TInput = unknown, TContext = ProcedureContext, TNewContext = TContext> = (opts: {
    ctx: TContext;
    input: TInput;
 ...
 * RPC error codes.
 */
export type RPCErrorCode = 'BAD_REQUEST' | 'UNAUTHORIZED' | 'FORBIDDEN' | 'NOT_FOUND' | 'METHOD_NOT_ALLOWED' | 'TIMEOUT' | 'CONFLICT' | 'PRECONDITION_FAILED' | 'PAYLOAD_TOO_LARGE' | 'UNPROCESSABLE_ENTITY' | 'TOO_MANY_REQUESTS' | 'CLIENT_CLOSED_REQUEST' | 'INTERNAL_SERVER_ERROR';
/**
 * HTTP status codes for RPC errors.
 */
export declare const RPC_ERROR_CODES_TO_HTTP: Record<RPCErrorCode, number>;
/**
 ...
 */
export type InferProcedureInput<TProcedure> = TProcedure extends ProcedureDefinition<ProcedureType, infer TInput, unknown, ProcedureContext> ? TInput : never;
/**
 * Infer output type from a procedure.
 */
export type InferProcedureOutput<TProcedure> = TProcedure extends ProcedureDefinition<ProcedureType, unknown, infer TOutput, ProcedureContext> ? TOutput : never;
/**
 * Check if a type is a procedure.
 */
export type IsProcedure<T> = T extends ProcedureDefinition<ProcedureType, unknown, unknown, ProcedureContext> ? true : false;
/**
 * Check if a procedure is a query.
 */
export type IsQuery<T> = T extends ProcedureDefinition<'query', unknown, unknown, ProcedureContext> ? true : false;
/**
 * Check if a procedure is a mutation.
 */
export type IsMutation<T> = T extends ProcedureDefinition<'mutation', unknown, unknown, ProcedureContext> ? true : false;
/**
 * Deep unwrap a router to get all procedure paths.
 */
export type RouterPaths<TRouter extends Router, TPrefix extends string = ''> = {
    [K in keyof TRouter]: TRouter[K] extends ProcedureDefinition<ProcedureType, unknown, unknown, ProcedureContext> ? TPrefix extends '' ? K : `${TPrefix}.${K & string}` : TRouter[K] extends Router ? RouterPaths<TRouter[K], TPrefix extends '' ? K & string : `${TPrefix}.${K & string}`> : never;
}[keyof TRouter];
/**
 * Get procedure at a specific path in a router.
 */
export type GetProcedureAtPath<TRouter extends Router, TPath extends string> = TPath extends `${infer First}.${infer Rest}` ? First extends keyof TRouter ? TRouter[First] extends Router ? GetProcedureAtPath<TRouter[First], Rest> : never : never : TPath extends keyof TRouter ? TRouter[TPath] : never;
/**
 * Client procedure type with fetch and hooks.
 */
export interface ClientProcedure<TInput, TOutput, TType extends ProcedureType> {
    /** Direct fetch call */
    fetch: TInput extends void ? () => Promise<TOutput> : (input: TInput) => Promise<TOutput>;
    /** React Query-style hook */
    useQuery: TType extends 'query' ? TInput extends void ? (options?: UseQueryOptions<TOutput>) => UseQueryResult<TOutput> : (input: TInput, options?: UseQueryOptions<TOutput>) => UseQueryResult<TOutput> : never;
    /** Mutation hook */
    useMutation: TType extends 'mutation' ? (options?: UseMutationOptions<TOutput, TInput>) => UseMutationResult<TOutput, TInput> : never;
    /** Subscription hook */
    useSubscription: TType extends 'subscription' ? (input: TInput, options?: UseSubscriptionOptions<TOutput>) => UseSubscriptionResult<TOutput> : never;
}
/**
 * Build client type from router.
 */
export type BuildClientFromRouter<TRouter extends Router> = {
    [K in keyof TRouter]: TRouter[K] extends ProcedureDefinition<infer TType, infer TInput, infer TOutput, ProcedureContext> ? ClientProcedure<TInput, TOutput, TType> : TRouter[K] extends Router ? BuildClientFromRouter<TRouter[K]> : never;
};
/**
 ...
 */
export type SubscriptionHandler<TInput, TOutput, TContext = ProcedureContext> = (opts: ProcedureOptions<TInput, TContext>) => AsyncGenerator<TOutput> | AsyncIterable<TOutput>;
/**
 ...
 */
export type IsSubscription<T> = T extends ProcedureDefinition<'subscription', unknown, unknown, ProcedureContext> ? true : false;
