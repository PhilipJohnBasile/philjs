...
export const Formulas = {
  /** Kinetic energy: E = ½mv² */
  kineticEnergy: (mass: number, velocity: number) => 0.5 * mass * velocity ** 2,

  /** Gravitational potential energy: U = -GMm/r */
  gravitationalPotential: (M: number, m: number, r: number) => -G * M * m / r,

  /** Escape velocity: v = √(2GM/r) */
  escapeVelocity: (M: number, r: number) => Math.sqrt((2 * G * M) / r),

  /** Schwarzschild radius: r_s = 2GM/c² */
  schwarzschildRadius: (M: number) => (2 * G * M) / c ** 2,

  /** De Broglie wavelength: λ = h/p */
  deBroglieWavelength: (momentum: number) => h / momentum,

  /** Photon energy: E = hν */
  photonEnergy: (frequency: number) => h * frequency,

  /** Ideal gas law: PV = nRT */
  idealGasPressure: (n: number, T: number, V: number) => (n * R * T) / V,

  /** Orbital period: T = 2π√(a³/GM) */
  orbitalPeriod: (semiMajorAxis: number, centralMass: number) =>
    2 * Math.PI * Math.sqrt(semiMajorAxis ** 3 / (G * centralMass)),

  /** Lorentz factor: γ = 1/√(1 - v²/c²) */
  lorentzFactor: (velocity: number) => 1 / Math.sqrt(1 - (velocity / c) ** 2),

  /** Time dilation: Δt' = γΔt */
  timeDilation: (properTime: number, velocity: number) =>
    Formulas.lorentzFactor(velocity) * properTime,

  /** Length contraction: L' = L/γ */
  lengthContraction: (properLength: number, velocity: number) =>
    properLength / Formulas.lorentzFactor(velocity),
};
