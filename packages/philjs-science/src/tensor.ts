...
function getTypedArrayConstructor(dtype: DType): Float32ArrayConstructor | Float64ArrayConstructor | Int32ArrayConstructor | Uint8ArrayConstructor {
  switch (dtype) {
...
export class Tensor {
  ...
  private _device: 'cpu' | 'gpu' = 'cpu';

  constructor(
    data: ArrayLike<number> | Float32Array | Float64Array | Int32Array | Uint8Array,
    shape?: number[],
  ...
  static fromArray(arr: number | number[] | number[][] | number[][][], options?: TensorOptions): Tensor {
    ...
    const flat = (Array.isArray(arr) ? arr.flat(Infinity) : [arr]) as number[];
  ...
  slice(...ranges: Array<number | [number, number] | null>): Tensor {
    ...
    // Copy data
    const copySlice = (srcOffset: number, dstOffset: number, dim: number): void => {
  ...
  transpose(axes?: number[]): Tensor {
    ...
    const copyTransposed = (indices: number[], depth: number): void => {
      if (depth === this.ndim) {
        let srcOffset = 0;
        let dstOffset = 0;
  ...
  sum(axis?: number, keepdims = false): Tensor | number {
    if (axis === undefined) {
      let total = 0;
      for (let i = 0; i < this.size; i++) {
        total += this.data[i]!;
  ...
  mean(axis?: number, keepdims = false): Tensor | number {
    if (axis === undefined) {
      return (this.sum() as number) / this.size;
    }

    const sumResult = this.sum(axis, keepdims) as Tensor;
    return sumResult.div(this.shape[axis]!);
  }

  /** Max along axis or all elements */
  max(axis?: number, keepdims = false): Tensor | number {
    if (axis === undefined) {
      let maxVal = -Infinity;
      for (let i = 0; i < this.size; i++) {
  ...
  min(axis?: number, keepdims = false): Tensor | number {
    if (axis === undefined) {
      let minVal = Infinity;
      for (let i = 0; i < this.size; i++) {
  ...
  private batchedMatmul(other: Tensor): Tensor {
  ...
  svd(): { U: Tensor; S: Tensor; V: Tensor } {
    if (this.ndim !== 2) {
    ...
    for (let i = 0; i < k; i++) {
      ...
      for (let j = 0; j < m; j++) U[j * k + i] = u.data[j]!;
      for (let j = 0; j < n; j++) V[j * k + i] = v.data[j]!;
  ...
  eig(): { values: Tensor; vectors: Tensor } {
    if (this.ndim !== 2 || this.shape[0] !== this.shape[1]) {
  ...
  broadcastTo(targetShape: number[]): Tensor {
    ...
    // Broadcast copy
    const copyBroadcast = (srcIndices: number[], dstIndex: number, dim: number): void => {
  ...
  toArray(): number[] | number[][] | number[][][] {
    ...
    const result: any = [];
    const toNestedArray = (offset: number, dim: number): any => {
      if (dim === this.ndim - 1) {
        const arr = [];
  ...
  private unravelIndex(flatIdx: number): number[] {
  ...
  private ravelIndex(indices: number[], shape: number[]): number {
  ...
  async toGPU(): Promise<void> {
    if (this._device === 'gpu') return;
