...
async function renderCurrentRoute(routeMap, transitionManager, preloader, target, url, navigate, forceRevalidate = false) {
    ...
    const ssrDataCache = typeof window !== "undefined" ? ((_a = window).__PHILJS_ROUTE_DATA__ || (_a.__PHILJS_ROUTE_DATA__ = {})) : undefined;
    const ssrErrorCache = typeof window !== "undefined" ? ((_b = window).__PHILJS_ROUTE_ERROR__ || (_b.__PHILJS_ROUTE_ERROR__ = {})) : undefined;
    // Load all route data in parallel (no waterfall!)
    const loadedMatches = await loadAllRouteData(matches, url, ssrDataCache, ssrErrorCache, forceRevalidate);
    // Get leaf route data
    ...
    if (typeof window !== "undefined") {
        const routeInfoStore = ((_c = window).__PHILJS_ROUTE_INFO__ || (_c.__PHILJS_ROUTE_INFO__ = {}));
        routeInfoStore.current = {
            path: url.pathname,
            params: leafMatch.params,
            error,
        };
        const dataStore = ((_d = window).__PHILJS_ROUTE_DATA__ || (_d.__PHILJS_ROUTE_DATA__ = {}));
        dataStore[cacheKey] = data;
        const errorStore = ((_e = window).__PHILJS_ROUTE_ERROR__ || (_e.__PHILJS_ROUTE_ERROR__ = {}));
