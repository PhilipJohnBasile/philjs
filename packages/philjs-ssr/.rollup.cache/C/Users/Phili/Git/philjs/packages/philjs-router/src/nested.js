 ...
 */
function matchRoutesRecursive(pathname, routes, parentPath, parentParams, caseSensitive) {
    for (const route of routes) {
        ...
        if (result) {
            ...
            if (route.children && route.children.length > 0 && result.remaining) {
                const childMatches = matchRoutesRecursive(result.remaining, route.children, fullPath, params, caseSensitive);
