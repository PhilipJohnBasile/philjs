...
export function buildCSP(options = {}) {
    const { directives = DEFAULT_CSP_DIRECTIVES, reportOnly = false, nonce, autoNonce = false, } = options;
 ...
 * ```
 */
export function mergeCSP(base, override) {
    const merged = { ...base };
...
export function validateCSP(directives) {
    ...
    if (scriptSrc.includes("'unsafe-eval'")) {
        warnings.push("script-src contains 'unsafe-eval' which allows eval() and is dangerous");
    }
    ...
    if (scriptSrc.includes("'unsafe-inline'")) {
        ...
        if (!hasNonce && !hasHash) {
            warnings.push("script-src contains 'unsafe-inline' without nonce or hash. Consider using nonces for better security.");
        }
    }
    // Check for wildcard sources
    if (scriptSrc.includes('*') || scriptSrc.includes('https:')) {
        warnings.push("script-src contains wildcard or protocol-only source which may be too permissive");
    }
    // Check for data: in script-src
    if (scriptSrc.includes('data:')) {
        warnings.push("script-src contains 'data:' which can be used for XSS attacks");
    }
    // Check for missing base-uri
    if (!directives['base-uri']) {
        warnings.push("Missing 'base-uri' directive. Consider setting it to 'self' or 'none'");
    }
    // Check for missing object-src
    if (!directives['object-src']) {
        warnings.push("Missing 'object-src' directive. Consider setting it to 'none'");
    }
    // Check for upgrade-insecure-requests in production
    if (!directives['upgrade-insecure-requests']) {
        warnings.push("Consider enabling 'upgrade-insecure-requests' for production environments");
    }
    return warnings;
}
