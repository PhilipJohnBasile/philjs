 ...
 * ```
 */
import { PPR_PLACEHOLDER_START, PPR_PLACEHOLDER_END, PPR_FALLBACK_START, PPR_FALLBACK_END, } from "./ppr-types.js";
// ============================================================================
...
export function isDynamic(value) {
    ...
    if ("props" in value &&
        ...
        value.props.__dynamicType === DYNAMIC_SYMBOL) {
        return true;
    }
    // Check if it's directly marked (for internal use)
    if ("__dynamicType" in value && value.__dynamicType === DYNAMIC_SYMBOL) {
        return true;
    }
    return false;
}
 ...
 */
function DynamicBoundary(props) {
...
DynamicBoundary.__dynamicType = DYNAMIC_SYMBOL;
// ============================================================================
 ...
 */
export function registerDynamicBoundary(ctx, props) {
 ...
 */
export function createDynamic(defaultOptions) {
    return function configuredDynamic(props) {
        return dynamic({
            ...defaultOptions,
            ...props,
        });
 ...
 */
export function dynamicWithDependencies(dependencies, props) {
    return dynamic({
        ...props,
 ...
 */
export function dynamicIf(condition, dynamicContent, staticContent, options) {
    // The condition is evaluated at request time
    // For build time, we always render the fallback
    return dynamic({
        ...options,
        children: {
            type: ConditionalDynamic,
            props: {
                condition,
                dynamicContent,
                staticContent,
 ...
 */
export function getDynamicBoundaryId(vnode) {
    if (vnode &&
        typeof vnode === "object" &&
        "props" in vnode &&
        vnode.props &&
 ...
 */
export function serverOnly(props) {
    return dynamic({
        ...props,
 ...
 */
export function dynamicWithRevalidation(revalidateSeconds, props) {
    return dynamic({
        ...props,
 ...
 */
export function dynamicForUser(props) {
    return dynamic({
        ...props,
