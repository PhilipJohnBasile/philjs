 ...
 */
export function configureErrorTracking(tracker) {
...
export function parseErrorStack(error) {
    ...
    if (error.stack)
        mainDiagnostic.stack = error.stack;
    ...
    if (error.stack) {
        ...
        for (const line of stackLines.slice(0, 5)) { // Show first 5 frames
            ...
            if (match) {
                ...
                if (file)
                    frameDiagnostic.file = file;
                if (lineStr)
                    frameDiagnostic.line = parseInt(lineStr, 10);
                if (columnStr)
                    frameDiagnostic.column = parseInt(columnStr, 10);
 ...
 */
export function generateErrorSuggestions(error) {
    const suggestions = [];
  ...
  <div class="container">
    ...
    <div class="card">
      ...
      <ul class="suggestions">
        ${suggestions.map(suggestion => `<li class="suggestion">${escapeHtml(suggestion)}</li>`).join('')}
      </ul>
      <div class="actions">
        <a href="/" class="btn btn-primary">Go Home</a>
 ...
 */
export function generateErrorResponse(error, statusCode, config, context) {
    const { url, requestId, timestamp = new Date() } = context || {};
    let html;
    if (statusCode === 404) {
        const notFoundProps = {};
        if (url)
            notFoundProps.url = url;
        if (config)
            notFoundProps.config = config;
        html = NotFoundPage(notFoundProps);
    }
    else {
        const errorProps = { error, timestamp };
        if (config)
            errorProps.config = config;
        if (requestId)
            errorProps.requestId = requestId;
