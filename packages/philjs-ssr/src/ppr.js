 ...
 */
import { PPR_PLACEHOLDER_START, PPR_PLACEHOLDER_END, PPR_FALLBACK_START, PPR_FALLBACK_END, hashContent, } from "./ppr-types.js";
 ...
 */
export function createPPRContext(mode, options = {}) {
    const ctx = {
        mode,
        boundaries: new Map(),
        boundaryId: 0,
 ...
 */
export async function renderToStaticShell(vnode, path, config = { ppr: true }) {
    ...
    const boundaryMetadata = new Map();
 ...
 */
async function renderWithPPR(vnode, ctx) {
 ...
 */
async function renderDynamicBoundary(vnode, ctx) {
    if (!isJSXElement(vnode)) {
        return "";
    }
 ...
 */
async function renderElement(tag, props, ctx) {
 ...
 */
async function renderStaticContent(vnode) {
    if (vnode == null || typeof vnode === "boolean") {
        return "";
    }
    if (typeof vnode === "string" || typeof vnode === "number") {
 ...
 */
export async function renderDynamicContent(boundary, ctx) {
 ...
 */
export async function renderAllDynamicContent(shell, requestData, originalVNode) {
 ...
 */
export function injectDynamicContent(shell, resolutions) {
    let html = shell.html;
 ...
 */
export async function generatePPRResponse(shell, vnode, request, options = {}) {
...
`));
                ...
                controller.enqueue(encoder.encode(`<script>${pendingIds.map((id) => `__PPR__.pending.add('${id}')`).join(";")}</script>\n`));
                options.onShellSent?.();
                ...
                const sortedBoundaries = Array.from(ctx.boundaries.entries()).sort(([, a], [, b]) => (b.priority || 5) - (a.priority || 5));
                ...
                for (const [id, boundary] of sortedBoundaries) {
                    try {
                        ...
                        const escapedWrapped = wrappedHtml
                            ...
                            .replace(/\n/g, "\\n")
                            .replace(/\r/g, "\\r");
                        controller.enqueue(encoder.encode(`<script>__PPR__.inject('${id}', '${escapedWrapped}');</script>\n`));
                        options.onBoundaryResolved?.(id);
                    }
                    catch (error) {
                        // Send error fallback
                        controller.enqueue(encoder.encode(`<script>__PPR__.inject('${id}', '<div data-ppr-error="true">Error loading content</div>');</script>\n`));
                    }
                }
                // Close document
 ...
 */
function replaceMarkersWithFallbacks(shell) {
    let html = shell.html;
...
async function extractAssets(html) {
    const assets = {
        css: [],
        js: [],
        fonts: [],
    ...
    const inlineStyleMatch = html.match(/<style[^>]*data-critical[^>]*>([\s\S]*?)<\/style>/);
    if (inlineStyleMatch && inlineStyleMatch[1] !== undefined) {
        assets.inlineCss = inlineStyleMatch[1];
    }
    return assets;
}
function generatePreloadTags(assets) {
    const tags = [];
