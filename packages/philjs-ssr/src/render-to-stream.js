 ...
 */
async function renderVNodeToString(vnode, ctx, isShell) {
    ...
    if (Array.isArray(vnode)) {
        const parts = await Promise.all(vnode.map((child) => renderVNodeToString(child, ctx, isShell)));
        return parts.join("");
    }
    ...
    if (isShell) {
        ...
        try {
            ...
            if (isPromiseLike(error)) {
                ...
                ctx.pendingBoundaries.set(id, error.then(() => children));
 ...
 */
async function renderElement(tag, props, ctx, isShell) {
