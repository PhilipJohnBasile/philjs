 ...
 * Stream pending suspense boundaries as they resolve.
 */
async function streamPendingBoundaries(
    ...
    const result = await Promise.race(
      entries.map(async ([id, promise]) => {
        try {
          ...
          return { id, content: null, error: error as Error };
        }
      })
    );
    ...
    try {
      ...
      if (isPromiseLike(error)) {
        if (isShell) {
          ...
          ctx.pendingBoundaries.set(id, (error as Promise<any>).then(() => type(props)));
