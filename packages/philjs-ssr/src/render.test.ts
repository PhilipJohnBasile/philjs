...
describe('Static Site Generation', () => {
  ...
  describe('StaticGenerator', () => {
    ...
    it('should handle different render functions', () => {
      const renderFn1 = async (path: string) => `<html><body>Page 1: ${path}</body></html>`;
      const renderFn2 = async (path: string) => `<html><body>Page 2: ${path}</body></html>`;
...
describe('Rate Limiting', () => {
  ...
  describe('RateLimiter', () => {
    it('should allow requests under limit', async () => {
      ...
      const info = (request as any).rateLimit;
    ...
    it('should handle concurrent requests', async () => {
      ...
      const promises = Array.from({ length: 10 }, (_, i) =>
        limiter.check(new Request(`https://example.com/test${i}`))
      );
