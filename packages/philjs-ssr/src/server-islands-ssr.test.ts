...
describe.skip('Server Islands SSR Integration', () => {
  ...
  describe('Basic Server Island Rendering', () => {
    ...
    it('should render island with props', async () => {
      const Component = (props: { name: string; count: number }) => {
  ...
  describe('Server Island Caching', () => {
    it('should cache island with TTL', async () => {
      ...
      const cacheConfig: ServerIslandCache = {
        ttl: 60,
      };
    ...
    it('should use custom cache key generator', async () => {
      const Component = (props: { userId: string }) => `User ${props.userId}`;
    ...
    it('should vary cache by props', async () => {
      const Component = (props: { page: number }) => `Page ${props.page}`;

      const cacheConfig: ServerIslandCache = { ttl: 60 };
  ...
  describe('Metrics and Monitoring', () => {
    ...
    it('should track stale hits', async () => {
      const Component = () => 'Content';
      const cacheConfig: ServerIslandCache = { ttl: 0.05, swr: 10 };
  ...
  describe('Cache Headers', () => {
    it('should generate public cache-control headers', () => {
      const config: ServerIslandCache = { ttl: 3600 };
    ...
    it('should generate private cache-control headers', () => {
      const config: ServerIslandCache = { ttl: 3600, private: true };
      const headers = getIslandCacheHeaders(config);
    ...
    it('should include stale-while-revalidate', () => {
      const config: ServerIslandCache = { ttl: 3600, swr: 86400 };
      const headers = getIslandCacheHeaders(config);
    ...
    it('should include vary headers', () => {
      const config: ServerIslandCache = {
        ttl: 3600,
    ...
    it('should include CDN cache headers for edge', () => {
      const config: ServerIslandCache = { ttl: 3600, edge: true };
      const headers = getIslandCacheHeaders(config);
  ...
  describe('Custom Cache Stores', () => {
    it('should use custom cache store', async () => {
      const customStore: IslandCacheStore = {
        cache: new Map<string, CachedIsland>(),
  ...
  describe('Complex Scenarios', () => {
    it('should handle personalized content with caching', async () => {
      const PersonalizedComponent = (props: { userId: string }) => {
        return jsx('div', {
      ...
      const html1 = await renderServerIsland(
        'greeting',
      ...
      const html2 = await renderServerIsland(
        'greeting',
        PersonalizedComponent,
        { userId: '456' },
        cacheConfig
    ...
    it('should handle product recommendations with cache tags', async () => {
      const ProductRecs = (props: { category: string }) => {
        return jsx('div', {
