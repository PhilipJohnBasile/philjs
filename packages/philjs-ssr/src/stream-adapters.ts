...
export function createThroughputMeasurer(): {
  ...
  getStats: () => { bytes: number; chunks: number; duration: number };
} {
  let bytesWritten = 0;
  let chunksWritten = 0;
 ...
 */
export function createMultiplexStream(
  destinations: WritableStream<Uint8Array>[]
...
export function createTimingStream(): TransformStream<
  Uint8Array,
  TimedChunk
> {
  let index = 0;

  return new TransformStream<Uint8Array, TimedChunk>({
    transform(chunk, controller) {
      controller.enqueue({
        chunk,
        timestamp: Date.now(),
...
export function createFilterStream(
  predicate: (chunk: Uint8Array) => boolean
): TransformStream<Uint8Array, Uint8Array> {
  return new TransformStream<Uint8Array, Uint8Array>({
    transform(chunk, controller) {
      if (predicate(chunk)) {
