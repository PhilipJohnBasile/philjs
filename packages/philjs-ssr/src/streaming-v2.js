...
export function createStreamingRenderer(config = {}) {
    ...
    return {
         ...
         */
        getResumabilityScript: (ctx) => {
            return generateResumabilityScript(ctx);
        },
    };
}
function generateShellStart(config = {}) {
    const { doctype = '<!DOCTYPE html>', htmlAttributes = { lang: 'en' }, head = '', bodyAttributes = {}, } = config;
...
function generateShellEnd(config = {}) {
    return `
...
// Core Rendering
// =============================================================================
function renderToStreamV2(rootNode, config, options) {
    const ctx = {
        config,
        boundaryId: 0,
        boundaries: new Map(),
    ...
    const flushBoundaries = async () => {
        ...
        for (const [id, boundary] of Array.from(ctx.boundaries.entries())) {
            if ((boundary.status === 'complete' || boundary.status === 'error') &&
...
async function renderNodeV2(node, ctx) {
    ...
    if (node.type === 'Suspense' || node.props?.__suspense) {
    ...
    const childHtml = innerHtml !== undefined ? String(innerHtml) : await renderChildrenV2(props?.children, ctx);
    if (voidElements.has(type)) {
        return `<${type}${attrs}>`;
    }
    return `<${type}${attrs}>${childHtml}</${type}>`;
}
async function renderSuspenseV2(node, ctx) {
...
async function renderChildrenV2(children, ctx) {
...
async function processBoundaries(ctx, flush, config) {
...
function generateBoundaryScript(boundary) {
...
function generateResumabilityScript(ctx) {
...
function renderAttributes(props, ctx) {
    if (!props)
        return '';
    const attrs = [];
...
//# sourceMappingURL=streaming-v2.js.map