 ...
 */
export async function renderToStreamingResponse(vnode, options = {}) {
...
`));
            ...
            while (ctx.pending.size > 0) {
                ...
                const settled = await Promise.race(promises.map(async ([id, promise]) => {
                    try {
 ...
 */
async function renderWithSuspense(vnode, ctx) {
