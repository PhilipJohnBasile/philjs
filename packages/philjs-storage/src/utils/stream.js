 ...
 */
export async function createStreamingUpload(stream, processor, options = {}) {
 ...
 */
export async function streamToBuffer(stream, options = {}) {
    const chunks = [];
    let totalBytes = 0;
    await createStreamingUpload(stream, async (generator) => {
 ...
 */
export function bufferToStream(buffer, chunkSize = 64 * 1024) {
    let offset = 0;
 ...
 */
export function createProgressStream(options) {
    let totalBytes = 0;
 ...
 */
export async function* splitIntoParts(stream, partSize) {
    const reader = stream.getReader();
    let buffer = new Uint8Array(0);
    let partNumber = 1;
    try {
 ...
 */
export function limitStream(stream, maxBytes) {
    let bytesRead = 0;
    const reader = stream.getReader();
    return new ReadableStream({
        async pull(controller) {
 ...
 */
export function estimateRemainingTime(bytesUploaded, totalBytes, elapsedMs) {
