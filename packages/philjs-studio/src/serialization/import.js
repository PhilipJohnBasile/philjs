...
const remapIds = (components, preserveIds) => {
    const idMap = new Map();
    if (preserveIds) {
...
const applyTransform = (bounds, options) => {
...
export const validateSchema = (data) => {
    ...
    if (!Array.isArray(schema['components']))
        return false;
    if (!Array.isArray(schema['rootIds']))
        return false;
    ...
    for (const comp of schema['components']) {
        ...
        if (!Array.isArray(c['children']))
            return false;
        if (!c['bounds'] || typeof c['bounds'] !== 'object')
            return false;
    }
    return true;
};
export const importFromJSON = (jsonString, options = {}) => {
    const errors = [];
    let data;
    try {
    ...
    switch (node.type) {
        ...
        case 'INSTANCE':
            ...
            if (nameLower.includes('button'))
                return 'Button';
            if (nameLower.includes('input') || nameLower.includes('text field'))
                return 'Input';
...
const extractStyles = (node) => {
    ...
    if (node.effects && node.effects.length > 0) {
        const shadowEffects = node.effects.filter((e) => e.type === 'DROP_SHADOW' || e.type === 'INNER_SHADOW');
...
const processFigmaNode = (node, parentId, context, options) => {
    const id = generateId();
...
export const importFromFigma = (figmaData, options = {}) => {
    const context = {
        components: {},
        errors: [],
        idMap: new Map(),
...
export const importFromFigmaJSON = (jsonString, options = {}) => {
    try {
...
// Clipboard Import
// ============================================================================
export const importFromClipboard = async (options = {}) => {
    try {
...
// File Import
// ============================================================================
export const importFromFile = (file, options = {}) => {
...
export const importDesign = async (source, format = 'auto', options = {}) => {
