 ...
 */
export function createStyled(Component) {
    return function createStyledComponent(stylesOrFactory) {
        ...
        return function StyledComponent(props) {
            ...
            if (typeof Component === 'function') {
                return Component({ ...rest, className: combinedClass, children });
            }
            return {
                type: Component,
...
function generateCSS(selector, styles) {
    ...
    for (const [key, value] of Object.entries(styles)) {
        if (typeof value === 'object' && value !== null) {
            ...
            if (key.startsWith('@')) {
                nested.push(`${key} { ${selector} { ${objectToDeclarations(value)} } }`);
            }
            else if (key.startsWith('&')) {
                const nestedSelector = key.replace('&', selector);
                nested.push(`${nestedSelector} { ${objectToDeclarations(value)} }`);
            }
            else {
                nested.push(`${selector} ${key} { ${objectToDeclarations(value)} }`);
            }
        }
        else {
...
function objectToDeclarations(styles) {
