 ...
 */
export function philjsTailwindVite(options = {}) {
    const { jit = true, content = ['./src/**/*.{js,ts,jsx,tsx}', './index.html'], purge = true, extractCritical = false, } = options;
    let criticalCSS = '';
    return {
        ...
        transformIndexHtml(html) {
            if (extractCritical && criticalCSS) {
                ...
                return html.replace('</head>', `<style id="critical-css">${critical}</style></head>`);
            }
            return html;
        },
    };
 ...
 */
function extractCriticalStyles(css) {
