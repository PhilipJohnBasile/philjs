...
class OpenAIProvider {
    ...
    async generateTests(source, analysis, config) {
    ...
    buildPrompt(source, analysis, config) {
...
class LocalProvider {
    ...
    generateFunctionTest(fn, framework, category) {
  ...
  ${expectation}
    ...
    generateEdgeCaseTests(fn, framework) {
        const tests = [];
        ...
        for (const param of fn.params) {
            if (!param.optional) {
                ...
                const paramsWithNull = fn.params.map((p, i) => p === param ? 'null' : (otherParams[i] || this.generateMockValue(p))).join(', ');
                tests.push({
  ...
  ${fn.async ? 'await ' : ''}expect(() => ${fn.name}(${paramsWithNull})).toThrow();
        ...
        for (const param of fn.params) {
            if (param.type === 'string' || !param.type) {
                const paramsWithEmpty = fn.params.map(p => p === param ? "''" : this.generateMockValue(p)).join(', ');
                tests.push({
  ...
  expect(result).toBeDefined();
    ...
    generateErrorTest(fn, framework) {
        return {
  ...
  ${fn.async ? 'await ' : ''}expect(() => ${fn.name}(undefined as any)).toThrow();
    ...
    generateClassTest(cls, framework) {
        return {
  ...
  expect(instance).toBeInstanceOf(${cls.name});
    ...
    generateMethodTest(cls, method, framework) {
...
function analyzeSource(source) {
    ...
    const complexity = Math.round(functions.length * 2 +
        classes.length * 3 +
 ...
 */
async function generateTests(source, config = { provider: 'local' }) {
 ...
 * ```
 */
async function describeAI(suiteName, behaviorDescription, context) {
 ...
 */
function generateTestFile(suite, framework = 'vitest') {
 ...
 */
async function runGeneratedTests(suite) {
