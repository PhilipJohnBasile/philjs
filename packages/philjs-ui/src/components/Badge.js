...
export function Badge(props) {
    const { children, variant = 'subtle', color = 'gray', size = 'md', rounded = false, className = '', } = props;
    const classes = [
...
export function StatusIndicator(props) {
    ...
    return (_jsxs("span", { className: `inline-flex items-center ${className}`, children: [_jsx("span", { className: `${statusSizes[size]} ${statusColors[status]} rounded-full`, "aria-hidden": "true" }), label && _jsx("span", { className: "ml-2 text-sm text-gray-600", children: label })] }));
}
export function NotificationBadge(props) {
    const { count, max = 99, showZero = false, color = 'red', className = '', } = props;
    if (count === 0 && !showZero)
        return null;
