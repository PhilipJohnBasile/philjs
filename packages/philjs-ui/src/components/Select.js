...
export function MultiSelect(props) {
    ...
    return (_jsxs("div", { className: `w-full ${className}`, children: [label && (_jsxs("label", { className: "block text-sm font-medium text-gray-700 mb-1", children: [label, required && _jsx("span", { className: "text-red-500 ml-1", children: "*" })] })), _jsxs("div", { className: "relative", children: [_jsxs("div", { className: `
          ...
          `, onClick: () => !disabled && isOpen.set(!isOpen()), children: [selectedValues().length === 0 && (_jsx("span", { className: "text-gray-400", children: placeholder })), selectedValues().map(val => {
                            ...
                            })] }), isOpen() && !disabled && (_jsx("div", { className: "absolute z-10 mt-1 w-full bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-auto", children: options.map(option => (_jsxs("div", { className: `
                  px-4 py-2 cursor-pointer
                  ${option.disabled ? 'text-gray-400 cursor-not-allowed' : 'hover:bg-gray-100'}
                  ${selectedValues().includes(option.value) ? 'bg-blue-50' : ''}
                `, onClick: () => !option.disabled && toggleOption(option.value), children: [_jsx("input", { type: "checkbox", checked: selectedValues().includes(option.value), disabled: option.disabled, readOnly: true, className: "mr-2" }), option.label] }, option.value))) }))] }), helperText && !hasError && (_jsx("p", { className: "mt-1 text-sm text-gray-500", children: helperText })), errorMessage && (_jsx("p", { className: "mt-1 text-sm text-red-600", role: "alert", children: errorMessage }))] }));
}
