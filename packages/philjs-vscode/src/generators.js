...
export async function createComponent(name) {
    ...
    const componentDir = config().get('componentDirectory', 'src/components');
...
`;
    await fs.promises.writeFile(path.join(targetDir, `${pascalName}.tsx`), componentContent);
    // Create index file
...
`;
    await fs.promises.writeFile(path.join(targetDir, `${pascalName}.test.tsx`), testContent);
    // Open the component file
    const doc = await vscode.workspace.openTextDocument(path.join(targetDir, `${pascalName}.tsx`));
...
export async function createRoute(name) {
    ...
    const routesDir = config().get('routesDirectory', 'src/routes');
...
`;
    await fs.promises.writeFile(path.join(targetDir, 'index.tsx'), routeContent);
    // Create loader
    const loaderContent = `/**
...
`;
    await fs.promises.writeFile(path.join(targetDir, 'loader.ts'), loaderContent);
    // Open the route file
    const doc = await vscode.workspace.openTextDocument(path.join(targetDir, 'index.tsx'));
    await vscode.window.showTextDocument(doc);
...
`;
    await fs.promises.writeFile(path.join(targetDir, `${kebabName}.tsx`), pageContent);
    // Open the page file
    const doc = await vscode.workspace.openTextDocument(path.join(targetDir, `${kebabName}.tsx`));
    await vscode.window.showTextDocument(doc);
...
`;
    await fs.promises.writeFile(path.join(targetDir, `${hookName}.ts`), hookContent);
    // Open the hook file
    const doc = await vscode.workspace.openTextDocument(path.join(targetDir, `${hookName}.ts`));
    await vscode.window.showTextDocument(doc);
...
`;
    await fs.promises.writeFile(path.join(targetDir, `${kebabName}.ts`), storeContent);
    // Open the store file
    const doc = await vscode.workspace.openTextDocument(path.join(targetDir, `${kebabName}.ts`));
    await vscode.window.showTextDocument(doc);
