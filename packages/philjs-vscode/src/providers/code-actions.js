...
export class PhilJSCodeActionsProvider {
    ...
    shouldAddSignalGet(line, range, document) {
    ...
    createAddSignalGetAction(document, range) {
        const action = new vscode.CodeAction('Add .get() to signal access', vscode.CodeActionKind.QuickFix);
        ...
        if (match) {
            ...
            const replaceRange = new vscode.Range(range.start.line, matchIndex, range.start.line, matchIndex + match[0].length);
            edit.replace(document.uri, replaceRange, newText);
            action.edit = edit;
        }
        return action;
    }
    shouldAddEffectCleanup(line, document, range) {
    ...
    createAddEffectCleanupAction(document, range) {
        const action = new vscode.CodeAction('Add cleanup function to effect', vscode.CodeActionKind.QuickFix);
    ...
    shouldWrapInMemo(document, range) {
        const line = document.lineAt(range.start.line).text;
    ...
    createWrapInMemoAction(document, range) {
        const action = new vscode.CodeAction('Wrap component in memo()', vscode.CodeActionKind.Refactor);
        action.command = {
            title: 'Wrap in memo',
    ...
    findMissingImport(document, range) {
    ...
    createAddImportAction(document, packageName) {
        const action = new vscode.CodeAction(`Import from '${packageName}'`, vscode.CodeActionKind.QuickFix);
        action.isPreferred = true;
        const edit = new vscode.WorkspaceEdit();
    ...
    createExtractToSignalAction(document, range) {
        const action = new vscode.CodeAction('Extract to signal', vscode.CodeActionKind.RefactorExtract);
        action.command = {
    ...
    isComponent(document, range) {
        const line = document.lineAt(range.start.line).text;
        return /export (function|const) [A-Z]\w+/.test(line);
    }
    createConvertToIslandAction(document, range) {
        const action = new vscode.CodeAction('Convert to island component', vscode.CodeActionKind.Refactor);
        action.command = {
