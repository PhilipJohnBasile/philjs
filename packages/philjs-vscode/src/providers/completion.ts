...
export class PhilJSCompletionProvider implements vscode.CompletionItemProvider {
  ...
  private getPhilJSCompletions(linePrefix: string): vscode.CompletionItem[] {
    ...
    if (linePrefix.includes('signal') || linePrefix.endsWith('.')) {
      completions.push(
        this.createCompletion('signal', 'Create a reactive signal', 'const $1 = signal($2);', vscode.CompletionItemKind.Function),
        this.createCompletion('signal.get', 'Get signal value', '.get()', vscode.CompletionItemKind.Method),
        this.createCompletion('signal.set', 'Set signal value', '.set($1)', vscode.CompletionItemKind.Method),
        this.createCompletion('signal.update', 'Update signal with function', '.update(prev => $1)', vscode.CompletionItemKind.Method),
      );
    }

    // Computed completions
    completions.push(
      this.createCompletion('computed', 'Create a computed value', 'const $1 = computed(() => $2);', vscode.CompletionItemKind.Function),
    );

    // Effect completions
    completions.push(
      this.createCompletion('effect', 'Create a side effect', 'effect(() => {\n\t$1\n});', vscode.CompletionItemKind.Function),
    );

    // Lifecycle completions
    completions.push(
      this.createCompletion('onMount', 'Run on component mount', 'onMount(() => {\n\t$1\n});', vscode.CompletionItemKind.Function),
      this.createCompletion('onCleanup', 'Run on cleanup', 'onCleanup(() => {\n\t$1\n});', vscode.CompletionItemKind.Function),
    );

    // Memo completions
    completions.push(
      this.createCompletion('memo', 'Memoize a component', 'const $1 = memo(($2) => {\n\treturn (\n\t\t$3\n\t);\n});', vscode.CompletionItemKind.Function),
    );

    // Context completions
    completions.push(
      this.createCompletion('createContext', 'Create a context', 'const $1Context = createContext<$2 | null>(null);', vscode.CompletionItemKind.Function),
      this.createCompletion('useContext', 'Use a context', 'const $1 = useContext($2Context);', vscode.CompletionItemKind.Function),
    );
