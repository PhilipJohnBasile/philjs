...
function readRustFile(filePath: string): string | null {
  try {
...
function getOutputPath(inputPath: string, specifiedOutput?: string): string {
  if (specifiedOutput) {
...
function generateCommand(args: ParsedArgs): CodegenResult {
  if (!args.input) {
...
async function watchCommand(args: ParsedArgs): Promise<void> {
  ...
  function generateForFile(filePath: string): void {
