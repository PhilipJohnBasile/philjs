 ...
 */
export async function checkZigInstalled() {
    try {
 ...
 * Build Zig project
 */
export async function buildZig(config = {}) {
    const { target = 'native', optimize = 'ReleaseFast', simd = true, outDir = './dist/zig', linkLibc = false, } = config;
    const args = ['build'];
    if (optimize !== 'Debug') {
 ...
 */
export async function initZigProject(dir, name) {
