...
type LogColor = keyof typeof colors;

function log(message: string, color: LogColor = 'reset') {
...
async function createTestApp(scenario: TestScenario): Promise<string> {
...
async function buildTestApp(testDir: string): Promise<BundleResult> {
  try {
    // Build with rollup
...
function analyzeBundle(bundle: BundleResult, scenario: TestScenario): BundleAnalysis {
...
async function runVerification(): Promise<void> {
  ...
  for (const scenario of testScenarios) {
    ...
    try {
      ...
      const err = error as { message?: string };
      log(`  ✗ Error: ${err.message || String(error)}`, 'red');
      failedTests++;
    }
  }
...
runVerification().catch(error => {
  const err = error as { message?: string };
  log(`Fatal error: ${err.message || String(error)}`, 'red');
  console.error(error);
  process.exit(1);
});
